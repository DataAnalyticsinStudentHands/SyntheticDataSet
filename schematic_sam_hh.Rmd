---
title: "Schematic Sam Block_Group Households"
author: "Dan Price"
date: "`r Sys.Date()`"
output:
  html_document:
...
set up group quarters
```{r getting tables for group quarters}
#P18: GROUP QUARTERS POPULATION BY SEX BY AGE BY MAJOR GROUP QUARTERS TYPE
...
bgGQ_age_data <- as.data.table(bg_gq_age_data_from_census)
bgGQ_age_data[,6:ncol(bgGQ_age_data)] <- 
  bgGQ_age_data[,lapply(.SD[,6:ncol(bgGQ_age_data)], as.numeric)]
bgGQ_age_data[,c("predicateOnly","hasGeoCollectionSupport"):=NULL]
rm(bg_gq_age_data_from_census)

bgGQ <- bgGQ_age_data %>%
  pivot_longer(4:ncol(bgGQ_age_data),names_to = "geoid", values_to = "number_sams") %>% 
  ...
  mutate(beg_age_gq = case_when(age_range=="Under 18 years" ~ as.numeric(0),
         ...
         gq_type_6 = case_when(str_detect(gq_type,"College") ~ #catching some idiosyncratic capitalization
  ...
  filter(!is.na(gq_type) & str_detect(geoid,"48201")) %>% #to get rid of aggregations by institutional type
  uncount(number_sams,.id = "bgGQ_id",.remove = TRUE) 
bgGQ <- as.data.table(bgGQ) #dyplyr had stripped it of dt
paste0("Number of individuals in file: ", nrow(bgGQ))
rm(bgGQ_age_data)
```

```{r tract level group quarters}
#PCT18 GROUP QUARTERS POPULATION BY SEX BY AGE BY MAJOR GROUP QUARTERS TYPE (by race/eth)
tr_gq_race_data_from_census <- 
  censusData_byGroupName(censusdir, vintage, state, censuskey, 
                         groupname = "PCT18",county_num = "*",
                         block="tract",api_type="dec/dhc",path_suff="est.csv")
trGQ_age_data <- as.data.table(tr_gq_race_data_from_census)
trGQ_age_data[,6:ncol(trGQ_age_data)] <- 
  trGQ_age_data[,lapply(.SD[,6:ncol(trGQ_age_data)], as.numeric)]
trGQ_age_data[,c("predicateOnly","hasGeoCollectionSupport"):=NULL]
rm(tr_gq_race_data_from_census)

trGQ <- trGQ_age_data %>%
  pivot_longer(4:ncol(trGQ_age_data),names_to = "tract", values_to = "number_sams") %>% 
  filter(label != "!!Total:") %>% #keep only the ones that aren't aggregated
  mutate(tract = str_remove_all(tract,"_"),
         race = substr(name,6,6),
         race_description_gq = str_replace(concept,"GROUP QUARTERS POPULATION BY SEX BY AGE BY MAJOR GROUP QUARTERS TYPE \\(",""),
         race_description_gq = str_replace(race_description_gq,"\\)",""),
         label = str_remove_all(label,":"),
         label = str_remove_all(label,"!!Total!!")) %>%
  separate(label, c("sex","age_range","institutionalized","gq_type"), sep = "!!", remove = F, convert = FALSE) %>%
  mutate(beg_age_gq = case_when(age_range=="Under 18 years" ~ as.numeric(0),
         ...
         gq_type_6 = case_when(str_detect(gq_type,"College") ~ #catching some idiosyncratic capitalization
                             ...
                             gq_type=="Other noninstitutional facilities (701-702, 704, 706, 801-802, 900-901, 903-904)" ~
                               "Other noninstitutional facilities")) %>%
  filter(!is.na(gq_type) & str_detect(tract,"48201"))  #to get rid of aggregations by institutional type
trGQ <- as.data.table(trGQ) #dyplyr had stripped it of dt


race_codes <- c("A","B","C","D","E","F","G")
trGQ_race <- trGQ %>%
  filter(race %in% race_codes) %>%
  uncount(number_sams,.id = "gq_age_race_id")
trGQ_race <- as.data.table(trGQ_race)
paste0("Number of individuals in race file: ", nrow(trGQ_race))
#exactly 20000 female? seems unlikely, but other things track ok...

trGQ_eth <- trGQ %>%
  filter(race %in% c("H","I")) %>%
  uncount(number_sams,.id = "gq_age_eth_id")
trGQ_eth <- as.data.table(trGQ_eth)
trGQ_eth[,("HvL"):=ifelse(race=="H",TRUE,FALSE)]

#PCT19 GROUP QUARTERS POPULATION BY SEX BY AGE BY GROUP QUARTERS TYPE (same info, but no race/eth)
#tr_gq_sex_age_data_from_census <- 
#  censusData_byGroupName(censusdir, vintage, state, censuskey, 
#                         groupname = "PCT19",county_num = "*",
#                         block="tract",api_type="dec/dhc",path_suff="est.csv")

#all show up blank, presumably need special access for small numbers - tried tract and block_group, but not larger - could look for city
#PC01 	GROUP QUARTERS POPULATION BY SEX BY AGE
#PC010 (is it subset?) GROUP QUARTERS POPULATION IN OTHER NONINSTITUTIONAL FACILITIES BY SEX BY AGE
#PC02 GROUP QUARTERS POPULATION IN INSTITUTIONAL FACILITIES BY SEX BY AGE
#PC03 GROUP QUARTERS POPULATION IN CORRECTIONAL FACILITIES FOR ADULTS BY SEX BY AGE
#PC04 GROUP QUARTERS POPULATION IN JUVENILE FACILITIES BY SEX BY AGE
#PC05 GROUP QUARTERS POPULATION IN NURSING FACILITIES/SKILLED-NURSING FACILITIES BY SEX BY AGE
#PC06 	GROUP QUARTERS POPULATION IN OTHER INSTITUTIONAL FACILITIES BY SEX BY AGE
```


```{r join trGQ with bgGQ, race and eth}
#table, by tract and by geoid are often the same, so it's ok to just transfer them...
test <- table(bgGQ[,tract],bgGQ[,geoid],bgGQ[,gq_type_6])
length(test[test==0])#1042353
length(test[test>0])#507
test <- table(bgGQ[,tract],bgGQ[,gq_type_6])
length(test[test==0])#1842
length(test[test>0])#450 i.e., around 57 geoids at risk of being misassigned - almost 90% tract has only one geoid with counts and match on other factors should still keep it straight. No need to try complicated cartesian expansion on geoid and race/eth
bgGQ[,("trbgGQ_match_id"):=
                    paste0(tract,sex,age_range,gq_type_6,as.character(100000+sample(1:.N))),
                  by=.(tract,sex,age_range,gq_type_6)]
trGQ_race[,("trbgGQ_match_id"):=
                    paste0(tract,sex,age_range,gq_type_6,as.character(100000+sample(1:.N))),
                  by=.(tract,sex,age_range,gq_type_6)]
trGQ_race[,("geoid"):=
                 bgGQ[.SD, list(geoid), on = .(trbgGQ_match_id)]]
bgGQ[,c("race","race_description_gq"):=
                 trGQ_race[.SD, c(list(race),list(race_description_gq)), on = .(trbgGQ_match_id)]]
nrow(bgGQ[is.na(race)])==0
nrow(trGQ_race[is.na(geoid)])==0
#then for eth - first, if I, make sure matched with A
bgGQ[race=="A",("trbgGQ_eth_match_id"):=
                    paste0(tract,sex,age_range,gq_type_6,as.character(100000+sample(1:.N))),
                  by=.(tract,sex,age_range,gq_type_6)]
trGQ_eth[race=="I",("trbgGQ_eth_match_id"):=
                    paste0(tract,sex,age_range,gq_type_6,as.character(100000+sample(1:.N))),
                  by=.(tract,sex,age_range,gq_type_6)]
trGQ_eth[race=="I",("geoid"):=
                 bgGQ[.SD, list(geoid), on = .(trbgGQ_eth_match_id)]]
bgGQ[race=="A",("HvL"):=
                 trGQ_eth[.SD, list(HvL), on = .(trbgGQ_eth_match_id)]]
nrow(bgGQ[race=="A"&!is.na(HvL)])==nrow(trGQ_eth[race=="I"])
nrow(trGQ_eth[!is.na(geoid)])==nrow(trGQ_eth[race=="I"])
#non-I A should be an H, and subtracted
bgGQ[race=="A"&is.na(HvL),("trbgGQ_eth1_match_id"):=
                    paste0(tract,sex,age_range,gq_type_6,as.character(100000+sample(1:.N))),
                  by=.(tract,sex,age_range,gq_type_6)]
trGQ_eth[race=="H",("trbgGQ_eth1_match_id"):=
                    paste0(tract,sex,age_range,gq_type_6,as.character(100000+sample(1:.N))),
                  by=.(tract,sex,age_range,gq_type_6)]
trGQ_eth[race=="H",("geoid"):=
                 bgGQ[.SD, list(geoid), on = .(trbgGQ_eth1_match_id)]]
bgGQ[race=="A"&is.na(HvL),("HvL"):=
                 trGQ_eth[.SD, list(HvL), on = .(trbgGQ_eth1_match_id)]]
nrow(bgGQ[race=="A"])==nrow(bgGQ[!is.na(HvL)])
nrow(trGQ_eth[!is.na(geoid)])==nrow(bgGQ[!is.na(HvL)])
#then rest, with order on race
bgGQ[is.na(HvL) & order(match(race,c("F","G","C","E","B","D"))),("trbgGQ_eth2_match_id"):=
                    paste0(tract,sex,age_range,gq_type_6,as.character(100000+sample(1:.N))),
                  by=.(tract,sex,age_range,gq_type_6)]
trGQ_eth[race=="H"&is.na(geoid),("trbgGQ_eth2_match_id"):=
                    paste0(tract,sex,age_range,gq_type_6,as.character(100000+sample(1:.N))),
                  by=.(tract,sex,age_range,gq_type_6)]
trGQ_eth[race=="H"&is.na(geoid),("geoid"):=
                 bgGQ[.SD, list(geoid), on = .(trbgGQ_eth2_match_id)]]
bgGQ[is.na(HvL),("HvL"):=
                 trGQ_eth[.SD, list(HvL), on = .(trbgGQ_eth2_match_id)]]
nrow(trGQ_eth)==nrow(bgGQ[!is.na(HvL)])
bgGQ[,("HvL"):=ifelse(is.na(HvL),FALSE,HvL)]
rm(list=ls(pattern = "tr"))
```

```{r add more specifics to age by gq_type}
#some problems with approach
#https://charts.hctx.net/jailpop/App/JailPopCurrent for weights
incarceration_distribution_18_64 <- c(rep(18,258-nrow(bgGQ[beg_age_gq==0 & 
                                  gq_type=="Correctional facilities for adults (101-106)"])), #258 are either 17 or 18, and we don't want any 17s in this group
                               c(sample(19:25,1763,replace = TRUE,
                                        prob = c(.06,.09,.12,.14,.18,.2,.21))),
                               c(sample(26:35,3184,replace = TRUE,
                                        prob = c(.09,.095,.105,.11,.11,.105,.1,.1,.095,.09))),
                               c(sample(36:45,2478,replace = TRUE,
                                        prob = c(.12,.115,.11,.105,.1,.1,.095,.09,.085,.08))),
                               c(sample(46:55,1117,replace = TRUE,
                                        prob = c(.12,.115,.11,.105,.1,.1,.095,.09,.085,.08))),
                               c(sample(56:64,550,replace = TRUE,
                                        prob = c(.15,.15,.13,.12,.11,.1,.09,.08,.07))) #making 31 people 65
                               )
incarceration_weights_18_64 <- table(incarceration_distribution_18_64)/sum(incarceration_distribution_18_64)
bgGQ[,("age"):=fcase(beg_age_gq==0 & gq_type=="College/University student housing (501)",as.integer(17),
                     beg_age_gq!=0 & gq_type=="College/University student housing (501)",
                        sample(18:24,.N,replace = TRUE,prob = c(.1,.3,.2,.2,.1,.08,.02)),
                     beg_age_gq==0 & gq_type=="Correctional facilities for adults (101-106)",as.integer(17),
                     beg_age_gq==18 & gq_type=="Correctional facilities for adults (101-106)",
                        sample(18:64,.N,replace = TRUE,prob = c(incarceration_weights_18_64)), 
                     beg_age_gq==65 & gq_type=="Correctional facilities for adults (101-106)",
                        sample(65:75,.N,replace = TRUE,prob = c(.18,.17,.15,.12,.1,.09,.07,.05,.03,.025,.015)),
                     #https://www.prisonpolicy.org/reports/youth2019.html#:~:text=Of%20the%2043%2C000%20youth%20in,while%20white%20youth%20are%20underrepresented.
                     beg_age_gq==0 & gq_type=="Juvenile facilities (201-203)",
                        sample(12:17,.N,replace = TRUE,prob = c(.005,.025,.065,.25,.3,.4)),
                     beg_age_gq!=0 & gq_type=="Juvenile facilities (201-203)",as.integer(18),
                     beg_age_gq==18 & gq_type=="Nursing facilities/Skilled-nursing facilities (301)",
                        sample(51:64,.N,replace = TRUE,prob = c(1/((64:51)*2)*8)),
                     beg_age_gq==65 & gq_type=="Nursing facilities/Skilled-nursing facilities (301)",
                        sample(65:105,.N,replace = TRUE,prob = c(4/((65:105)*2))),
                     beg_age_gq==0 & str_detect(gq_type,"Other"),
                        sample(12:17,.N,replace = TRUE,prob = c(.01,.03,.06,.2,.3,.4)),
                     beg_age_gq==18 & str_detect(gq_type,"Other"),
                        sample(18:64,.N,replace = TRUE),
                     beg_age_gq==65 & str_detect(gq_type,"Other"),
                        sample(65:105,.N,replace = TRUE,prob = c(4/((65:105)*2))),
                     default = as.integer(999)
                     )]
nrow(bgGQ[age==999])==0
#remember, because of fudging, to not use this as final age; really just playing around with sample
```


```{r take bgSARE and decide who is group quarters and who is in households}
bgSARE <- read_rds(paste0(censusdir,vintage,"/working/bgSARE.RDS"))
#for Harris only
bgSARE <- bgSARE[str_starts(geoid,"48201")]
bgSARE[,("HvL"):=ifelse(str_detect(race,"NOT"),FALSE,TRUE)]
bgSARE[,("age4match"):=fcase(age<18,as.integer(age), #just to keep them aligned, but separate
                             age>17&age<65,as.integer(age/15)+as.integer(1000),
                             age>64,as.integer(age/12)+as.integer(10000)
                             )]
bgGQ[,("age4match"):=fcase(age<18,as.integer(age), #just to keep them aligned, but separate
                             age>17&age<65,as.integer(age/15)+as.integer(1000),
                             age>64,as.integer(age/12)+as.integer(10000)
                             )]
#match with bgGQ on geoid, sex, age, race, HvL
bgGQ[,("bgGQ_match_id"):=
                    paste0(geoid,sex,age4match,race,HvL,as.character(100000+sample(1:.N))),
                  by=.(geoid,sex,age4match,race,HvL)]
bgSARE[,("bgGQ_match_id"):=
                    paste0(geoid,sex,age4match,re_code,HvL,as.character(100000+sample(1:.N))),
                  by=.(geoid,sex,age4match,re_code,HvL)]
bgSARE[,c("institutionalized","gq_type"):=
                 bgGQ[.SD, list(geoid), on = .(bgGQ_match_id)]]
bgGQ[,("matched"):=
                 bgSARE[.SD, list(re_code), on = .(bgGQ_match_id)]]
nrow(bgSARE[!is.na(gq_type)]) #38887 (about 80%)
nrow(bgSARE[!is.na(gq_type)])/nrow(bgGQ)
#what's missing?
table(bgGQ[is.na(matched),race],bgGQ[is.na(matched),HvL])
nrow(bgGQ[is.na(matched)&HvL==TRUE])-nrow(bgGQ[is.na(matched)&HvL==FALSE])
nrow(bgGQ[HvL==TRUE])-nrow(bgGQ[HvL==FALSE])

#on theory that race & HvL match is from tract level, but we really want to keep gq_type at geoid...
bgGQ[is.na(matched),("bgGQ_match1_id"):=
                    paste0(geoid,sex,age4match,race,as.character(100000+sample(1:.N))),
                  by=.(geoid,sex,age4match,race)]
bgSARE[is.na(gq_type),("bgGQ_match1_id"):=
                    paste0(geoid,sex,age4match,re_code,as.character(100000+sample(1:.N))),
                  by=.(geoid,sex,age4match,re_code)]
bgSARE[is.na(gq_type),c("institutionalized","gq_type"):=
                 bgGQ[.SD, list(geoid), on = .(bgGQ_match1_id)]]
bgGQ[is.na(matched),("matched"):=
                 bgSARE[.SD, list(re_code), on = .(bgGQ_match1_id)]]
nrow(bgSARE[!is.na(gq_type)])#44331  (over 92%)
table(bgGQ[is.na(matched),race],bgGQ[is.na(matched),sex])
#then only age
bgSARE[,("age4match"):=fcase(age<18&age>15,as.integer(1000), #just to keep them aligned, but separate
                             age>17&age<65,as.integer(2000),
                             age>64,as.integer(10000)
                             )]
bgGQ[,("age4match"):=fcase(age<18&age>15,as.integer(1000), #just to keep them aligned, but separate
                             age>17&age<65,as.integer(2000),
                             age>64,as.integer(10000)
                             )]
bgGQ[is.na(matched),("bgGQ_match2_id"):=
                    paste0(geoid,age4match,as.character(100000+sample(1:.N))),
                  by=.(geoid,age4match)]
bgSARE[is.na(gq_type),("bgGQ_match2_id"):=
                    paste0(geoid,age4match,as.character(100000+sample(1:.N))),
                  by=.(geoid,age4match)]
bgSARE[is.na(gq_type),c("institutionalized","gq_type"):=
                 bgGQ[.SD, list(geoid), on = .(bgGQ_match2_id)]]
bgGQ[is.na(matched),("matched"):=
                 bgSARE[.SD, list(re_code), on = .(bgGQ_match2_id)]]
nrow(bgSARE[!is.na(gq_type)])-nrow(bgGQ) #-45 - GQ is supposed to be held invariant; have to think about whether the weirdness is attributable to differential privacy or just an error
table(bgGQ[is.na(matched),gq_type],bgGQ[is.na(matched),geoid]) #everything that doesn't match was in 3 geoids... could check...
table(bgGQ[is.na(matched),gq_type],bgGQ[is.na(matched),age]) #not exclusively, but lots of seniors in noninstitutional facilities
bgSARE[,("household"):=ifelse(is.na(institutionalized),"In household","In group quarters")]
bgSARE[,c("bgGQ_match_id","bgGQ_match1_id","bgGQ_match2_id","age4match"):=NULL]
#make sense of missing ones
```

```{r pull in hh_dec and assign the bg population to _dec.}  
bgHH_dec <- read_rds(paste0(censusdir,vintage,"/working/bgHH_dec.RDS"))
#assign two or more race children to parents, etc.
#use hh_size, largest to smallest, along with own_kids to figure out which households have no kids
#that should create a size of hh not counting kids, that let's us assign couples with no kids to two-hh, etc.
#then distribute the rest
```

```{r other future joins}
...
bg_hh_own_children_type_data_from_census <- 
  censusData_byGroupName(censusdir, vintage, state, censuskey, 
                         ...
                         block="block_group",api_type="dec/dhc",path_suff="est.csv")
bgHH_own_kids_fam_data <- as.data.table(bg_hh_own_children_type_data_from_census)
bgHH_own_kids_fam_data[,6:ncol(bgHH_own_kids_fam_data)] <- 
  bgHH_own_kids_fam_data[,lapply(.SD[,6:ncol(bgHH_own_kids_fam_data)], as.numeric)]
bgHH_own_kids_fam_data[,c("predicateOnly","hasGeoCollectionSupport"):=NULL]
rm(bg_hh_own_children_type_data_from_census)

bgHH_own_kids_fam <- bgHH_own_kids_fam_data %>%
  pivot_longer(4:ncol(bgHH_own_kids_fam_data),names_to = "geoid", values_to = "number_sams") %>% 
  mutate(geoid = str_remove_all(geoid,"_"),
         tract = substr(geoid,1,11),
         label = str_remove_all(label,":"),
         label = str_remove_all(label,"!!Total!!")) %>%
...
bgHH_own_kids_fam <- as.data.table(bgHH_own_kids_fam)
bgHH_over_65 <- bgHH_own_kids_fam[!is.na(hh_age_range)]
bgHH_own_kids_fam <- bgHH_own_kids_fam[is.na(hh_age_range)]
#join two, with "Under 65 years" added for hh_age_range - which is only for those living alone! Might not need.... 
...
bgHH_fam_kids <- bgHH_fam_kids_data %>%
  ...
  mutate(
    ...
    tract = substr(geoid,1,11),
    family = ifelse(family=="Family Households","Family households",family)
         ) %>%
...
#6 TENURE BY PRESENCE AND AGE OF OWN CHILDREN
...
trHH_own_kids_tenure <- trHH_own_kids_tenure_data %>%
  ...
  mutate(kids_age = ifelse(own_kids=="No own children under 18 years", "No own children under 18 years", kids_age)
         ) %>%
...
#6b - same stuff, without family type - but for all children? 
...
rm(tr_hh_all_children_tenure_data_from_census)
...
```
```{r get bgSARE for matching}

if (file.exists(paste0(censusdir,vintage,"/working/bgSARE.RDS"))){
  readRDS(paste0(censusdir,vintage,"/working/bgSARE.RDS"))
}
```


```{r relationship files}
...
#add two together to get bg race, etc.


#stepchild - stepchild under 18 gives you stepchild >17; adopted; biological; grandchild



#pick up trHH_SARE_data_from_census from below and double check on the bg stuff
...
trHH_SARE_data_from_census <- 
  censusData_byGroupName(censusdir, vintage="2020", state, censuskey, 
                         groupname = "PCT13",county_num = county,
                         block="tract",api_type="dec/dhc",path_suff="est.csv")#api_type="dec/sf1" for 2010
...
group_quarters_block_data_from_census <- 
  censusData_byGroupName(censusdir, vintage="2020", state, censuskey, 
                         ...
                         block="block_group",api_type="dec/dhc",path_suff="est.csv") #api_type="dec/sf1" for 2010
...
LOOKS LIKE THIS AND OTHER GQ THINGS WERE DROPPED IN 2020
