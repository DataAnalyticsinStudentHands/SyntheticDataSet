...
#triangulate with bgHH_dec - by adding age_range to dec from bgHH_age_eth by HvL... maybe need to match some others...
...
```{r join bgHH_age_race to bgHH_type on rent_own and age_range_3}
#then do the match
bgHH_age_race[,("bgHH_aer_match_id"):=
                    paste0(geoid,rent_own,hh_age_range_3,as.character(100000+sample(1:.N))),
                  by=.(geoid,rent_own,hh_age_range_3)]
bgHH_type[,("bgHH_aer_match_id"):=
                    paste0(geoid,rent_own,hh_age_range_3,as.character(100000+sample(1:.N))),
                  by=.(geoid,rent_own,hh_age_range_3)]
bgHH_type[,("race"):=
                    bgHH_age_race[.SD, list(race), on = .(bgHH_aer_match_id)]]
nrow(bgHH_age_race)==nrow(bgHH_type1[!is.na(race)])
...
#the naive match on race restricts the space of possible matches in a way that causes conflict later.
#let's double the size of bgHH_type, so that every row could possibly be either HvL or not and family or not
bgHH_type1 <- rbindlist(list(bgHH_type,bgHH_type[rep(!is.na(geoid),4,length=.N),])) 
#now do a double on family==,???? why still 19k short????

bgHH_dec[,("bgHH_ftr_match_id"):=
                    paste0(geoid,family,hh_type,race,as.character(100000+seq.int(1:.N))),
                  by=.(geoid,family,hh_type,race)]
bgHH_type[,("bgHH_ftr_match_id"):=
                    paste0(geoid,family,hh_type,race,as.character(100000+seq.int(1:.N))),
                  by=.(geoid,family,hh_type,race)]
bgHH_type[,("re_code"):=
                    bgHH_dec[.SD, list(re_code), on = .(bgHH_ftr_match_id)]]
bgHH_dec[,("matched"):=
                    bgHH_type1[.SD, list(re_code), on = .(bgHH_ftr_match_id)]]
bgHH_type1 <- bgHH_type2[!is.na(re_code)]
