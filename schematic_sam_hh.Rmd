...
Trying to think about what it means to iterate through construction / composition.
...
#move HvL from _dec to type, based on family and type
...
bgHH_dec[,("HvL_type_matched"):=
                 bgHH_type[.SD, list(HvL), on = .(bgHH_ad_match_id)]]
nrow(bgHH_type[HvL==TRUE])-nrow(bgHH_age_eth) #0 made 2309 match by prepping the geoid and HvL
nrow(bgHH_dec[HvL_type_matched==TRUE])==nrow(bgHH_type[!is.na(HvL)])
...
bgHH_type[alone=="Living alone",("bgHH_aet1_match_id"):=
                    paste0(geoid,HvL,hh_age_range_3,rent_own,family,as.character(100000+sample(1:.N))),
                  by=.(geoid,HvL,hh_age_range_3,rent_own,family)]
bgHH_age_eth[hh_size=="1-person household",c("hh_type","hh_type_fm"):=
                 bgHH_type[.SD, c(list(hh_type),list(hh_type_fm)), on = .(bgHH_aet1_match_id)]]
bgHH_type[alone=="Living alone",("age_eth_matched"):=
                 bgHH_age_eth[.SD, list(HvL), on = .(bgHH_aet1_match_id)]]
nrow(bgHH_type[alone=="Living alone"])
nrow(bgHH_age_eth[!is.na(hh_type)]) #74,020 - fewer than before
...
nrow(bgHH_age_eth[!is.na(hh_type)]) #7893
...
bgHH_age_eth[is.na(hh_type),("bgHH_aet_match_id"):=
                    paste0(tract,HvL,hh_age_range_3,rent_own,as.character(100000+sample(1:.N))),
                  by=.(tract,HvL,hh_age_range_3,rent_own)]
...
length(test[test>0]) / length(test) #42%
length(test[test>7]) / length(test) #4.5%
#have to decide how to characterize test on subtraction
#match bgHH_type1 to bgHH_age_eth, moving type and family to eth, but sampling on .N (not seq.int)
test <- table(bgHH_dec[HvL==TRUE,tract],
              bgHH_dec[HvL==TRUE,rent_own],
              bgHH_dec[HvL==TRUE,family]
              )-
...
length(test[test>0]) / length(test) #33%
length(test[test>7]) / length(test) #120 / 5%
...
nrow(bgHH_age_eth[is.na(race)]) #31,342
#let's see if taking out hh_type can help the match, since the five categories are matched in family already
#table(bgHH_age_eth[is.na(race),family],bgHH_age_eth[is.na(race),hh_type])
#                      
#                       Female householder, no spouse present Householder living alone Householder not living alone
#  Family households                                    15959                        0                            0
#  Nonfamily households                                     0                     1722                         2554
#                      
#                       Male householder, no spouse present Married couple family
#  Family households                                   3791                  7226
#  Nonfamily households                                   0                     0
...
bgHH_age_eth[is.na(race),("bgHH_dre_match_id"):=
                    paste0(geoid,family,HvL,as.character(100000+sample(1:.N))),
                  by=.(geoid,family,HvL)]
bgHH_age_eth[is.na(race),c("race","hh_type"):=
                 bgHH_dec[.SD, c(list(race),list(hh_type)), on = .(bgHH_dre_match_id)]]
bgHH_dec[is.na(matched_age_eth_race),("matched_age_eth_race"):=
                 bgHH_age_eth[.SD, list(race), on = .(bgHH_dre_match_id)]]
nrow(bgHH_age_eth[is.na(race)]) #9,026 / 1.5%
...
length(test[test==FALSE]) #3027  /length(test) = .107 -89% of matches at geoid level work

#pick up last little bit, 1.5% so that it respects _dec_eth and _age_eth
bgHH_dec[is.na(matched_age_eth_race),("bgHH_dref_match_id"):=
                    paste0(geoid,HvL,as.character(100000+sample(1:.N))),
                  by=.(geoid,HvL)]
bgHH_age_eth[is.na(race),("bgHH_dref_match_id"):=
                    paste0(geoid,HvL,as.character(100000+sample(1:.N))),
                  by=.(geoid,HvL)]
bgHH_age_eth[is.na(race),c("race","hh_type","family"):=
                 bgHH_dec[.SD, c(list(race),list(hh_type),list(family)), on = .(bgHH_dref_match_id)]]
bgHH_dec[is.na(matched_age_eth_race),("matched_age_eth_race"):=
                 bgHH_age_eth[.SD, list(race), on = .(bgHH_dref_match_id)]]
nrow(bgHH_age_eth[is.na(race)])==0
nrow(bgHH_dec[!is.na(matched_age_eth_race)])==nrow(bgHH_age_eth[!is.na(race)])
nrow(bgHH_age_eth[is.na(race)])==nrow(bgHH_age_eth[is.na(hh_type)]) #that is, we're just missing ones from earlier merges...
test <- table(
  bgHH_age_eth[,geoid],
  bgHH_age_eth[,HvL],
  bgHH_age_eth[,hh_type],
...
bgHH_age_race[,("bgHH_are_match_id"):=
                    paste0(geoid,race,rent_own,hh_age_range,as.character(100000+sample(1:.N))),
                  by=.(geoid,race,rent_own,hh_age_range)]
bgHH_age_eth[,("bgHH_are_match_id"):=
                    paste0(geoid,race,rent_own,hh_age_range,as.character(100000+sample(1:.N))),
                  by=.(geoid,race,rent_own,hh_age_range)]
bgHH_age_race[,c("HvL","hh_type","hh_type_fm","alone","family","hh_size"):=
                 bgHH_age_eth[.SD, c(list(HvL),list(hh_type),list(hh_type_fm),list(alone),
                                     list(family),list(hh_size)), on = .(bgHH_are_match_id)]]
nrow(bgHH_age_eth) - nrow(bgHH_age_race[!is.na(HvL)]) #72,204 / 12%
#we have been given the number of single people households with a senior present - which gives us some ages
#get all the way back up to bgHH_size_race in same way, so we're triangulating
bgHH_size_race[,("bgHH_asre_match_id"):=
                    paste0(tract,race,rent_own,family,hh_size,as.character(100000+sample(1:.N))),
                  by=.(tract,race,rent_own,family,hh_size)]
bgHH_age_eth[,("bgHH_asre_match_id"):=
                    paste0(tract,race,rent_own,family,hh_size,as.character(100000+sample(1:.N))),
                  by=.(tract,race,rent_own,family,hh_size)]
bgHH_size_race[,c("HvL","hh_type","hh_type_fm","alone"):=
                 bgHH_age_eth[.SD, c(list(HvL),list(hh_type),list(hh_type_fm),list(alone)), on = .(bgHH_asre_match_id)]]
...
bgHH_type[alone=="Living alone" & is.na(age_eth_matched),("bgHH_art1_match_id"):=
                    paste0(geoid,hh_age_range_3,rent_own,family,as.character(100000+sample(1:.N))),
                  by=.(geoid,hh_age_range_3,rent_own,family)]
bgHH_age_race[hh_size=="1-person household" & HvL==FALSE,c("hh_type","hh_type_fm"):=
                 bgHH_type[.SD, c(list(hh_type),list(hh_type_fm)), on = .(bgHH_art1_match_id)]]
...
bgHH_type[HvL==FALSE,("bgHH_rt_match_id"):=
                    paste0(geoid,hh_age_range_3,rent_own,as.character(100000+sample(1:.N))),
                  by=.(geoid,hh_age_range_3,rent_own)]
bgHH_age_race[is.na(family),c("hh_type","hh_type_fm","family"):=
                 bgHH_type[.SD, c(list(hh_type),list(hh_type_fm),list(family)), on = .(bgHH_rt_match_id)]]
