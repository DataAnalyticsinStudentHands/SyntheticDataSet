...
#do just race, first, then get matches from _dec??
#join tract and block group
...
bgHH_type[,c("re_code","sex_sr","alone_sr"):=
                 trHH_race_typesr[.SD, c(list(re_code),list(sex),list(alone)), on = .(trbgHH_typesr_match_id)]]
nrow(bgHH_type[!is.na(re_code)]) #125292
#have about 3,500 not matching on sex, because bgHH_type has more "Not known" - will replace with sex as available, with sex_sr to compare
...
bgHH_type[is.na(re_code),c("re_code","sex_sr","alone_sr"):=
                 trHH_race_typesr[.SD, c(list(re_code),list(sex),list(alone)), on = .(trbgHH_typesr1_match_id)]]
nrow(bgHH_type[!is.na(re_code)])
#now have 311094 matched out of 324061 (96%)
trHH_race_typesr[is.na(tr_typesr_match),("trbgHH_typesr2_match_id"):=
                    paste0(tract,hh_age_range_3,as.character(100000+sample(1:.N))),
                  by=.(tract,hh_age_range_3)]
bgHH_type[is.na(re_code),("trbgHH_typesr2_match_id"):=
                    paste0(tract,hh_age_range_3,as.character(100000+sample(1:.N))),
                  by=.(tract,hh_age_range_3)]
trHH_race_typesr[is.na(tr_typesr_match),("tr_typesr_match"):=
                 bgHH_type[.SD, list(family), on = .(trbgHH_typesr2_match_id)]]
bgHH_type[is.na(re_code),c("re_code","sex_sr","alone_sr"):=
                 trHH_race_typesr[.SD, c(list(re_code),list(sex),list(alone)), on = .(trbgHH_typesr2_match_id)]]

nrow(bgHH_type[!is.na(re_code)]) 
nrow(trHH_race_typesr)
nrow(bgHH_type[hh_age_range_3=="Householder 65 years and over" & is.na(re_code)]) #8694 - seems like they must be in different tracts!!

#since they don't line up exactly, let's treat _dec as the ground truth on this
bgHH_dec[,("trbgHH_typesre_match_id"):=
                    paste0(geoid,family,hh_type,race,as.character(100000+sample(1:.N))),
                  by=.(geoid,family,hh_type,race)]
bgHH_type[,("trbgHH_typesre_match_id"):=
                    paste0(geoid,family,hh_type,re_code,as.character(100000+sample(1:.N))),
                  by=.(geoid,family,hh_type,re_code)]
bgHH_dec[,("tr_typesr_match"):=
                 bgHH_type[.SD, list(hh_type), on = .(trbgHH_typesre_match_id)]]
bgHH_type[,("HvL"):=
                 bgHH_dec[.SD, list(HvL), on = .(trbgHH_typesre_match_id)]]
nrow(bgHH_type[!is.na(HvL)]) #310474 ???? should be 233144 tops...
...
```{r join bgHH_age_eth to size, type, and dec after cartesian expansion}
...
cart_age_eth_fam <- bgHH_age_eth[rep(1:.N,1)][,c("family","indx"):=
                                                   c(list("Family households"),list(1:.N)),
                                                 by=.(geoid,hh_age_range_3)] #don't forget to add race in when doing second iteration, although not sure it changes anything...
cart_age_eth_nonfam <- bgHH_age_eth[rep(1:.N,1)][,c("family","indx"):=
                                                   c(list("Nonfamily households"),list(1:.N)),
                                                 by=.(geoid,hh_age_range_3)]
cart_age_eth_fnf <- rbindlist(list(cart_age_eth_fam,cart_age_eth_nonfam))
cart_age_eth_fnf[,("bgHH_aes_match_id"):=
                    paste0(geoid,HvL,family,rent_own,as.character(100000+sample(1:.N))),
                  by=.(geoid,HvL,family,rent_own)]
bgHH_size_eth[,("bgHH_aes_match_id"):=
                    paste0(geoid,HvL,family,rent_own,as.character(100000+sample(1:.N))),
                  by=.(geoid,HvL,family,rent_own)]
#remember that family distribution on _dec needs to be final by geoid...
cart_age_eth_fnf[,c("hh_size","matched_age"):=
                 bgHH_size_eth[.SD, c(list(hh_size),list(family)), on = .(bgHH_aes_match_id)]]
nrow(cart_age_eth_fnf[!is.na(hh_size)])==nrow(bgHH_size_eth) 
nrow(cart_age_eth_fnf[!is.na(matched_age)])==nrow(bgHH_size_eth)
#if it works
bgHH_age_eth1 <- cart_age_eth_fnf[!is.na(matched_age)]
#and do some tests! 
...
#and then put eth and race together just on those matches.... (can compare with a statistical approach)


#then add to bgHH_dec, with the alone, and senior stuff working, then test against bgHH_type without the race/eth, for the totals
