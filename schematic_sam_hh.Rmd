...
pop_totals_hh <- bgHH_data[name=="P16_001N",4:ncol(bgHH_data)]
sum(pop_totals_hh[,],na.rm = TRUE) #==sum(pop_totals_bg[,],na.rm = TRUE)
paste0("Number of block groups: ", ncol(bgHH_data[,4:ncol(bgHH_data)]))
...
bgHH_dec <- bgHH_data %>%
  ...
  mutate(
    ...
    hispanic = if_else(str_detect(race_description,"NOT"),FALSE,TRUE),
...
#feed to test.R (doesn't work with the Hispanic in there in it's odd way)

#uncount by race and by Hispanic
bgHH_age <- bgHH_age_data %>%
  ...
  mutate(
    ...
    race_description = str_replace(concept,"TENURE BY AGE OF HOUSEHOLDER \\(",""),
    race_description = str_replace(race_description,"\\)",""),
...
nrow(bgHH_dec[hispanic==TRUE])==nrow(bgHH_age_eth)
...
```{r add hh type and tenure by race and ethnicity}
#3
...
#triangulate with bgHH_dec - by adding age_range to dec from bgHH_age_eth by hispanic... maybe need to match some others...

#bgHH_dec[,("hh_age_match_id"):=
#                    paste0(geoid,race,as.character(100000+seq.int(1:.N))),
...
bg_hh_size_tenure_data_from_census <- 
  censusData_byGroupName(censusdir, vintage, state, censuskey, 
                         ...
                         block="block_group", api_type="dec/dhc",
                         path_suff="est.csv")
#add with PCT7 (tract), which breaks these out for family and non-family, but otherwise matches...
...
bgHH_size <- bgHH_size_data %>%
  ...
  mutate(
    geoid = str_remove_all(geoid,"_"),
    race = substr(name,4,4),
    race_description = str_replace(concept,"TENURE BY HOUSEHOLD SIZE \\(",""),
    race_description = str_replace(race_description,"\\)","")
    ) %>%
...
bgHH_size_eth <- bgHH_size %>%
  filter(race=="H") %>%
  uncount(number_sams,.id = "hh_age_eth_id")
nrow(bgHH_dec[hispanic==TRUE])==nrow(bgHH_size_eth)
...
So, for every row that we would have matched by adding a sample, just do a left_join that saves multiples rows, then on the second leg of the triangle, do the sample, but back down to the smaller number.
...
Just append eth to race, and then do the matching on the full set from dec, discarding the ones from age/eth/race that don't match

```{r join bgHH_age to bgHH_type on rent_own and age_range_3}
bgHH_age_eth[,("hispanic"):=TRUE]
...
bgHH_type[,("bgHH_aet_match_id"):=
                  ...
                  by=.(geoid,rent_own,hh_age_range_3)]
bgHH_type[,("hispanic"):=
                    bgHH_age_eth[.SD, list(hispanic), on = .(bgHH_aet_match_id)]]
nrow(bgHH_age_eth)==nrow(bgHH_type[!is.na(hispanic)])
bgHH_type[is.na(hispanic),("hispanic"):=FALSE]

test <- table(
  bgHH_type[hispanic==TRUE,geoid],
  bgHH_type[hispanic==TRUE,rent_own],
  bgHH_type[hispanic==TRUE,hh_age_range_3],
  bgHH_type[hispanic==TRUE,hispanic]
) == table(
  bgHH_age_eth[,geoid],
  bgHH_age_eth[,rent_own],
  bgHH_age_eth[,hh_age_range_3],
  bgHH_age_eth[,hispanic]
)
length(test[test==FALSE])==0

#add race from bgHH_dec on family, hh_type and hispanic - add the amount that didn't match by duplicating categories???
#what if we just doubled the size of the folks that have hispanic == TRUE
#point is that each row that is hispanic either should have been or should not have been, but that a bunch are now seen as only being possibly not and thus are excluded. Once larger for hispanic and once larger for rent_own?
#this made it worse! with only one, it actually matched better!!
bgHH_type1 <- rbindlist(list(bgHH_type,bgHH_type[rep(hispanic==TRUE,2,length=.N),])) 
bgHH_type1a <- rbindlist(list(bgHH_type1,bgHH_type[rep(hispanic==FALSE,2,length=.N),])) 
...
bgHH_dec[,("bgHH_ft_match_id"):=
                    paste0(geoid,family,hh_type,hispanic,as.character(100000+seq.int(1:.N))),
                  by=.(geoid,family,hh_type,hispanic)]
...
test <- table(
  bgHH_type[is.na(hispanic),geoid],
  bgHH_type[is.na(hispanic),rent_own],
  bgHH_type[is.na(hispanic),hh_age_range_3],
  bgHH_type[is.na(hispanic),hispanic]
) == table(
  bgHH_age_eth[,geoid],
  bgHH_age_eth[,rent_own],
  bgHH_age_eth[,hh_age_range_3],
...
#compare with 10 and 11 / could use just for testing that the others still hold, but probably don't need
bg_hh_race_eth_tenure_data_from_census <- 
  censusData_byGroupName(censusdir, vintage, state, censuskey, 
                         groupname = "H7",county_num = "*",
                         block="block_group",api_type="dec/dhc",path_suff="est.csv")
bg_hh_tenure_data_from_census <- 
  censusData_byGroupName(censusdir, vintage, state, censuskey, 
                         groupname = "H10",county_num = "*",
                         block="block_group",api_type="dec/dhc",path_suff="est.csv")

bg_hh_tenure_eth_data_from_census <- 
  censusData_byGroupName(censusdir, vintage, state, censuskey, 
                         groupname = "H11",county_num = "*",
                         block="block_group",api_type="dec/dhc",path_suff="est.csv")
...
bg_hh_type_relation_data_from_census <-  #seems to have all the categories of PCT17, which also has race/ethnicity...
  censusData_byGroupName(censusdir, vintage, state, censuskey, 
                         ...
                         block="block_group",api_type="dec/dhc",path_suff="est.csv")
#7
bg_hh_seniors_data_from_census <- 
  censusData_byGroupName(censusdir, vintage, state, censuskey, 
                         groupname = "P19",county_num = "*",
                         block="block_group",api_type="dec/dhc",path_suff="est.csv")
#and PCT3: HOUSEHOLD TYPE BY AGE OF HOUSEHOLDER and ; and PCT9 HOUSEHOLD TYPE BY RELATIONSHIP FOR THE POPULATION 65 YEARS AND OVER
