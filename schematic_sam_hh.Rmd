...
bgHH_type[alone!="Living alone",c("age_eth_matched"):=
...
#could make the HvL_ae move over the value of HvL that came from dec on type
```
...
nrow(bgHH_age_eth[is.na(race)]) #41875 / 93% matched on all
...
#put tr family from size and bg hh_size onto bg_age_race, then do matching by subtraction
...
#hh_age_range_3 could be wrong in match coming from dec - let it not match, but don't change on age_rac
...
#TEST WITH HH_TYPE ON MATCH, THEN MOVE IT IF NEEDED!
...
nrow(bgHH_dec[is.na(family_ar)])

#move type to age_race - MAY NEED TO RETHINK WHAT MATCHES HERE - FOLLOW THE ETH MODEL??
