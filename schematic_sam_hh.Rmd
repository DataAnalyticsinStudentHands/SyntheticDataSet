...
#then do the match
#or expand by family already, since we don't know on it??
...
bgHH_age_race1[,("bgHH_taer_match_id"):=
...
#double up on the side with smaller difference, for more matches? should be after??? really just about stats, still
...
bgHH_type1[,("bgHH_taer_match_id"):=
                    paste0(geoid,family,rent_own,hh_age_range_3,race,HvL,as.character(100000+seq.int(1:.N))),
                  by=.(geoid,family,rent_own,hh_age_range_3,race,HvL)]
bgHH_type1[,c("hh_age_range","hh_age_range_3"):=
                    bgHH_age_race1[.SD, c(list(hh_age_range),list(hh_age_range_3)), on = .(bgHH_taer_match_id)]]
bgHH_age_race1[,c("matched","hh_type"):=
                    bgHH_type1[.SD, c(list(race),list(hh_type)), on = .(bgHH_taer_match_id)]]
#do tests to just make sure basic steps are being performed
nrow(bgHH_age_race1[!is.na(matched)])==nrow(bgHH_type1[!is.na(hh_age_range)])
test <- table(
  ...
  bgHH_age_race1[!is.na(matched),geoid],
  bgHH_age_race1[!is.na(matched),family],
  bgHH_age_race1[!is.na(matched),rent_own],
  bgHH_age_race1[!is.na(matched),hh_age_range],
  bgHH_age_race1[!is.na(matched),hh_age_range_3],
  bgHH_age_race1[!is.na(matched),race],
  bgHH_age_race1[!is.na(matched),HvL]
)
length(test[test==FALSE])==0
...
bgHH_type2[,("bgHH_ftr_match_id"):=
                    paste0(geoid,family,hh_type,race,HvL,as.character(100000+seq.int(1:.N))),
                  by=.(geoid,family,hh_type,race,HvL)]
bgHH_dec[,c("rent_own","hh_age_range","hh_age_range_3"):=
                    bgHH_type2[.SD, c(list(rent_own),list(hh_age_range),list(hh_age_range_3)), on = .(bgHH_ftr_match_id)]]
bgHH_type2[,("matched2"):=
                    bgHH_dec[.SD, list(re_code), on = .(bgHH_ftr_match_id)]]

nrow(bgHH_type)==nrow(bgHH_dec[!is.na(rent_own)])
