...
```{r add details on age data from tract level}
#add specifics from tract level by age
tr_hh_type_age_data_from_census <- 
  censusData_byGroupName(censusdir, vintage, state, censuskey, 
                         groupname = "PCT3",county_num = "*",
                         block="tract",api_type="dec/dhc",path_suff="est.csv")
trHH_type_data <- as.data.table(tr_hh_type_age_data_from_census)
trHH_type_data[,6:ncol(trHH_type_data)] <- 
  trHH_type_data[,lapply(.SD[,6:ncol(trHH_type_data)], as.numeric)]
trHH_type_data[,c("predicateOnly","hasGeoCollectionSupport"):=NULL]
rm(tr_hh_type_age_data_from_census)

#make long
trHH_type <- trHH_type_data %>%
  filter(label!=" !!Total:") %>%
  mutate(label=str_remove(label,"!!Total:!!"),
         label=str_remove(label,"!!Total:")) %>%
  pivot_longer(4:ncol(trHH_type_data),names_to = "tract", values_to = "number_sams",
               names_transform = list(tract=as.character)) %>%
  filter(str_detect(tract,"48201")) %>% #for test, only Harris
  separate(label, c("family","hh_age_range_9"), sep = ":!!", remove = F, convert = FALSE) %>%
  mutate(
    hh_age_range_3 = case_when(hh_age_range_9=="Householder 15 to 24 years" ~ "Householder 15 to 34 years",
                               hh_age_range_9=="Householder 25 to 34 years" ~ "Householder 15 to 34 years",
                               hh_age_range_9=="Householder 35 to 44 years" ~ "Householder 35 to 64 years",
                               hh_age_range_9=="Householder 45 to 54 years" ~ "Householder 35 to 64 years",
                               hh_age_range_9=="Householder 55 to 59 years" ~ "Householder 35 to 64 years",
                               hh_age_range_9=="Householder 60 to 64 years" ~ "Householder 35 to 64 years",
                               hh_age_range_9=="Householder 65 to 74 years" ~ "Householder 65 years and over",
                               hh_age_range_9=="Householder 75 to 84 years" ~ "Householder 65 years and over",
                               hh_age_range_9=="Householder 85 years and over" ~ "Householder 65 years and over")
    ) %>%
  filter(number_sams > 0, !is.na(hh_age_range_9)) %>% 
  select(-c(name,label,concept)) %>%
  uncount(number_sams,.id = "hh_tr_type_age_id")
nrow(bgHH_dec)==nrow(trHH_type)
trHH_type <- as.data.table(trHH_type)

#join tract and block group
trHH_type[,("trbgHH_type_match_id"):=
                    paste0(tract,family,hh_age_range_3,as.character(100000+sample(1:.N))),
                  by=.(tract,family,hh_age_range_3)]
bgHH_type[,("trbgHH_type_match_id"):=
                    paste0(tract,family,hh_age_range_3,as.character(100000+sample(1:.N))),
                  by=.(tract,family,hh_age_range_3)]
trHH_type[,("tr_type_match"):=
                 bgHH_type[.SD, list(family), on = .(trbgHH_type_match_id)]]
bgHH_type[,c("tr_type_match","hh_age_range_9"):=
                 trHH_type[.SD, c(list(family),list(hh_age_range_9)), on = .(trbgHH_type_match_id)]]
nrow(bgHH_type[is.na(hh_age_range_9)]) #2031743
...
#then H - the table for bgHH_dec will be our ground, although it doesn't have age
...
nrow(bgHH_type[!is.na(re_code)]) #125292
...
cart_bgHH_size_eth_nonfamily <- bgHH_size_eth[rep(1:.N,1)][,c("family","indx"):=
                                                 ...
                                                 by=.(geoid,hh_size)]
cart_HvL_bgHH_size <- rbindlist(list(cart_bgHH_size_eth_family,cart_bgHH_size_eth_nonfamily))
#now take the expansion by family/non-family, and put them in all possible block_group combinations
cart_trHH_size_eth <- cart_HvL_bgHH_size[rep(1:.N,maxblocks)][,("indx"):=
                                                   list(1:.N),
                                                 by=.(geoid,hh_size)] 
...
bg_max_blocks <- bgHH_size_eth[,c("tract","maxblocks")]
...
trHH_size_eth_maxblocks <- trHH_size_eth[rep(1:.N,max(bgHH_size_eth[,maxblocks]))]
trHH_size_eth_maxblocks[,("geoid"):=paste0(tract,1:.N),by=.(id4rep)] #do for all possible geoids / 6 is max number of blocks
#match to reduce for sake of expand with rent_own
...
trHH_size_eth_maxblocks[,("trbgHH_sizeHvL_match_id"):=
                    paste0(geoid,HvL,family,hh_size,as.character(100000+sample(1:.N))),
                  by=.(geoid,HvL,family,hh_size)]
trHH_size_eth_maxblocks[,("bg_size_match"):=
                 cart_trHH_size_eth[.SD, list(hh_size), on = .(trbgHH_sizeHvL_match_id)]]
cart_trHH_size_eth[,("bg_size_match"):=
                 trHH_size_eth_maxblocks[.SD, list(hh_size), on = .(trbgHH_sizeHvL_match_id)]]
nrow(cart_trHH_size_eth[!is.na(bg_size_match)])
#see if we have right number, although we don't know which of the duplicated id4reps is the right match...
length(unique(trHH_size_eth_maxblocks[!is.na(bg_size_match),id4rep]))==nrow(trHH_size_eth)

#can use this to show that just reducing to unique doesn't get right subset
#trHH_match_eth_size <- trHH_size_eth_maxblocks[!is.na(bg_size_match)&!duplicated(id4rep)] #519420 - 
#reduce then add rent_own
trHH_match_eth_size <- trHH_size_eth_maxblocks[!is.na(bg_size_match)]
...
nrow(cart_size_eth_fnf[!is.na(size_match)])
nrow(cart_size_eth_or[!duplicated(id4rep)])==nrow(bgHH_size_eth)
...
nrow(trHH_size_eth[is.na(size_matchf)]) #8743 -i.e., almost 98.5% match...
#get further matches from cart_size_eth_or, in order to keep family/hh_size/matches...
#then by tract
...
nrow(trHH_size_eth[is.na(size_matchf)]) #4592 - not much added... more than half remaining
...
length(test[test==0])/length(test) #14%
length(test[test<5])/length(test) #87%
length(test[test<25])/length(test) #99.9%
...
cart_race_bgHH_size <- rbindlist(list(cart_bgHH_size_race_family,cart_bgHH_size_race_nonfamily))
#now take the expansion by family/non-family, and put them in all possible block_group combinations
cart_trHH_size_race <- cart_race_bgHH_size[rep(1:.N,maxblocks)][,("indx"):=
                                                   list(1:.N),
                                                 by=.(geoid,race,hh_size)] 
#should be faster way to get a unique value on each; not sure
trHH_size_race[,("id4rep"):=paste0(tract,race,hh_size,family,tr_hh_size_race_id)]
#put maxblocks from bg to tr
bg_max_blocks <- bgHH_size_race[,c("tract","maxblocks")]
bg_max_blocks[,("rownumber"):=1:.N,by=.(tract)]
trHH_size_race[,("rownumber"):=1:.N,by=.(tract)]
trHH_size_race <- bg_max_blocks[trHH_size_race,on=.(tract,rownumber)]
trHH_size_race_maxblocks <- trHH_size_race[rep(1:.N,max(bgHH_size_race[,maxblocks]))]
trHH_size_race_maxblocks[,("geoid"):=paste0(tract,1:.N),by=.(id4rep)] #do for all possible geoids / 6 is max number of blocks
rm(bg_max_blocks)
...
trHH_size_race_maxblocks[,("trbgHH_sizerace_match_id"):=
                    paste0(geoid,race,family,hh_size,as.character(100000+sample(1:.N))),
                  by=.(geoid,race,family,hh_size)]
trHH_size_race_maxblocks[,("bg_size_match"):=
                 cart_trHH_size_race[.SD, list(hh_size), on = .(trbgHH_sizerace_match_id)]]
cart_trHH_size_race[,("bg_size_match"):=
                 trHH_size_race_maxblocks[.SD, list(hh_size), on = .(trbgHH_sizerace_match_id)]]
nrow(cart_trHH_size_race[!is.na(bg_size_match)]) #3928088
#see if we have right number, although we don't know which of the duplicated id4reps is the right match... - we lost some here!!!
length(unique(trHH_size_race_maxblocks[!is.na(bg_size_match),id4rep])) == nrow(trHH_size_race)

#reduce then add rent_own
trHH_race_size_reduced <- trHH_size_race_maxblocks[!is.na(bg_size_match)]
...
nrow(cart_size_race_fnf[!is.na(size_match)]) #2031743
nrow(cart_size_race_or[!duplicated(id4rep)]) == nrow(bgHH_size_race)
#try going to trHH_size_eth with geoid and rent_own
...
1-length(test[test>5])/length(test) #83% within 5 families on rent_own
...
nrow(trHH_size_race[is.na(size_matchf)]) #27247 -i.e., almost 98.4% match...
#then by tract
...
nrow(trHH_size_race[is.na(size_matchf)]) #14471 - about half remaining
...
length(test[test==0])/length(test) #8.4% exact match - need _dec to be the ground truth for family
1-length(test[test>5])/length(test) #76% less than 5 families off
1-length(test[test>10])/length(test) #89% less than 10 families off
1-length(test[test>25])/length(test) #99% less than 25 families off
#at tract levels, family will match - at geoid, should allow family from _dec to rule the day
...
nrow(bgHH_size_eth[is.na(re_code)]) #2560 left - _dec will be final
...
cart_age_race[,("bgHH_art_match_id"):=
                    paste0(geoid,race,HvL,family,rent_own,hh_age_range,as.character(100000+sample(1:.N))),
                  by=.(geoid,race,HvL,family,rent_own,hh_age_range)]
bgHH_type[,("bgHH_art_match_id"):=
                    paste0(geoid,re_code,HvL,family,rent_own,hh_age_range_9,as.character(100000+sample(1:.N))),
                  by=.(geoid,re_code,HvL,family,rent_own,hh_age_range_9)]
...
nrow(bgHH_type[hh_age_range_3=="Householder 65 years and over"]) - nrow(cart_age_race[!is.na(hh_type)]) #50k
...
cart_age_race[is.na(hh_type),("bgHH_art1_match_id"):=
                    paste0(geoid,family,rent_own,hh_age_range,as.character(100000+sample(1:.N))),
                  by=.(geoid,family,rent_own,hh_age_range)]
bgHH_type[is.na(race),("bgHH_art1_match_id"):=
                    paste0(geoid,family,rent_own,hh_age_range_9,as.character(100000+sample(1:.N))),
                  by=.(geoid,family,rent_own,hh_age_range_9)]
...
nrow(bgHH_type[is.na(race)]) #6325
#get last matches at tract level, since family may not match at geoid
#cart_age_race[is.na(hh_type),("bgHH_art2_match_id"):=
#                    paste0(tract,family,rent_own,hh_age_range,as.character(100000+sample(1:.N))),
#                  by=.(tract,family,rent_own,hh_age_range)]
#bgHH_type[is.na(race),("bgHH_art2_match_id"):=
#                    paste0(tract,family,rent_own,hh_age_range_9,as.character(100000+sample(1:.N))),
#                  by=.(tract,family,rent_own,hh_age_range_9)]
#bgHH_type[is.na(race),c("race","HvL"):=
#                 cart_age_race[.SD, c(list(race),list(HvL)), on = .(bgHH_art2_match_id)]]
#cart_age_race[is.na(hh_type),c("hh_type","matched_age"):=
#                 bgHH_type[.SD, c(list(hh_type),list(family)), on = .(bgHH_art2_match_id)]]
#nrow(cart_age_race[!is.na(hh_type)])
#nrow(bgHH_type[is.na(race)]) / nrow(bgHH_type) #1326 - .08%
##then to get the final ones
#cart_age_race[is.na(hh_type),("bgHH_art3_match_id"):=
#                    paste0(tract,hh_age_range,as.character(100000+sample(1:.N))),
#                  by=.(tract,hh_age_range)]
#bgHH_type[is.na(race),("bgHH_art3_match_id"):=
#                    paste0(tract,hh_age_range_9,as.character(100000+sample(1:.N))),
#                  by=.(tract,hh_age_range_9)]
#bgHH_type[is.na(race),c("race","HvL"):=
#                 cart_age_race[.SD, c(list(race),list(HvL)), on = .(bgHH_art3_match_id)]]
#cart_age_race[is.na(hh_type),c("hh_type","matched_age"):=
#                 bgHH_type[.SD, c(list(hh_type),list(family)), on = .(bgHH_art3_match_id)]]
#nrow(cart_age_race[!is.na(hh_type)])
#nrow(bgHH_type[is.na(race)]) == 0
#some tests
#NOT TOO FAR OFF, BUT NEED TO HAVE _DEC BE THE FINAL IN ANY CASE, SO DON'T NEED THIS 
#> table(bgHH_type[,race])
#
#     A      B      C      D      E      F      G 
#708064 349595  15816 117625   1114 256310 244206 
#> table(bgHH_age_race[,race])
#
#     A      B      C      D      E      F      G 
#699864 351342  16450 120083   1158 258206 245627 
#type to dec ; size to dec??
rm(list=ls(pattern='^cart'))
```

What am I thinking??? age_race before _size? or add age and size together...would the Cartesian work better for them??

```{r add _age race and eth to _type}
bgHH_age_race[,("bgHH_at_match_id"):=
                    paste0(geoid,race,hh_age_range,rent_own,as.character(100000+sample(1:.N))),
                  by=.(geoid,race,hh_age_range,rent_own)]
bgHH_type[,("bgHH_at_match_id"):=
                    paste0(geoid,re_code,hh_age_range_9,rent_own,as.character(100000+sample(1:.N))),
                  by=.(geoid,re_code,hh_age_range_9,rent_own)]
#sex_sr, alone_sr info is all given in crosstabs with age
bgHH_type[,("race"):=
                 bgHH_age_race[.SD, list(race), on = .(bgHH_at_match_id)]]
...
bgHH_age_race[,c("hh_type","hh_type_fm","alone","sex"):=
                 bgHH_type[.SD, c(list(hh_type),list(hh_type_fm),list(alone),list(sex)), 
                           on = .(bgHH_at_match_id)]]
nrow(bgHH_type[is.na(race)])-nrow(bgHH_type[is.na(re_code)]) #27891
#re_code and HvL only for seniors, and all but 28k matched, so remove
bgHH_age_race[is.na(hh_type),("bgHH_at_match1_id"):=
                    paste0(geoid,hh_age_range,rent_own,as.character(100000+sample(1:.N))),
                  by=.(geoid,hh_age_range,rent_own)]
...
bgHH_type[is.na(race),("race"):=
                 bgHH_age_race[.SD, list(race), on = .(bgHH_at_match1_id)]]
bgHH_age_race[is.na(hh_type),c("hh_type","hh_type_fm","alone","sex"):=
                 bgHH_type[.SD, c(list(hh_type),list(hh_type_fm),list(alone),list(sex)), 
                           on = .(bgHH_at_match1_id)]]
nrow(bgHH_type[is.na(race)]) #36300 (~2% not matched)
nrow(bgHH_size_race[is.na(hh_type)])

```

```{r add _size to _type}
bgHH_size_race[,("bgHH_st_match_id"):=
                    paste0(geoid,race,HvL,family,rent_own,as.character(100000+sample(1:.N))),
                  by=.(geoid,race,HvL,family,rent_own)]
bgHH_type[,("bgHH_st_match_id"):=
                    paste0(geoid,re_code,HvL,family,rent_own,as.character(100000+sample(1:.N))),
                  by=.(geoid,re_code,HvL,family,rent_own)]
#sex_sr, alone_sr info is all given in crosstabs with age
bgHH_type[,("hh_size"):=
                 bgHH_size_race[.SD, list(hh_size), on = .(bgHH_st_match_id)]]
bgHH_size_race[,c("hh_type","hh_type_fm","alone","sex"):=
                 bgHH_type[.SD, c(list(hh_type),list(hh_type_fm),list(alone),list(sex)), 
                           on = .(bgHH_st_match_id)]]
nrow(bgHH_type[is.na(hh_size)])-nrow(bgHH_type[is.na(re_code)]) #27891
#re_code and HvL only for seniors, and all but 28k matched, so remove
bgHH_size_race[is.na(hh_type),("bgHH_st_match1_id"):=
                    paste0(geoid,family,rent_own,as.character(100000+sample(1:.N))),
                  by=.(geoid,family,rent_own)]
bgHH_type[is.na(hh_size),("bgHH_st_match1_id"):=
                    paste0(geoid,family,rent_own,as.character(100000+sample(1:.N))),
                  by=.(geoid,family,rent_own)]
bgHH_type[is.na(hh_size),("hh_size"):=
                 bgHH_size_race[.SD, list(hh_size), on = .(bgHH_st_match1_id)]]
bgHH_size_race[is.na(hh_type),c("hh_type","hh_type_fm","alone","sex"):=
                 bgHH_type[.SD, c(list(hh_type),list(hh_type_fm),list(alone),list(sex)), 
                           on = .(bgHH_st_match1_id)]]
nrow(bgHH_type[is.na(hh_size)]) #36300 (~2% not matched)
nrow(bgHH_size_race[is.na(hh_type)])
...
bgHH_dec[,c("rent_own","hh_age_range_3","hh_age_range_9","hh_type_fm","alone"):=
                 bgHH_type[.SD, c(list(rent_own),list(hh_age_range_3),list(hh_age_range_9),
...
bgHH_type[alone=="Living alone" & is.na(age_eth_matched),("HvL_ae"):=ifelse(HvL==TRUE,FALSE,FALSE)]

#one person households with HvL_ae coming from age, potentially overriding from dec - 
#if select on subset and then overwrite HvL directly, it causes older values of HvL to go NA
...
```{r get age_race matches with age_eth (dec)}
...
#THIS IS MUCH CLOSER NOW, SO TRY AGAIN - 
#family and hh_size could be wrong by geoid, so try with and without 
bgHH_age_race[is.na(HvL),("bgHH_aref_match_id"):=
                    paste0(geoid,race,family,rent_own,hh_age_range_3,as.character(100000+sample(1:.N))),
                  by=.(geoid,race,family,rent_own,hh_age_range_3)]
...
bgHH_age_eth[is.na(matched_eth_race),c("matched_eth_race","family_ar", #put family_ar here for comparison
                                       "hh_size_ar"):= #eventually move back to _dec???
                 bgHH_age_race[.SD, c(list(race),list(family),list(hh_size)), 
                               on = .(bgHH_aref_match_id)]]
nrow(bgHH_age_eth) - nrow(bgHH_age_race[!is.na(HvL)]) #55043 / 9%
...
bgHH_age_race[is.na(HvL),("bgHH_ref_match_id"):=
                    paste0(geoid,race,family,rent_own,as.character(100000+sample(1:.N))),
                  by=.(geoid,race,family,rent_own)]
...
nrow(bgHH_age_eth) - nrow(bgHH_age_race[!is.na(HvL)]) #22,895
#then without family
#hh_age_range_3 could be wrong in match coming from dec - let it not match, but don't change on age_race
bgHH_age_race[is.na(HvL),("bgHH_re_match_id"):=
                    paste0(geoid,race,rent_own,as.character(100000+sample(1:.N))),
                  by=.(geoid,race,rent_own)]
bgHH_age_eth[is.na(matched_eth_race),("bgHH_re_match_id"):=
                    paste0(geoid,race,rent_own,as.character(100000+sample(1:.N))),
                  by=.(geoid,race,rent_own)]
bgHH_age_race[is.na(HvL),c("HvL","hh_type","hh_type_fm","alone"):= 
                 bgHH_age_eth[.SD, c(list(HvL),list(hh_type),list(hh_type_fm),
                                     list(alone)), on = .(bgHH_re_match_id)]]
bgHH_age_eth[is.na(matched_eth_race),c("matched_eth_race","family_ar",
                                       "hh_size_ar","hh_age_range_3_ar"):=
                 bgHH_age_race[.SD, c(list(race),list(family),list(hh_size),
                                      list(hh_age_range_3)), on = .(bgHH_re_match_id)]]
nrow(bgHH_age_eth) - nrow(bgHH_age_race[!is.na(HvL)]) #14329
...
Need to draw all this / diagram!
```{r move type to age_race}
#starting with 1-person households
bgHH_age_race[is.na(HvL),("HvL"):=FALSE]
bgHH_age_race[hh_size=="1-person household",("bgHH_tare_match_id"):=
                    paste0(geoid,HvL,hh_type,race,rent_own,family,hh_age_range_3,as.character(100000+sample(1:.N))),
                  by=.(geoid,HvL,hh_type,race,rent_own,family,hh_age_range_3)]
bgHH_type[alone=="Living alone",("bgHH_tare_match_id"):=
                    paste0(geoid,HvL_ae,hh_type,race,rent_own,family,hh_age_range_3_ae,as.character(100000+sample(1:.N))),
                  by=.(geoid,HvL_ae,hh_type,race,rent_own,family,hh_age_range_3_ae)]
bgHH_age_race[hh_size=="1-person household",c("hh_type_t","hh_type_fm_t","alone_t"):=
                 bgHH_type[.SD, c(list(hh_type),list(hh_type_fm),
                                     list(alone)), on = .(bgHH_tare_match_id)]]
bgHH_type[alone=="Living alone",("matched_type_race"):=
                 bgHH_age_race[.SD, list(race), on = .(bgHH_tare_match_id)]]
nrow(bgHH_age_race[!is.na(hh_type_t) & hh_size=="1-person household"]) #68827
#without HvL of hh_type, because the _ae gave it more chances to match, and it's relatively 
#uncertain on bgHH_type (keep from age_race); hh_type is only for HvL on age_race, and some will not match on HvL here
bgHH_age_race[hh_size=="1-person household" & is.na(hh_type_t),("bgHH_tare1_match_id"):=
...
bgHH_type[alone=="Living alone" & is.na(matched_type_race),("bgHH_tare1_match_id"):=
                    paste0(geoid,race,rent_own,family,hh_age_range_3_ae,as.character(100000+sample(1:.N))),
                  by=.(geoid,race,rent_own,family,hh_age_range_3_ae)]
bgHH_age_race[hh_size=="1-person household" & is.na(hh_type_t),c("hh_type_t","hh_type_fm_t","alone_t"):=
                 bgHH_type[.SD, c(list(hh_type),list(hh_type_fm),
                                     list(alone)), on = .(bgHH_tare1_match_id)]]
bgHH_type[alone=="Living alone" & is.na(matched_type_race),("matched_type_race"):=
                 bgHH_age_race[.SD, list(race), on = .(bgHH_tare1_match_id)]]
nrow(bgHH_age_race[!is.na(hh_type_t)]) # 
nrow(bgHH_type[alone=="Living alone"]) #433934
#without family, since it may be wrong on the geoid b/c it comes from tract level
bgHH_age_race[hh_size=="1-person household" & is.na(hh_type_t),("bgHH_tare2_match_id"):=
                    paste0(geoid,race,rent_own,hh_age_range_3,as.character(100000+sample(1:.N))),
                  by=.(geoid,race,rent_own,hh_age_range_3)]
bgHH_type[alone=="Living alone" & is.na(matched_type_race),("bgHH_tare2_match_id"):=
                    paste0(geoid,race,rent_own,hh_age_range_3_ae,as.character(100000+sample(1:.N))),
                  by=.(geoid,race,rent_own,hh_age_range_3_ae)]
bgHH_age_race[hh_size=="1-person household" & is.na(hh_type_t),c("hh_type_t","hh_type_fm_t","alone_t"):=
                 bgHH_type[.SD, c(list(hh_type),list(hh_type_fm),
                                     list(alone)), on = .(bgHH_tare2_match_id)]]
bgHH_type[alone=="Living alone" & is.na(matched_type_race),("matched_type_race"):=
                 bgHH_age_race[.SD, list(race), on = .(bgHH_tare2_match_id)]]
nrow(bgHH_age_race[!is.na(hh_type_t)]) #350190 / 15% missing
nrow(bgHH_type[alone=="Living alone"]) #433934
#without age_range_3
bgHH_age_race[hh_size=="1-person household" & is.na(hh_type_t),("bgHH_tare3_match_id"):=
                    paste0(geoid,race,rent_own,as.character(100000+sample(1:.N))),
                  by=.(geoid,race,rent_own)]
bgHH_type[alone=="Living alone" & is.na(matched_type_race),("bgHH_tare3_match_id"):=
                    paste0(geoid,race,rent_own,as.character(100000+sample(1:.N))),
                  by=.(geoid,race,rent_own)]
bgHH_age_race[hh_size=="1-person household" & is.na(hh_type_t),c("hh_type_t","hh_type_fm_t","alone_t"):=
                 bgHH_type[.SD, c(list(hh_type),list(hh_type_fm),
                                     list(alone)), on = .(bgHH_tare3_match_id)]]
bgHH_type[alone=="Living alone" & is.na(matched_type_race),("matched_type_race"):=
                 bgHH_age_race[.SD, list(race), on = .(bgHH_tare3_match_id)]]
nrow(bgHH_age_race[!is.na(hh_type_t)]) #400584 / 15% missing
nrow(bgHH_type[alone=="Living alone"]) #433934
#and only race
bgHH_age_race[hh_size=="1-person household" & is.na(hh_type_t),("bgHH_tare4_match_id"):=
                    paste0(geoid,race,as.character(100000+sample(1:.N))),
                  by=.(geoid,race)]
bgHH_type[alone=="Living alone" & is.na(matched_type_race),("bgHH_tare4_match_id"):=
                    paste0(geoid,race,as.character(100000+sample(1:.N))),
                  by=.(geoid,race)]
bgHH_age_race[hh_size=="1-person household" & is.na(hh_type_t),c("hh_type_t","hh_type_fm_t","alone_t"):=
                 bgHH_type[.SD, c(list(hh_type),list(hh_type_fm),
                                     list(alone)), on = .(bgHH_tare4_match_id)]]
bgHH_type[alone=="Living alone" & is.na(matched_type_race),("matched_type_race"):=
                 bgHH_age_race[.SD, list(race), on = .(bgHH_tare4_match_id)]]
nrow(bgHH_age_race[!is.na(hh_type_t)]) == nrow(bgHH_type[alone=="Living alone"]) #433934

#then do it for hh_size!="1-person household" and alone!="Living alone" - which is just is.na(hh_type_t)
bgHH_age_race[is.na(hh_type_t),("bgHH_ftare_match_id"):=
                    paste0(geoid,HvL,hh_type,race,rent_own,family,hh_age_range_3,as.character(100000+sample(1:.N))),
                  by=.(geoid,HvL,hh_type,race,rent_own,family,hh_age_range_3)]
bgHH_type[is.na(matched_type_race),("bgHH_ftare_match_id"):=
                    paste0(geoid,HvL_ae,hh_type,race,rent_own,family,hh_age_range_3_ae,as.character(100000+sample(1:.N))),
                  by=.(geoid,HvL_ae,hh_type,race,rent_own,family,hh_age_range_3_ae)]
bgHH_age_race[is.na(hh_type_t),c("hh_type_t","hh_type_fm_t","alone_t"):=
                 bgHH_type[.SD, c(list(hh_type),list(hh_type_fm),
                                     list(alone)), on = .(bgHH_ftare_match_id)]]
bgHH_type[is.na(matched_type_race),("matched_type_race"):=
                 bgHH_age_race[.SD, list(race), on = .(bgHH_ftare_match_id)]]
nrow(bgHH_age_race[!is.na(hh_type_t)]) / nrow(bgHH_type) #838302/ 49.5%
#without HvL of hh_type, because the _ae gave it more chances to match, and it's relatively 
#uncertain on bgHH_type (keep from age_race); hh_type is only for HvL on age_race, and some will not match on HvL here
bgHH_age_race[is.na(hh_type_t),("bgHH_ftare1_match_id"):=
                    paste0(geoid,race,rent_own,family,hh_age_range_3,as.character(100000+sample(1:.N))),
                  by=.(geoid,race,rent_own,family,hh_age_range_3)]
bgHH_type[is.na(matched_type_race),("bgHH_ftare1_match_id"):=
                    paste0(geoid,race,rent_own,family,hh_age_range_3_ae,as.character(100000+sample(1:.N))),
                  by=.(geoid,race,rent_own,family,hh_age_range_3_ae)]
bgHH_age_race[is.na(hh_type_t),c("hh_type_t","hh_type_fm_t","alone_t"):=
                 bgHH_type[.SD, c(list(hh_type),list(hh_type_fm),
                                     list(alone)), on = .(bgHH_ftare1_match_id)]]
bgHH_type[is.na(matched_type_race),("matched_type_race"):=
                 bgHH_age_race[.SD, list(race), on = .(bgHH_ftare1_match_id)]]
nrow(bgHH_age_race[!is.na(hh_type_t)]) / nrow(bgHH_type) #88%
#without family, since it may be wrong on the geoid b/c it comes from tract level
bgHH_age_race[is.na(hh_type_t),("bgHH_ftare2_match_id"):=
                    paste0(geoid,race,rent_own,hh_age_range_3,as.character(100000+sample(1:.N))),
                  by=.(geoid,race,rent_own,hh_age_range_3)]
bgHH_type[is.na(matched_type_race),("bgHH_ftare2_match_id"):=
                    paste0(geoid,race,rent_own,hh_age_range_3_ae,as.character(100000+sample(1:.N))),
                  by=.(geoid,race,rent_own,hh_age_range_3_ae)]
bgHH_age_race[is.na(hh_type_t),c("hh_type_t","hh_type_fm_t","alone_t"):=
                 bgHH_type[.SD, c(list(hh_type),list(hh_type_fm),
                                     list(alone)), on = .(bgHH_ftare2_match_id)]]
bgHH_type[is.na(matched_type_race),("matched_type_race"):=
                 bgHH_age_race[.SD, list(race), on = .(bgHH_ftare2_match_id)]]
nrow(bgHH_age_race[!is.na(hh_type_t)]) / nrow(bgHH_type) #90%
#without age_range_3
bgHH_age_race[is.na(hh_type_t),("bgHH_ftare3_match_id"):=
                    paste0(geoid,race,rent_own,as.character(100000+sample(1:.N))),
                  by=.(geoid,race,rent_own)]
bgHH_type[is.na(matched_type_race),("bgHH_ftare3_match_id"):=
                    paste0(geoid,race,rent_own,as.character(100000+sample(1:.N))),
                  by=.(geoid,race,rent_own)]
bgHH_age_race[is.na(hh_type_t),c("hh_type_t","hh_type_fm_t","alone_t"):=
                 bgHH_type[.SD, c(list(hh_type),list(hh_type_fm),
                                     list(alone)), on = .(bgHH_ftare3_match_id)]]
bgHH_type[is.na(matched_type_race),("matched_type_race"):=
                 bgHH_age_race[.SD, list(race), on = .(bgHH_ftare3_match_id)]]
nrow(bgHH_age_race[!is.na(hh_type_t)]) / nrow(bgHH_type) #95%
#and only race
bgHH_age_race[is.na(hh_type_t),("bgHH_ftare4_match_id"):=
                    paste0(geoid,race,as.character(100000+sample(1:.N))),
                  by=.(geoid,race)]
bgHH_type[is.na(matched_type_race),("bgHH_ftare4_match_id"):=
                    paste0(geoid,race,as.character(100000+sample(1:.N))),
                  by=.(geoid,race)]
bgHH_age_race[is.na(hh_type_t),c("hh_type_t","hh_type_fm_t","alone_t"):=
                 bgHH_type[.SD, c(list(hh_type),list(hh_type_fm),
                                     list(alone)), on = .(bgHH_ftare4_match_id)]]
bgHH_type[is.na(matched_type_race),("matched_type_race"):=
                 bgHH_age_race[.SD, list(race), on = .(bgHH_ftare4_match_id)]]
nrow(bgHH_age_race[!is.na(hh_type_t)]) == nrow(bgHH_type)
test <- table(
  bgHH_age_race[,geoid],
  bgHH_age_race[,race],
  bgHH_age_race[,hh_type_t],
  bgHH_age_race[,hh_type_fm_t],
  bgHH_age_race[,alone_t]
  )==
  table(bgHH_type[,geoid],
        bgHH_type[,race],
        bgHH_type[,hh_type],
        bgHH_type[,hh_type_fm],
        bgHH_type[,alone]
        )
length(test[test==FALSE]) == 0
```
...
```{r move age_race variables down to _dec}
#move age_race variables down to _dec (with HvL as one of the matches) 
#remember, latest dec is on "hh_age_range_3_ar","rent_own_ar","family_ar" (only eth) - not full matches
bgHH_age_race[,("bgHH_far_match_id"):=
                    paste0(geoid,HvL,race,family,rent_own,hh_type_t,
                           hh_age_range_3,as.character(100000+sample(1:.N))),
                  by=.(geoid,HvL,race,family,rent_own,hh_type_t,
                           hh_age_range_3)]
bgHH_dec[,("bgHH_far_match_id"):=
                    paste0(geoid,HvL,race,family_ar,rent_own_ar,hh_type,
                           hh_age_range_3_ar,as.character(100000+sample(1:.N))),
                  by=.(geoid,HvL,race,family_ar,rent_own_ar,hh_type,
                           hh_age_range_3_ar)]
bgHH_dec[,c("hh_size","hh_type_t","hh_type_fm_t","alone_t","rent_own_r",
            "hh_age_range","hh_age_range_3_r"):=
                 bgHH_age_race[.SD, c(list(hh_size),list(hh_type_t),list(hh_type_fm_t),
                                     list(alone_t),list(rent_own),
                                     list(hh_age_range),list(hh_age_range_3)), 
                               on = .(bgHH_far_match_id)]]
bgHH_age_race[,("matched_type_age_race"):=
                 bgHH_dec[.SD, list(race), on = .(bgHH_far_match_id)]]
nrow(bgHH_dec[!is.na(hh_size)]) / nrow(bgHH_dec[HvL==TRUE]) #79.5%
#get without family or type, since _dec is already ground truth on those
#change to hh_age_range_3 - ar has all been matched
bgHH_age_race[is.na(matched_type_age_race),("bgHH_far1_match_id"):=
                    paste0(geoid,HvL,race,rent_own,
                           hh_age_range_3,as.character(100000+sample(1:.N))),
                  by=.(geoid,HvL,race,rent_own,
                           hh_age_range_3)]
bgHH_dec[is.na(hh_size),("bgHH_far1_match_id"):=
                    paste0(geoid,HvL,race,rent_own_ar,
                           hh_age_range_3,as.character(100000+sample(1:.N))),
                  by=.(geoid,HvL,race,rent_own_ar,
                           hh_age_range_3)]
bgHH_dec[is.na(hh_size),c("hh_size","hh_type_t","hh_type_fm_t","alone_t","rent_own_r",
                          "hh_age_range","hh_age_range_3_r"):=
                 bgHH_age_race[.SD, c(list(hh_size),list(hh_type_t),list(hh_type_fm_t),
                                     list(alone_t),list(rent_own),
                                     list(hh_age_range),list(hh_age_range_3)), 
                               on = .(bgHH_far1_match_id)]]
bgHH_age_race[is.na(matched_type_age_race),("matched_type_age_race"):=
                 bgHH_dec[.SD, list(race), on = .(bgHH_far1_match_id)]]
nrow(bgHH_dec[!is.na(hh_size)]) / nrow(bgHH_dec[HvL==TRUE]) #90.2%
#get without age
bgHH_age_race[is.na(matched_type_age_race),("bgHH_far2_match_id"):=
                    paste0(geoid,HvL,race,rent_own,as.character(100000+sample(1:.N))),
                  by=.(geoid,HvL,race,rent_own)]
bgHH_dec[is.na(hh_size),("bgHH_far2_match_id"):=
                    paste0(geoid,HvL,race,rent_own_ar,as.character(100000+sample(1:.N))),
                  by=.(geoid,HvL,race,rent_own_ar)]
bgHH_dec[is.na(hh_size),c("hh_size","hh_type_t","hh_type_fm_t","alone_t","rent_own_r",
                          "hh_age_range","hh_age_range_3_r"):=
                 bgHH_age_race[.SD, c(list(hh_size),list(hh_type_t),list(hh_type_fm_t),
                                     list(alone_t),list(rent_own),
                                     list(hh_age_range),list(hh_age_range_3)), 
                               on = .(bgHH_far2_match_id)]]
bgHH_age_race[is.na(matched_type_age_race),("matched_type_age_race"):=
                 bgHH_dec[.SD, list(race), on = .(bgHH_far2_match_id)]]
nrow(bgHH_dec[!is.na(hh_size)]) == nrow(bgHH_dec[HvL==TRUE]) 

#match for HvL==FALSE on bgHH_age_race
bgHH_age_race[is.na(matched_type_age_race),("bgHH_farr_match_id"):=
                    paste0(geoid,race,rent_own,family,hh_type_t,
                           hh_age_range_3,as.character(100000+sample(1:.N))),
                  by=.(geoid,race,rent_own,family,hh_type_t,hh_age_range_3)]
bgHH_dec[is.na(hh_size),("bgHH_farr_match_id"):=
                    paste0(geoid,race,rent_own,family,hh_type,
                           hh_age_range_3,as.character(100000+sample(1:.N))),
                  by=.(geoid,race,rent_own,family,hh_type,hh_age_range_3)]
bgHH_dec[is.na(hh_size),c("hh_size","hh_type_t","hh_type_fm_t","alone_t","rent_own_r",
                          "hh_age_range","hh_age_range_3_r"):=
                 bgHH_age_race[.SD, c(list(hh_size),list(hh_type_t),list(hh_type_fm_t),
                                     list(alone_t),list(rent_own),
                                     list(hh_age_range),list(hh_age_range_3)), 
                               on = .(bgHH_farr_match_id)]]
bgHH_age_race[is.na(matched_type_age_race),("matched_type_age_race"):=
                 bgHH_dec[.SD, list(race), on = .(bgHH_farr_match_id)]]
nrow(bgHH_dec[!is.na(hh_size)]) / nrow(bgHH_dec) #87%
#without family and hh_type
bgHH_age_race[is.na(matched_type_age_race),("bgHH_farr1_match_id"):=
                    paste0(geoid,race,rent_own,
                           hh_age_range_3,as.character(100000+sample(1:.N))),
                  by=.(geoid,race,rent_own,hh_age_range_3)]
bgHH_dec[is.na(hh_size),("bgHH_farr1_match_id"):=
                    paste0(geoid,race,rent_own,
                           hh_age_range_3,as.character(100000+sample(1:.N))),
                  by=.(geoid,race,rent_own,hh_age_range_3)]
bgHH_dec[is.na(hh_size),c("hh_size","hh_type_t","hh_type_fm_t","alone_t","rent_own_r",
                 ...
                 bgHH_age_race[.SD, c(list(hh_size),list(hh_type_t),list(hh_type_fm_t),
                                     list(alone_t),list(rent_own),
                                     list(hh_age_range),list(hh_age_range_3)), 
                               on = .(bgHH_farr1_match_id)]]
bgHH_age_race[is.na(matched_type_age_race),("matched_type_age_race"):=
                 bgHH_dec[.SD, list(race), on = .(bgHH_farr1_match_id)]]
nrow(bgHH_dec[!is.na(hh_size)]) / nrow(bgHH_dec) #91.5%
#and without age
bgHH_age_race[is.na(matched_type_age_race),("bgHH_farr2_match_id"):=
                    paste0(geoid,race,rent_own,
                           as.character(100000+sample(1:.N))),
                  by=.(geoid,race,rent_own)]
bgHH_dec[is.na(hh_size),("bgHH_farr2_match_id"):=
                    paste0(geoid,race,rent_own,
                           as.character(100000+sample(1:.N))),
                  by=.(geoid,race,rent_own)]
bgHH_dec[is.na(hh_size),c("hh_size","hh_type_t","hh_type_fm_t","alone_t","rent_own_r",
                 ...
                 bgHH_age_race[.SD, c(list(hh_size),list(hh_type_t),list(hh_type_fm_t),
                                     list(alone_t),list(rent_own),
                                     list(hh_age_range),list(hh_age_range_3)), 
                               on = .(bgHH_farr2_match_id)]]
bgHH_age_race[is.na(matched_type_age_race),("matched_type_age_race"):=
                 bgHH_dec[.SD, list(race), on = .(bgHH_farr2_match_id)]]
nrow(bgHH_dec[!is.na(hh_size)]) / nrow(bgHH_dec) #95.3%
#and with only race
#and without age
bgHH_age_race[is.na(matched_type_age_race),("bgHH_farr3_match_id"):=
                    paste0(geoid,race,
                           as.character(100000+sample(1:.N))),
                  by=.(geoid,race)]
bgHH_dec[is.na(hh_size),("bgHH_farr3_match_id"):=
                    paste0(geoid,race,
                           as.character(100000+sample(1:.N))),
                  by=.(geoid,race)]
bgHH_dec[is.na(hh_size),c("hh_size","hh_type_t","hh_type_fm_t","alone_t","rent_own_r",
                 ...
                 bgHH_age_race[.SD, c(list(hh_size),list(hh_type_t),list(hh_type_fm_t),
                                     list(alone_t),list(rent_own),
                                     list(hh_age_range),list(hh_age_range_3)), 
                               on = .(bgHH_farr3_match_id)]]
bgHH_age_race[is.na(matched_type_age_race),("matched_type_age_race"):=
                 bgHH_dec[.SD, list(race), on = .(bgHH_farr3_match_id)]]
nrow(bgHH_dec[!is.na(hh_size)]) == nrow(bgHH_dec)

#look at tables for hh_age_range_3_r and rent_own_r, too
#need to go through originals - race/eth/age/type (type_fm and alone)/rent_own/family/size
test <- table(
  bgHH_age_race[,geoid],
  bgHH_age_race[,race],
  bgHH_age_race[,hh_type_t],
...
length(test[test>5]) / length(test) #.020% - really, should be about age_range_3 matching...
#go through the older relationships and see what's broken
test <- table(
  ...
  table(bgHH_dec[,geoid],
        bgHH_dec[,race],
        bgHH_dec[,hh_size],
        bgHH_dec[,rent_own_r] #compare with rent_own - which is way off!
        )
length(test[test==FALSE])
test <- table(
  #bgHH_size_eth[,geoid],
  #bgHH_size_eth[,hh_size],
  bgHH_size_eth[,family]
  )==
  table(#bgHH_dec[HvL==TRUE,geoid],
        #bgHH_dec[HvL==TRUE,hh_size],
        bgHH_dec[HvL==TRUE,family_ar] #UGH!!!!!!
        )
length(test[test==FALSE])
...
#PCT12 and 13 give sex by age by race/eth in hh and total, so could deduce group quarters but at tract level; could get something, but seems small advances
...
#group quarters codes: https://www2.census.gov/programs-surveys/decennial/2020/technical-documentation/complete-tech-docs/demographic-and-housing-characteristics-file-and-demographic-profile/2020census-demographic-and-housing-characteristics-file-and-demographic-profile-techdoc.pdf
#https://www.census.gov/programs-surveys/decennial-census/decade/2020/planning-management/count/service-based-enumeration.html
#https://www.census.gov/programs-surveys/decennial-census/decade/2020/planning-management/plan/memo-series/2020-memo-2021_08.html
...
bg_hh_own_children_type_data_from_census <- 
  censusData_byGroupName(censusdir, vintage, state, censuskey, 
                         ...
                         block="block_group",api_type="dec/dhc",path_suff="est.csv")
#5b
#only HOUSEHOLDS BY PRESENCE OF PEOPLE UNDER 18 YEARS, and whether in families or not...
...
#
bg_hh_type_relation_data_from_census <-  #seems to have all the categories of PCT17, but not race/ethnicity; type is just inHH or gq
  censusData_byGroupName(censusdir, vintage, state, censuskey, 
                         groupname = "P17",county_num = "*",
                         block="block_group",api_type="dec/dhc",path_suff="est.csv")

tr_hh_type_relation_race_data_from_census <-  #seems to have all the categories of P17, but also has race/ethnicity...
  censusData_byGroupName(censusdir, vintage, state, censuskey, 
