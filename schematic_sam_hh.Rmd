...
bgHH_dec[,c("hh_size","hh_age_range_3","hh_age_range_9","rent_own","hh_type_fm","alone","sex"):=
                 bgHH_type[.SD, c(list(hh_size),list(hh_age_range_3),list(hh_age_range_9),
                                  list(hh_type_fm),list(alone),list(sex)), 
                           on = .(bgHH_dt_match_id)]]
nrow(bgHH_dec[is.na(hh_size)])
```
...
#by ending on bgHH_type, it uses its bg representation of family and rent as ground. Move rest over to have complete on race
```
