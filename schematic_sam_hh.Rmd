...
#                                59919                                293337 
#> table(bgHH_age_eth[HvL==TRUE,hh_type])
#
#Female householder, no spouse present              Householder living alone          Householder not living alone 
#                               109516                                130294                                 36767 
#  Male householder, no spouse present                 Married couple family 
#                                52832                                264273 
#could show how that's different with an idealized geoid, with equal parts for every part of the cartesian product
...
#grow age_eth by switching family / non-family (if you don't flip, it just matches on duplicate geoids)
#2x (for family) got it down to 2161 missing; 3x to 767; 4x to 503 - let's try a different strategy, with only 2x on this
#bgHH_age_eth1a <- bgHH_age_eth[rep(!is.na(geoid),2,length=.N),]
#bgHH_age_eth1a[,("family"):=ifelse(family=="Nonfamily households","Family households","Nonfamily households")]
#bgHH_age_eth1 <- rbindlist(list(bgHH_age_eth,bgHH_age_eth1a))  
...
nrow(bgHH_age_eth[is.na(race)]) #53,230 i.e., just over 10%
...
#NOW DO THE RACE BY THE SAME STEPS
...
bgHH_age_eth[,("bgHH_aert_match_id"):=
                    paste0(geoid,hh_age_range_3,rent_own,family,hh_type,as.character(100000+sample(1:.N))),
                  by=.(geoid,hh_age_range_3,rent_own,family,hh_type)]
bgHH_type[,("bgHH_aert_match_id"):=
                    paste0(geoid,hh_age_range_3,rent_own,family,hh_type,as.character(100000+sample(1:.N))),
                  by=.(geoid,hh_age_range_3,rent_own,family,hh_type)]
bgHH_type[,c("HvL","race"):=
                 bgHH_age_eth[.SD, c(list(HvL),list(race)), on = .(bgHH_aert_match_id)]]
nrow(bgHH_age_eth)-nrow(bgHH_type[!is.na(race)]) #1985. 
#test
...
bgHH_type[HvL==FALSE,("bgHH_rt_match_id"):=
                    paste0(geoid,hh_age_range_3,rent_own,as.character(100000+sample(1:.N))),
                  by=.(geoid,hh_age_range_3,rent_own)]
bgHH_age_race[is.na(family),c("hh_type","family"):=
                 bgHH_type[.SD, c(list(hh_type),list(family)), on = .(bgHH_rt_match_id)]]
nrow(bgHH_age_race[!is.na(family)]) #1512045, or 180000 unmatched!!!
