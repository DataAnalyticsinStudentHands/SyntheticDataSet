...
hh_eth_size_reduced <- trHH_size_eth_maxblocks[!is.na(bg_size_match)]
cart_size_eth_rent <- hh_eth_size_reduced[rep(1:.N,1)][,c("rent_own","indx"):=
                                                   c(list("Renter occupied"),list(1:.N)),
                                                 by=.(geoid,hh_size)]
cart_size_eth_own <- hh_eth_size_reduced[rep(1:.N,1)][,c("rent_own","indx"):=
...
cart_size_eth_fnf[!is.na(size_match),("bgHH_sizeHvL_match0_id"):=
...
trHH_size_eth[,("bgHH_sizeHvL_match0_id"):=
                    paste0(tract,HvL,family,hh_size,as.character(100000+sample(1:.N))),
                  by=.(tract,HvL,family,hh_size)]
cart_size_eth_fnf[!is.na(size_match),("size_match0"):=
                 trHH_size_eth[.SD, list(hh_size), on = .(bgHH_sizeHvL_match0_id)]]
trHH_size_eth[,c("size_match0","geoid","rent_own"):=
                 cart_size_eth_fnf[.SD, c(list(hh_size),list(geoid),list(rent_own)), on = .(bgHH_sizeHvL_match0_id)]]
nrow(trHH_size_eth[is.na(size_match0)])
...
bgHH_size_eth[,("bgHH_sizeHvL_matchf_id"):=
                    paste0(geoid,HvL,rent_own,family,hh_size,as.character(100000+sample(1:.N))),
                  by=.(geoid,HvL,rent_own,family,hh_size)]
trHH_size_eth[,("bgHH_sizeHvL_matchf_id"):=
                    paste0(geoid,HvL,rent_own,family,hh_size,as.character(100000+sample(1:.N))),
                  by=.(geoid,HvL,rent_own,family,hh_size)]
bgHH_size_eth[,("size_matchf"):=
                 trHH_size_eth[.SD, list(hh_size), on = .(bgHH_sizeHvL_matchf_id)]]
trHH_size_eth[,c("size_matchf"):=
                 bgHH_size_eth[.SD, list(hh_size), on = .(bgHH_sizeHvL_matchf_id)]]
nrow(trHH_size_eth[is.na(size_matchf)]) #43649 -i.e., almost 93% match...
#then by tract
bgHH_size_eth[is.na(size_matchf),("bgHH_sizeHvL_matchf1_id"):=
                    paste0(tract,HvL,rent_own,family,hh_size,as.character(100000+sample(1:.N))),
                  by=.(tract,HvL,rent_own,family,hh_size)]
trHH_size_eth[is.na(size_matchf),("bgHH_sizeHvL_matchf1_id"):=
                    paste0(tract,HvL,rent_own,family,hh_size,as.character(100000+sample(1:.N))),
                  by=.(tract,HvL,rent_own,family,hh_size)]
bgHH_size_eth[is.na(size_matchf),("size_matchf"):=
                 trHH_size_eth[.SD, list(hh_size), on = .(bgHH_sizeHvL_matchf1_id)]]
trHH_size_eth[is.na(size_matchf),c("size_matchf"):=
                 bgHH_size_eth[.SD, list(hh_size), on = .(bgHH_sizeHvL_matchf1_id)]]
nrow(trHH_size_eth[is.na(size_matchf)]) #42677 - not much added... less than 1% improvement
...
#create possible matches by cartesian products
#MAYBE EXPAND BY ALL racES!!! Just not sure!!!
...
cart_trHH_size_race <- cart_race_bgHH_size[rep(1:.N,maxblocks)][,("indx"):=
                                                 ...
                                                 by=.(geoid,hh_size)] 
#should be faster way to get a unique value on each; not sure
trHH_size_race[,("id4rep"):=paste0(tract,hh_size,family,tr_hh_size_race_id)]
#put maxblocks from bg to tr
...
nrow(cart_trHH_size_race[!is.na(bg_size_match)])
#see if we have right number, although we don't know which of the duplicated id4reps is the right match...
length(unique(trHH_size_race_maxblocks[!is.na(bg_size_match),id4rep])) #893657
...
nrow(cart_size_race_fnf[!is.na(size_match)])
nrow(cart_size_race_or[!duplicated(id4rep)]). #893657
#try going to trHH_size_eth with geoid and rent_own
cart_size_race_fnf[!is.na(size_match),("bgHH_sizerace_match0_id"):=
...
trHH_size_race[,("bgHH_sizerace_match0_id"):=
                    paste0(tract,race,family,hh_size,as.character(100000+sample(1:.N))),
                  by=.(tract,race,family,hh_size)]
cart_size_race_fnf[!is.na(size_match),("size_match0"):=
                 trHH_size_race[.SD, list(hh_size), on = .(bgHH_sizerace_match0_id)]]
trHH_size_race[,c("size_match0","geoid","rent_own"):=
                 cart_size_race_fnf[.SD, c(list(hh_size),list(geoid),list(rent_own)), on = .(bgHH_sizerace_match0_id)]]
nrow(trHH_size_race[is.na(size_match0)]) / nrow(trHH_size_race)
...
trHH_size_race[,("bgHH_sizerace_matchf_id"):=
                    paste0(geoid,race,rent_own,family,hh_size,as.character(100000+sample(1:.N))),
                  by=.(geoid,race,rent_own,family,hh_size)]
bgHH_size_race[,("size_matchf"):=
                 trHH_size_race[.SD, list(hh_size), on = .(bgHH_sizerace_matchf_id)]]
trHH_size_race[,c("size_matchf"):=
                 bgHH_size_race[.SD, list(hh_size), on = .(bgHH_sizerace_matchf_id)]]
nrow(trHH_size_race[is.na(size_matchf)]) #43649 -i.e., almost 93% match...
#then by tract
bgHH_size_race[is.na(size_matchf),("bgHH_sizerace_matchf1_id"):=
                    paste0(tract,race,rent_own,family,hh_size,as.character(100000+sample(1:.N))),
                  by=.(tract,race,rent_own,family,hh_size)]
trHH_size_race[is.na(size_matchf),("bgHH_sizerace_matchf1_id"):=
                    paste0(tract,race,rent_own,family,hh_size,as.character(100000+sample(1:.N))),
                  by=.(tract,race,rent_own,family,hh_size)]
bgHH_size_race[is.na(size_matchf),("size_matchf"):=
                 trHH_size_race[.SD, list(hh_size), on = .(bgHH_sizerace_matchf1_id)]]
trHH_size_race[is.na(size_matchf),c("size_matchf"):=
                 bgHH_size_race[.SD, list(hh_size), on = .(bgHH_sizerace_matchf1_id)]]
nrow(trHH_size_race[is.na(size_matchf)]) #42677 - not much added... less than 1% improvement
...
trHH_size_eth[is.na(size_matchf),c("size_matchf"):=
                 bgHH_size_eth[.SD, list(hh_size), on = .(bgHH_sizerace_matchf2_id)]]
nrow(trHH_size_race[is.na(size_matchf)]) == 0 
...
```{r matching with age}
#and don't forget that we have some things that will help match by age for hh_size (like seniors living with...)
#can we avoid tract level
#and at tract level
trHH_size_eth[is.na(geoid_dec),("trbgHH_decHvL2_match_id"):=
                    paste0(tract,HvL,family,as.character(100000+sample(1:.N))),
                  by=.(tract,HvL,family)]
bgHH_dec[is.na(trbg_dec_match),("trbgHH_decHvL2_match_id"):=
                    paste0(tract,HvL,family,as.character(100000+sample(1:.N))),
                  by=.(tract,HvL,family)]
bgHH_dec[is.na(trbg_dec_match),("trbg_dec_match"):=
                 trHH_size_eth[.SD, list(hh_size), on = .(trbgHH_decHvL2_match_id)]]
trHH_size_eth[is.na(geoid_dec),("geoid_dec"):=
                 bgHH_dec[.SD, list(geoid), on = .(trbgHH_decHvL2_match_id)]]
nrow(trHH_size_eth[is.na(geoid_dec)])

test <- table(trHH_size_eth[,tract],
              trHH_size_eth[,geoid_dec],
              trHH_size_eth[,HvL]
              )==
        table(bgHH_size_eth[,tract],
              bgHH_size_eth[,geoid],
...
length(test[test==FALSE])==0
#now back down to bgHH_size_eth
trHH_size_eth[,("trbgHH_size_match_id"):=
                    paste0(geoid_dec,HvL,rent_own,hh_size,as.character(100000+sample(1:.N))),
                  by=.(geoid_dec,HvL,rent_own,hh_size)]
bgHH_size_eth[,("trbgHH_size_match_id"):=
                    paste0(geoid,HvL,rent_own,hh_size,as.character(100000+sample(1:.N))),
                  by=.(geoid,HvL,rent_own,hh_size)]
bgHH_size_eth[,("family"):=
                 trHH_size_eth[.SD, list(family), on = .(trbgHH_size_match_id)]]
trHH_size_eth[,("matched_bg"):=
                 bgHH_size_eth[.SD, list(geoid), on = .(trbgHH_size_match_id)]]
nrow(bgHH_size_eth[is.na(family)]) #32,387
#and again, but using rent_own from bg as ground
trHH_size_eth[is.na(matched_bg),("trbgHH_size1_match_id"):=
                    paste0(geoid_dec,HvL,hh_size,as.character(100000+sample(1:.N))),
                  by=.(geoid_dec,HvL,hh_size)]
bgHH_size_eth[is.na(family),("trbgHH_size1_match_id"):=
                    paste0(geoid,HvL,hh_size,as.character(100000+sample(1:.N))),
                  by=.(geoid,HvL,hh_size)]
bgHH_size_eth[is.na(family),("family"):=
                 trHH_size_eth[.SD, list(family), on = .(trbgHH_size1_match_id)]]
trHH_size_eth[is.na(matched_bg),("matched_bg"):=
                 bgHH_size_eth[.SD, list(geoid), on = .(trbgHH_size1_match_id)]]
nrow(bgHH_size_eth[is.na(family)]) #2124
#and then on tract
trHH_size_eth[is.na(matched_bg),("trbgHH_size2_match_id"):=
                    paste0(tract,HvL,hh_size,as.character(100000+sample(1:.N))),
                  by=.(tract,HvL,hh_size)]
bgHH_size_eth[is.na(family),("trbgHH_size2_match_id"):=
                    paste0(tract,HvL,hh_size,as.character(100000+sample(1:.N))),
                  by=.(tract,HvL,hh_size)]
bgHH_size_eth[is.na(family),("family"):=
                 trHH_size_eth[.SD, list(family), on = .(trbgHH_size2_match_id)]]
trHH_size_eth[is.na(matched_bg),("matched_bg"):=
                 bgHH_size_eth[.SD, list(geoid), on = .(trbgHH_size2_match_id)]]
nrow(bgHH_size_eth[is.na(family)]) == 0
test <- table(bgHH_dec[HvL==TRUE,tract], #geoid gives 4488 FALSE
              bgHH_dec[HvL==TRUE,family],
              bgHH_dec[HvL==TRUE,HvL]
              )==
        table(bgHH_size_eth[,tract],
...
cart_HvL_bgHH_size_A <- cart_HvL_bgHH_size[rep(1:.N,1)][,c("race","raceindx"):=
                                                   c(list("A"),list(1:.N)),
                                                 by=.(geoid,hh_size)]
cart_HvL_bgHH_size_B <- cart_HvL_bgHH_size[rep(1:.N,1)][,c("race","raceindx"):=
                                                   c(list("B"),list(1:.N)),
                                                 by=.(geoid,hh_size)]
cart_HvL_bgHH_size_C <- cart_HvL_bgHH_size[rep(1:.N,1)][,c("race","raceindx"):=
                                                   c(list("C"),list(1:.N)),
                                                 by=.(geoid,hh_size)]
cart_HvL_bgHH_size_D <- cart_HvL_bgHH_size[rep(1:.N,1)][,c("race","raceindx"):=
                                                   c(list("D"),list(1:.N)),
                                                 by=.(geoid,hh_size)]
cart_HvL_bgHH_size_E <- cart_HvL_bgHH_size[rep(1:.N,1)][,c("race","raceindx"):=
                                                   c(list("E"),list(1:.N)),
                                                 by=.(geoid,hh_size)]
cart_HvL_bgHH_size_F <- cart_HvL_bgHH_size[rep(1:.N,1)][,c("race","raceindx"):=
                                                   c(list("F"),list(1:.N)),
                                                 by=.(geoid,hh_size)]
cart_HvL_bgHH_size_G <- cart_HvL_bgHH_size[rep(1:.N,1)][,c("race","raceindx"):=
                                                   c(list("G"),list(1:.N)),
                                                 by=.(geoid,hh_size)]
cart_HvL_bgHH_size_race <- rbindlist(list(cart_HvL_bgHH_size_A,cart_HvL_bgHH_size_B,
                                          cart_HvL_bgHH_size_C,cart_HvL_bgHH_size_D,
                                          cart_HvL_bgHH_size_E,cart_HvL_bgHH_size_F,
                                          cart_HvL_bgHH_size_G))
#do trHH x max number of geoids in a tract, since we don't know which it falls into - have to make sure both sides are large enough




#match cart_trHH with cart_bgHH? so you still have the extra matches - but should be back down somewhat in size
bgHH_dec[,("trbgHH_ds_match_id"):=
                    paste0(tract,HvL,family,as.character(100000+sample(1:.N))),
                  by=.(tract,HvL,family)]
cart_trHH_size_eth[,("trbgHH_ds_match_id"):=
                    paste0(tract,HvL,family,as.character(100000+sample(1:.N))),
                  by=.(tract,HvL,family)]
...
#match this with _dec on race x HvL x family x rent_own - should be back to right size - and can put size right on _dec??
#how to test that some of the ones weren't accidentally wrong matches??
```



and then number the geoid + the same .N in the paste,
eth family from dec is also a limit - it reduces to that limit...
as it counts down from the .N for that match...


```{r old attempts to join for comparison}
...
ABOVE, WHERE HH_SIZE_AR IS MADE, NEED TO MAKE SURE DIDN'T ACCIDENTALLY MOVE AWAY FROM HH_TYPE !!
