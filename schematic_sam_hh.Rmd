...
trHH_size_race_maxblocks[,("rowsizenumber"):=paste0(tract,hh_size,1:.N),by=.(tract,hh_size)]
bgHH_size_race_maxblocks[,("rowsizenumber"):=paste0(tract,hh_size,1:.N),by=.(tract,hh_size)]
bgHH_size_race_geoids <- bgHH_size_eth_maxblocks[,c("tract","rowsizenumber","geoid")]

trHH_size_race_max <- bgHH_size_race_geoids[trHH_size_race_maxblocks,on=.(tract,rowsizenumber)]
#test
length(unique(trHH_size_race_max[,geoid]))==length(unique(bgHH_size_race[,geoid])) #missing one geoid???
...
nrow(cart_trHH_size_race[!is.na(bg_size_match)]) #671729
#see if we have right number, although we don't know which of the duplicated id4reps is the right match... - we lost some here!!!
length(unique(trHH_size_race_max[!is.na(bg_size_match),id4rep]))#409064  == nrow(trHH_size_race)
...
nrow(cart_size_race_fnf[!is.na(size_match)]) #750305
nrow(cart_size_race_or[!duplicated(id4rep)]) #409064 == nrow(bgHH_size_race)
#try going to trHH_size_eth with geoid and rent_own
