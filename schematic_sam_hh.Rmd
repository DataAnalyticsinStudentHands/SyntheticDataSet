...
#match_eth_size <- trHH_size_eth_maxblocks[!is.na(bg_size_match)&!duplicated(id4rep)] #519420 - 
#reduce then add rent_own
match_eth_size <- trHH_size_eth_maxblocks[!is.na(bg_size_match)]
cart_size_eth_rent <- match_eth_size[rep(1:.N,1)][,c("rent_own","indx"):=
                                                   c(list("Renter occupied"),list(1:.N)),
                                                 by=.(geoid,hh_size)]
cart_size_eth_own <- match_eth_size[rep(1:.N,1)][,c("rent_own","indx"):=
...
#reduce then add rent_own
hh_race_size_reduced <- trHH_size_race_maxblocks[!is.na(bg_size_match)]
cart_size_race_rent <- hh_race_size_reduced[rep(1:.N,1)][,c("rent_own","indx"):=
                                                   c(list("Renter occupied"),list(1:.N)),
                                                 by=.(geoid,race,hh_size)]
cart_size_race_own <- hh_race_size_reduced[rep(1:.N,1)][,c("rent_own","indx"):=
                                                   c(list("Owner occupied"),list(1:.N)),
                                                 by=.(geoid,race,hh_size)]
...
NEED TO ANSWER: should we be doing the cartesian trick on any of these??? Diagrams!!!!
...
bgHH_size_race[,c("hh_type","matched_size"):=
                 bgHH_type[.SD, c(list(hh_type),list(family)), on = .(bgHH_st_match_id)]]
nrow(bgHH_type[is.na(hh_size)])
#UGH - really need to think about what we're doing wrong...
#> nrow(bgHH_type[is.na(hh_size)])
#[1] 1408634
#> nrow(bgHH_type[!is.na(hh_size)])
#[1] 284096
