...
```{r pull in for tract size and family data}
...
```{r joining tract and block group size after expand for simple cartesian example}
...
cart_trHH_size_eth <- cart_HvL_bgHH_size[rep(1:.N,maxblocks)][,("indx"):=
                                                   list(1:.N),
                                                 by=.(geoid,hh_size)]
#should be faster way to get value on each; not sure
trHH_size_eth[,("id4rep"):=paste0(tract,tr_hh_age_eth_id,hh_size,family)]
...
bgHH_size_eth[,("bgHH_sizeHvL_match_id"):=
                    paste0(geoid,HvL,rent_own,hh_size,as.character(100000+sample(1:.N))),
                  by=.(geoid,HvL,rent_own,hh_size)]
cart_size_eth_or[,("size_match"):=
                 bgHH_size_eth[.SD, list(hh_size), on = .(bgHH_sizeHvL_match_id)]]
bgHH_size_eth[,c("size_match","family"):=
                 cart_size_eth_or[.SD, c(list(hh_size),list(family)), on = .(bgHH_sizeHvL_match_id)]]
nrow(bgHH_size_eth[is.na(size_match)])
#test
...
length(test[test==FALSE])==0 
#NEED TO DRAW!!! NOT SURE WHAT I"M DOING WRONG
