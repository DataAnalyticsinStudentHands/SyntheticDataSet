...
length(test[test>0])#450 i.e., around 57 geoids at risk of being misassigned - almost 90% tract has only one geoid with counts...
...
bgGQ[,("age"):=fcase(beg_age_gq==0 & gq_type=="College/University student housing (501)",as.integer(17),
                     ...
                     beg_age_gq!=0 & gq_type=="Nursing facilities/Skilled-nursing facilities (301)",
                        sample(51:105,.N,replace = TRUE,prob = c(rep(.002,15),rep(.02,30),rep(.0001,10))),
                     beg_age_gq==0 & str_detect(gq_type,"Other"),
                        sample(12:17,.N,replace = TRUE,prob = c(.01,.03,.06,.2,.3,.4)),
                     beg_age_gq!=0 & str_detect(gq_type,"Other"),
                        sample(18:105,.N,replace = TRUE,prob = c(rep(.025,48),rep(.02,30),rep(.0001,10))),
...
nrow(bgSARE[!is.na(gq_type)])-nrow(bgGQ) #-198
table(bgGQ[is.na(matched),gq_type])
table(bgGQ[is.na(matched),gq_type],bgGQ[is.na(matched),geoid]) #everything that doesn't match was in 3 geoids... could check...
#stop??
#try on tract level... only got 30 more...
#bgGQ[is.na(matched),("bgGQ_match3_id"):=
#                    paste0(tract,age4match,as.character(100000+sample(1:.N))),
#                  by=.(tract,age4match)]
#bgSARE[is.na(gq_type),("bgGQ_match3_id"):=
#                    paste0(tract,age4match,as.character(100000+sample(1:.N))),
#                  by=.(tract,age4match)]
#bgSARE[is.na(gq_type),c("institutionalized","gq_type"):=
#                 bgGQ[.SD, list(geoid), on = .(bgGQ_match3_id)]]
#bgGQ[is.na(matched),("matched"):=
#                 bgSARE[.SD, list(re_code), on = .(bgGQ_match3_id)]]
#nrow(bgSARE[!is.na(gq_type)])-nrow(bgGQ) #-754 (98.4% match)
...
PLAN - pull in bg population - assign group quarters to folks (with some idea they may switch toward end) - pull in hh_dec and assign the bg population to _dec. 
