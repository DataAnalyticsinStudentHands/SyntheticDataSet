...
if(names(bg_gq_age_data_from_census)[6]=="label_1"){
  ...
  label_c1 <- c("sex","age","gq_institution","gq_type")
  #row_c1 determined by hand
  row_c1 <- c(unique(bg_gq_age_data_from_census[!is.na(label_4),name]))
  pop_level <- "group_quarters"
  test_result <- tests_download_data(bg_gq_age_data_from_census,label_c1,row_c1,pop_level)
  bgGQ <- relabel(bg_gq_age_data_from_census[!is.na(label)],label_c1,row_c1)
  write_relabel(bgGQ,censusdir,vintage,state,censuskey,geo_type,groupname,county_num,api_type,path_suff)
}else{
  bgGQ <- bg_gq_age_data_from_census
}
