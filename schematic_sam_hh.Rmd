...
bgGQ[,("age"):=fcase(beg_age_gq==0 & gq_type=="College/University student housing (501)",as.integer(17),
                     ...
                     beg_age_gq==0 & str_detect(gq_type,"institutional"),
                        sample(12:17,.N,replace = TRUE,prob = c(.01,.03,.06,.2,.3,.4)),
                     beg_age_gq!=0 & str_detect(gq_type,"institutional"),
...
nrow(bgSARE[!is.na(gq_type)]) #41322
#on theory that HvL match is problematic and close to same overall...
...
nrow(bgSARE[!is.na(gq_type)])#47997
bgGQ[is.na(matched),("bgGQ_match2_id"):=
                    paste0(tract,sex,age4match,race,as.character(100000+sample(1:.N))),
                  by=.(tract,sex,age4match,race)]
bgSARE[is.na(gq_type),("bgGQ_match2_id"):=
                    paste0(tract,sex,age4match,re_code,as.character(100000+sample(1:.N))),
                  by=.(tract,sex,age4match,re_code)]
bgSARE[is.na(gq_type),c("institutionalized","gq_type"):=
                 bgGQ[.SD, list(geoid), on = .(bgGQ_match2_id)]]
bgGQ[is.na(matched),("matched"):=
                 bgSARE[.SD, list(re_code), on = .(bgGQ_match2_id)]]
nrow(bgSARE[!is.na(gq_type)])#47419
#no match on race or sex...
...
nrow(bgSARE[!is.na(gq_type)])-nrow(bgGQ) #807
bgSARE[,("household"):=ifelse(is.na(institutionalized),TRUE,FALSE)]
bgSARE[,c("bgGQ_match1_id","bgGQ_match2_id","bgGQ_match3_id"):=NULL]
```
