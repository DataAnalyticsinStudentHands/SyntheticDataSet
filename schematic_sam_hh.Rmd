...
```{r joining tract and block group hh_size after expand for simple cartesian example}
...
#do without cartesian expansion for rent_own
#get just the uniques that also have bg_size_match
#can use this to show that just reducing to unique doesn't get right subset
#match_eth_size <- trHH_size_eth_maxblocks[!is.na(bg_size_match)&!duplicated(id4rep)]
#reduce to then add rent_own
...
#then reduce to dedup on id4rep - but how get right ones...
bgHH_size_eth_test <- cart_size_eth_or[!is.na(size_match)]
bgHH_size_eth_test <- bgHH_size_eth_test[!duplicated(id4rep)]
#ONLY 443669!!!! perhaps the hh_size by geoid variation????
#test
test <- table(bgHH_size_eth_test[,tract],
              #bgHH_size_eth_test[,geoid],
              #bgHH_size_eth_test[,rent_own],
              bgHH_size_eth_test[,family],
              bgHH_size_eth_test[,hh_size],
              bgHH_size_eth_test[,HvL]
              )-
        table(trHH_size_eth[,tract],
              #trHH_size_eth[,geoid],
              #trHH_size_eth[,rent_own],
              trHH_size_eth[,family],
              trHH_size_eth[,hh_size],
              trHH_size_eth[,HvL]
              )
length(test[test<0])==0 
#so all correct matches are in bgHH_size_eth_test - how to get them out???
#should we use _dec??? family x HvL x geoid - no, because correct family is in the id4rep
#use bgHH_age_eth, since it has correct rent_own (but not moving anything yet!)
#doesn't work!!! Do I have to expand it by family / non-family??
...
###UGHH
bgHH_size_eth_test[,("bgHH_sizeHvL_match_id"):=
                    paste0(geoid,HvL,rent_own,as.character(100000+sample(1:.N))),
                  by=.(geoid,HvL,rent_own)]
bgHH_age_eth[,("bgHH_sizeHvL_match_id"):=
                    paste0(geoid,HvL,rent_own,as.character(100000+sample(1:.N))),
                  by=.(geoid,HvL,rent_own)]
bgHH_size_eth_test[,("age_match"):=
                 bgHH_age_eth[.SD, list(rent_own), on = .(bgHH_sizeHvL_match_id)]]
bgHH_age_eth[,("age_match"):=
                 bgHH_size_eth_test[.SD, list(rent_own), on = .(bgHH_sizeHvL_match_id)]]
nrow(bgHH_age_eth[is.na(age_match)])==0
nrow(bgHH_size_eth_test[!is.na(age_match)])
bgHH_size_eth_test1 <- bgHH_size_eth_test[!is.na(age_match)]
test <- table(bgHH_size_eth_test1[,tract],
              #bgHH_size_eth_test1[,geoid],
              #bgHH_size_eth_test1[,rent_own],
              bgHH_size_eth_test1[,family],
              #bgHH_size_eth_test1[,hh_size],
              bgHH_size_eth_test1[,HvL]
              )-
        table(trHH_size_eth[,tract],
              #trHH_size_eth[,geoid],
              #trHH_size_eth[,rent_own],
...
#constrain with trHH_size_eth 
cart_trHH_size_eth[!is.na(bg_size_match),("trbgHH_sizeHvL1_match_id"):=
                    paste0(tract,family,hh_size,as.character(100000+sample(1:.N))),
                  by=.(tract,family,hh_size)]
trHH_size_eth[,("trbgHH_sizeHvL1_match_id"):=
                    paste0(tract,family,hh_size,as.character(100000+sample(1:.N))),
                  by=.(tract,family,hh_size)]
cart_trHH_size_eth[!is.na(bg_size_match),("bg_size_match1"):=
                 trHH_size_eth[.SD, list(hh_size), on = .(trbgHH_sizeHvL1_match_id)]]
trHH_size_eth[,c("geoid","rent_own"):=
                 cart_trHH_size_eth[.SD, c(list(geoid),list(rent_own)), on = .(trbgHH_sizeHvL1_match_id)]]
nrow(trHH_size_eth[is.na(geoid)])
#test <- (table(bgHH_size_eth[,geoid])-table(trHH_size_eth[,geoid]))/table(trHH_size_eth[,geoid])
#second time through, had different total number of geoids, so couldn't run
#> length(test[test<.00001])/length(test)
#[1] 1526
#> length(test)
#[1] 2824
#can we correct from _dec now? #can we avoid tract level
#if we do the cart expansion on geoids and rent_own first 
trHH_size_eth[,("trbgHH_decHvL1_match_id"):=
                    paste0(geoid,HvL,family,as.character(100000+sample(1:.N))),
                  by=.(geoid,HvL,family)]
bgHH_dec[,("trbgHH_decHvL1_match_id"):=
                    paste0(geoid,HvL,family,as.character(100000+sample(1:.N))),
                  by=.(geoid,HvL,family)]
bgHH_dec[,("trbg_dec_match"):=
                 trHH_size_eth[.SD, list(hh_size), on = .(trbgHH_decHvL1_match_id)]]
trHH_size_eth[,("geoid_dec"):=
                 bgHH_dec[.SD, list(geoid), on = .(trbgHH_decHvL1_match_id)]]
nrow(trHH_size_eth[is.na(geoid_dec)]) #14360
