...
#this approach makes too many people at 105!!!! - maybe get some probabilities from bgSARE distribution??
#could fix it by matching on order when moving over, instead of on age exactly.... not sure...
...
bgGQ[,("age"):=fcase(beg_age_gq==0 & gq_type=="College/University student housing (501)",as.integer(17),
                     ...
                     beg_age_gq!=0 & gq_type=="Nursing facilities/Skilled-nursing facilities (301)",
                        sample(51:105,.N,replace = TRUE,prob = c(rep(.002,15),rep(.02,30),rep(.005,10))),
                     beg_age_gq==0 & str_detect(gq_type,"institutional"),
                        sample(12:17,.N,replace = TRUE,prob = c(.01,.03,.06,.2,.3,.4)),
                     beg_age_gq!=0 & str_detect(gq_type,"institutional"),
                        sample(18:105,.N,replace = TRUE,prob = c(rep(.025,48),rep(.02,30),rep(.005,10))),
