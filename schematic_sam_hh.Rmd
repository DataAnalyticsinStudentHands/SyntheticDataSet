...
#add family to bgHH_size - see how much just the variation in race/eth keeps the family in the right block...
...
nrow(bgHH_type[alone=="Living alone"])-nrow(bgHH_age_eth[!is.na(hh_type)]) #for race, it should be close, here 94,502 matched
...
bgHH_age_eth[is.na(hh_type),c("hh_type","hh_type_fm","alone","family","geoid"):= # - maybe play with that later - lots of moving parts
                 bgHH_type[.SD, c(list(hh_type),list(hh_type_fm),list(alone),
                                  list(family),list(geoid)), on = .(bgHH_aet_match_id)]]
...
bgHH_dec[,("bgHH_dre_match_id"):=
                    paste0(tract,family,hh_type,HvL,as.character(100000+sample(1:.N))),
                  by=.(tract,family,hh_type,HvL)]
bgHH_age_eth[,("bgHH_dre_match_id"):=
                    paste0(tract,family,hh_type,HvL,as.character(100000+sample(1:.N))),
                  by=.(tract,family,hh_type,HvL)]
bgHH_age_eth[,c("race","geoid"):=
                 bgHH_dec[.SD, c(list(race),list(geoid)), on = .(bgHH_dre_match_id)]]
bgHH_dec[,("matched_age_eth_race"):=
                 bgHH_age_eth[.SD, list(race), on = .(bgHH_dre_match_id)]]
nrow(bgHH_age_eth[is.na(race)]) #31158
...
length(test[test==FALSE]) #4607
#get rest (31k) after matching on race in other ways...
...
alone?
#START HERE (after double check this block, above) _ SHOULD WE DO is.na(HvL?)
#first add hh_size, since the one-person family will help matches on type
