...
#This is all of Texas, and for this example, we'll just use Harris County. 
...
paste0("Number of Harris County tracts: ", ncol(trSAR_dec_data_from_census_DT[,4:ncol(trSAR_dec_data_from_census_DT)]))
...
cat(check_summary[1])
pop_totals_bg <- bgSAR_dec_data_from_census_DT[label=="!!Total:"&concept=="SEX BY AGE FOR SELECTED AGE CATEGORIES",4:ncol(bgSAR_dec_data_from_census_DT)]
sum(pop_totals_tract[,])==sum(pop_totals_bg[,],na.rm = TRUE)
paste0("Number of Harris County block groups: ", ncol(bgSAR_dec_data_from_census_DT[,4:ncol(bgSAR_dec_data_from_census_DT)]))
paste0("Average population size for Harris County block groups: ", as.integer(mean(as.numeric(pop_totals_bg[,]),na.rm = TRUE)))
paste0("Median population size for Harris County block groups: ", as.integer(median(as.numeric(pop_totals_bg[,]),na.rm = TRUE)))
paste0("Maximum population size for Harris County block group: ", max(as.numeric(pop_totals_bg[,]),na.rm = TRUE))
paste0("Minimum population size for Harris County block group: ", min(as.numeric(pop_totals_bg[,]),na.rm = TRUE))
rm(pop_totals_bg)
rm(pop_totals_tract)
...
bgSARE_dec <- bgSAR_dec_data_from_census %>%
  pivot_longer(6:ncol(bgSAR_dec_data_from_census),names_to = "geoid", values_to = "number_sams",
                   names_transform = list(geoid=as.character)) %>%
