create_babies<-function(people_still_living_in_the_tract,Census_data,tract,seed){
  
  #We're going to do this by tract
  #subset a random tract to play with
  people_still_living_in_the_tract=people_still_living_in_the_tract[people_still_living_in_the_tract$tract==tract,]
  
  #Read in current data
  current_data_for_update=Census_data[Census_data$county==201&Census_data$tract==tract&Census_data$state==48,]
  
  number_of_babies_to_make=current_data_for_update$same.house.under.5-sum(people_still_living_in_the_tract$real_age<5,na.rm=TRUE)
  ...
  if(number_of_babies_to_make > 0){
    number_of_babies=number_of_babies_to_make
  ...
  babies$age=rep(1,nrow(babies))

  return(list(people_still_living_in_the_tract=people_still_living_in_the_tract,babies=babies))
