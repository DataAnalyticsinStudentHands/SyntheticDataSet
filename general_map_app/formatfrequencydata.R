...
number_of_people=read.csv("people.csv")
number_of_people$GEOID=paste0(number_of_people$county,number_of_people$tract)
number_of_people=number_of_people[c("GEOID","B01001_001E")]
colnames(number_of_people)=c("GEOID","total_people_recorded_in_Census")

number_synthetic_people=data.frame(GEOID=syntheticfrequencypertract$GEOID,number_of_simulated_people=rowSums(syntheticfrequencypertract[20:21]))

number_of_people=merge(number_of_people,number_synthetic_people,by="GEOID")

households=merge(households,householdsbytract)
number_of_people=merge(number_of_people,households,by="GEOID")
...
#Looking more at the difference between number of people
number_of_people$difference_between_simulated_and_Census=number_of_people$total_people_recorded_in_Census-number_of_people$number_of_simulated_people
boxplot(number_of_people$difference_between_simulated_and_Census)
summary(number_of_people$difference_between_simulated_and_Census)
...
apply(comparison,2,function(x)length(unique(x)))
why_duplicates_not_noticed_201211000=as.data.frame(apply(weird_tract_201211000,2,function(x)gsub('\\s+', '',x)))
duplicated(why_duplicates_not_noticed_201211000)
#