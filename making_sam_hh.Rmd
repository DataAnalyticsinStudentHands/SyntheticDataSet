...
Let's finish up the age representation for the households, and then see if we can use that combined table to construct a better overall representation of households.

We can add H17, on the race table to H18 already joined with P22, and then add ethnicity by specification across the triangle relation. This then gives us a robust match of age with the other household characteristics.
...
dec_bgHH_type_tenure_age_10[,("hhr_tenure_age_match_id"):=
                    paste0(geoid,family,as.character(100000+seq.int(1:.N))),
                  by=.(geoid,family)]
dec_bgHH_type_tenure_age_10[,c("race","beg_age_P22","end_age_P22"):=
                    dec_bgHHr_tenure_age_10[.SD, c(list(beg_age_P22),list(end_age_P22)), 
...
THEN JUST USE TO JOIN ETHNICITY GOING OTHER WAY; THEN JOIN THAT BACK TO MAIN

```{r join hh_type and age to create a better tenure with type and size data}
dec_bgHHe_tenure_age_10[,("hhe_tenure_age_match_id"):=
                    paste0(geoid,family,beg_age_P22,end_age_P22,as.character(100000+seq.int(1:.N))),
                  by=.(geoid,family,beg_age_P22,end_age_P22)]
dec_bgHH_type_tenure_age_10[,("hhr_tenure_age_match_id"):=
                    paste0(geoid,family,beg_age_P22,end_age_P22,as.character(100000+seq.int(1:.N))),
                  by=.(geoid,family,beg_age_P22,end_age_P22)]
dec_bgHH_type_tenure_age_10[,("ethnicity"):=
                    dec_bgHHe_tenure_age_10[.SD, list(ethnicity), 
                                            on = .(hhr_tenure_age_match_id)]]
```
...
dec_bgHHr_total_pop_10 <- dec_bgHH_total_pop_data_10 %>%
    ...
    paste0("Number of households in file: ", nrow(dec_bgHHe_total_pop_10))
    #should clean up
```
