...
```
We could run through it once with only the 361,888 householders who live alone listed from each side, and then match the others on a random sample. Or we could generate a new number, following some reasoning, that we then use for the sort. We'll do a bit of both. Following the approach we used on the average size, above, let's assign numbers based on family_detail, while also creating a little ifelse ("ifelse(hh_size_n==1,"1_person",">1")") in the name for the matching.
...
dec_bgHHr_10[order(hh_size_n),
             ("hh_family_match_id"):=
                  ...
                  by=.(geoid,family,one_match,race)]
dec_bgHHr_type_10[order(family_detail_matcher), #breaks the .N! order(family_detail_matcher)
                  ("hh_family_match_id"):=
...
THE SYMMETRY TEST SHOULD STILL WORK!! BUT ALSO WANT TO SEE ABOUT THE DISTRIBUTION OF THE OTHERS!!!
...
length(test[test==F])==0
```
Let's do a couple of other easy tests to look at the statistical structure, too.
...
It would be foolish to try to encapsulate all of statistical practice with one sweeping gesture, but it does seem safe to say that the more aware you are of the presuppositions that lie behind any given practice, the more likely you are to avoid bias and prejudice - and other forms of sloppy statistical thinking. What the last two examples were meant to show is that there are ways in which the statistical structures in simple ideas like average can and should be understood within the context of the space that it is projected onto (or constructed as an object within). Without getting into the deeper philosophical questions around constructivism (in its myriad forms), one notes that it's possible to become actively engaged with at least some of the contours that have shaped that space, and to dive into the reasons that the space has the current shape it has. 

What we've also tried to highlight is that there is a reason to think that attention to the relational components of the constructed representation will lead to better overall clarity, consistency, and computability. The first tool we started using was the commutative triangle, and we've already pointed to the fact that many mathematicians would recognize it as an informal symmetric monoidal category (SMC for short). When used appropriately, they provide some other important tools for our purposes, and we'll try to slowly introduce them into - and motivate them by - the construction of Sam City. [[BOY THIS IS A LONG SENTENCE]] We're trying to ground the choices in a different horizon of questions - spaces and relations as structures within which individuals are placed and emerge as such - as opposed to saying that the old approach to construction - determining what counts as an individual (or aggregate of individuals) and adding relations (or dimensions or factors) with sophisticated techniques for modifying the representation to account for uncertainty, etc - is simply wrong, we are saying that there are choices that descend from that first decision about whether you're looking at the relation or the individual.



section title - endpoints, terminal objects, and paths

One of our basic tools for constructing Sam City is using a relation as the unit that we add to, with the individual level (in this case a household) serving as an endpoint for the relation, and something that only fully emerges in its robust fullness as more and more of the relations are pulled together and allowed to intersect in that otherwise empty space. In the terms of symmetric monoidal categories, that endpoint is a terminal object, and it allows us to understand the point around which commutative triangle structures are found (or constructed), and to understand the algebraic possibilities that arise along with those constructions. In that vein, we turn to H16, tenure (rent or own) by household size by race and ethnicity. Since the relations between household size and race and ethnicity are the same for each individual, we can build the city from the fact that both share an edge of a triangle, which is commutative at the type level of the household in this case. We are going to build a somewhat more complicated spatial structure, and at the end we'll talk briefly about why the symmetric monoidal categories provide greater clarity than imposing simplicial complexes (the geometric representation of repeated structures in space).

Let's download H16 and look at it for some clues to how to build the data representation with the right commutative structure. Very simply, if the relation between household size and race or ethnicity matches on both data tables, then we'll get the relationship from tenure to household type for free - without having introduced any uncertainty into the construction, at this point, although we'll have to ensure that there aren't illegitimate (or unaccounted for) steps in further building down the line. (do we need to include a diagram?) At some point, mention that we have H14, Tenure by race of householder, which is just a subset of this. But it does let you have the H11 - total population in occupied housing units by tenure by race/eth. And has average housing size by tenure H12, which we can triangulate, too., with H17 Tenure by age of householder, with age, in the ten year blocks. H18 hh_type by tenure by age is helpful, and it also has the same under 18 over 18 that we had with hh_type. The HCT3 and HCT4 seem to overlap with the same under type, too - could be parallel in the same way that race / eth are. P16 is population in households by age and race, but only at the under 18 and 18 and over level.
...
Let's simply add the tenure information onto the household type and size information, but with an eye toward comparing it with a process that builds the construction from the commutative triangles. The process is similar to what we did with the sex, age, and race/ethnicity files, where we were able to use age_range as part of the relation and add ethnicity back onto the individuals who had only race for the tract level. 
...
```{r test tables expanded for P28 and H16}
nrow(dec_bgHHr_10[is.na(tenure_test1)])
test <- table(dec_bgHHr_10[,geoid],
              dec_bgHHr_10[,hh_size_n],
              dec_bgHHr_10[,race],
              dec_bgHHr_10[,tenure_test1]
  ...
  dec_bgHHr_tenure_10[,hh_size_n],
  dec_bgHHr_tenure_10[,race],
  dec_bgHHr_tenure_10[,tenure_test1]
)
length(test[test==F])==0
...
test <- table(dec_bgHHe_10[,geoid],
  ...
  dec_bgHHe_tenure_10[,hh_size_n],
  dec_bgHHe_tenure_10[,ethnicity],
  dec_bgHHe_tenure_10[,tenure_test1]
)
length(test[test==F])==0
#have to think through when and where to rm()
...
Now we can download P22, 2010 decennial block group level data by household type by age, but without race and ethnicity.
...
dec_bgHH_type_tenure_age_10 <- dec_bgHH_type_tenure_age_data_10 %>%
      ...
      mutate(label = str_remove_all(label,"Other family!!"),
        ...
        label = str_remove_all(label,"Total!!")) %>%
      filter(label != "Total") %>% 
      separate(label, c("tenure","family","sex_hh","hh_status","age_range"), sep = "!!", remove = F, convert = FALSE) %>%
...
We quickly see that H18 needs some of the information from P18 to actually join correctly with P22, above. Let's just join them into one HHr_type file. We already know quite a bit about ethnicity and race at the individual and household levels, so will need to be careful about how this information gets joined - otherwise the commutative structures could be lost. 
MOVING P18 UP


#add p18 to h18, and then add to p22
