...
Let's look at some statistics to see how the newly generated household size looks compared to the originals. First, let's confirm that we didn't accidentally assign the household size to the same households. We're looking for a relatively random distribution, just showing that the old hh_size_n is independent from the new hh_size_generated.
...
```{r summary for hh_size_generated}
  ...
  paste0("All - order is published average, calculated average from P28, and calculated average from newly generated household size")
  summary(dec_bgHH_avg_size_10[name=="P017001",avg_hh_size])
  summary(dec_bgHHr_10[,(avg_hh_size_bg)])
...
Let's also look at the variation within a single block group.
...
summary(dec_bgHHr_10[str_detect(geoid,"48_201_233"),(hh_size_generated)])
```
