...
#there's a weird glitch in the number available to match (130 tracts literally cannot match)
...
#nrow(test[test>0])# 130 #there's something way off, since living alone and not living alone should also be there!
#sum(test[test>0]) #7074 which means that's the number that don't exist in that tract
#max(test[test>0]) #231
#sum(test[test<0]) #-1537081, which, when added to 7074 = 1530007 - but it means there are 130 tracts with no living alone hh?
#this would make sense, if all tracts in test were negative
#for now, don't match final .2%, but need to remember...

#table(tr_nfhh[is.na(match_nf),alone])
...
#tr_nfhh[,("tract"):=str_remove_all(substr(GEOID,1,13),"_")]
...
#length(test[test!=0]) #4270 (of 55168) 
...
#nrow(tr_hhSizeTypeOwnKids[is.na(match_STOK)])#5285 .05%
...
#tr_hhCouple[,("tract"):=str_remove_all(substr(GEOID,1,13),"_")]
#length(table(tr_hhCouple[,tract])) #6855
...
bg_hhOwnKids[,("sex"):=fcase(household_type_4=="Female householder, no spouse or partner present","Female",
                             ...
                             default = "Sex not known")]
#alone and rel_in_house match on "Living alone" only
...
#nrow(bg_hhOwnKids[is.na(hh_type_3)&hh_over_64=="Householder 65 years and over"]) #456
...
#nrow(tr_hhCouple[is.na(match_OK)&alone=="Living alone"])==0
#table(bg_hhOwnKids[,alone],bg_hhOwnKids[,rel_in_house])==c(2607626,0,0,0,0)
#table(bg_hhOwnKids[,alone],bg_hhOwnKids[,household_type_4])==c(0,1360582,1247008,0)
...
tr_hhCouple[is.na(match_OK),("match_OK"):=
              bg_hhOwnKids[.SD,list(household_type_4),on=.(bg_OKso_match_id)]]
#nrow(tr_hhCouple[is.na(match_OK)])# 639922
#table(bg_hhOwnKids[,alone],bg_hhOwnKids[,rel_in_house],bg_hhOwnKids[,family])
#need to decide what to do!! - less than 1% for the most part...
#without sex to pick up couples with sex named, but move over where known; need to compare with originals
#doesn't add any more matches
...
bg_hhOwnKids[,("sex"):=fcase(str_detect(household_type_4,"Female"),"Female",
             ...
             default = sex)]
#should we try to fix ad hoc?
#table(bg_hhOwnKids[,family_type],bg_hhOwnKids[,rel_in_house]) # ~5k off total
#table(bg_hhOwnKids[,family_type],bg_hhOwnKids[,alone])
#table(bg_hhOwnKids[,rel_in_house],bg_hhOwnKids[,alone])
#bg_hhOwnKids[,("rel_in_house"):=fcase(str_detect(family_type,"not")&rel_in_house=="Living alone",
#                                      "With own children under 18",
#                                      default = rel_in_house)]
#

#rm(tr_hhCouple) - keep for now, in case better (non-ad hoc) way of matching later...
#create a better combined family_type
...
bg_hhTypeTenure[,("alone"):=fcase(no_spouse_sex=="Living alone","Living alone",
                                  default = "Not living alone")]
...
bg_hhTypeTenure[,("family_type_7"):=fcase(str_detect(family_type,"Female")&
                                         ...
                                         # "Family households",
                                          default = family_type)]
#FAMILY_TYPE 7 is not matching very well!!! - seems like the "Other family" mix up may be problem

#do with hh_over_64 first; keeping bg_hhTypeTenure age_range for end...
bg_hhTypeTenure[,("bg_TT_match_id"):=
              paste0(GEOID,alone,family,family_type_7,sex_match,age_range_3,as.character(100000+sample(1:.N))),
            by=.(GEOID,alone,family,family_type_7,sex_match,age_range_3)]
bg_hhOwnKids[,("bg_TT_match_id"):=
               paste0(GEOID,alone,family_2,family_type_7,sex,hh_over_64,as.character(100000+sample(1:.N))),
             by=.(GEOID,alone,family_2,family_type_7,sex,hh_over_64)]
bg_hhTypeTenure[,c("hh_type_3","hh_type_4","rel_in_house","own_kids",
                   "sex","same_sex","couple_gender","family_4","family_type_4"):=
                  bg_hhOwnKids[.SD,c(list(hh_type_3),list(household_type_4),list(rel_in_house),list(own_kids),
                                     list(sex),list(same_sex),list(couple_gender),
                                 list(family),list(family_type)),on=.(bg_TT_match_id)]]
bg_hhOwnKids[,("match_TT"):=
                  bg_hhTypeTenure[.SD,list(rent_own),on=.(bg_TT_match_id)]]
#nrow(bg_hhOwnKids[is.na(match_TT)])#9597807 
#nrow(bg_hhOwnKids[is.na(match_TT)&hh_over_64=="Householder 65 years and over"])#34
#then without age 
bg_hhTypeTenure[is.na(hh_type_3),("bg_TTa_match_id"):=
                  paste0(GEOID,alone,family,family_type_7,sex_match,as.character(100000+sample(1:.N))),
                by=.(GEOID,alone,family,family_type_7,sex_match)]
bg_hhOwnKids[is.na(match_TT),("bg_TTa_match_id"):=
               paste0(GEOID,alone,family_2,family_type_7,sex,as.character(100000+sample(1:.N))),
             by=.(GEOID,alone,family_2,family_type_7,sex)]
...
bg_hhOwnKids[is.na(match_TT),("match_TT"):=
               bg_hhTypeTenure[.SD,list(rent_own),on=.(bg_TTa_match_id)]]
#nrow(bg_hhOwnKids[is.na(match_TT)])#2803919 #50k better by matching on alone as well
...
#looks like Nonfamily households living alone is exactly equal to family households == "Other family"!!
#changing below to family_type_7, instead of family (might switch back)
...
bg_hhTypeTenure[is.na(hh_type_3),("bg_TTbg_match_id"):=
                  paste0(GEOID,alone,family,sex_match,as.character(100000+sample(1:.N))),
                by=.(GEOID,alone,family,sex_match)]
bg_hhOwnKids[is.na(match_TT),("bg_TTbg_match_id"):=
               paste0(GEOID,alone,family_2,sex,as.character(100000+sample(1:.N))),
             by=.(GEOID,alone,family_2,sex)]
bg_hhTypeTenure[is.na(hh_type_3),c("hh_type_3","hh_type_4","rel_in_house","own_kids",
                                   "sex","same_sex","couple_gender","family_4","family_type_4"):=
                  bg_hhOwnKids[.SD,c(list(hh_type_3),list(household_type_4),list(rel_in_house),list(own_kids),
                                     list(sex),list(same_sex),list(couple_gender),
...
bg_hhOwnKids[is.na(match_TT),("match_TT"):=
               bg_hhTypeTenure[.SD,list(rent_own),on=.(bg_TTbg_match_id)]]
#nrow(bg_hhOwnKids[is.na(match_TT)])#134069 
...
bg_hhTypeTenure[is.na(hh_type_3),("tr_TTbg_match_id"):=
                  paste0(tract,alone,family,sex_match,as.character(100000+sample(1:.N))),
                by=.(tract,alone,family,sex_match)]
bg_hhOwnKids[is.na(match_TT),("tr_TTbg_match_id"):=
               paste0(tract,alone,family_2,sex,as.character(100000+sample(1:.N))),
             by=.(tract,alone,family_2,sex)]
bg_hhTypeTenure[is.na(hh_type_3),c("hh_type_3","hh_type_4","rel_in_house","own_kids",
                                   "sex","same_sex","couple_gender","family_4","family_type_4"):=
                  bg_hhOwnKids[.SD,c(list(hh_type_3),list(household_type_4),list(rel_in_house),list(own_kids),
                                     list(sex),list(same_sex),list(couple_gender),
...
bg_hhOwnKids[is.na(match_TT),("match_TT"):=
               bg_hhTypeTenure[.SD,list(rent_own),on=.(tr_TTbg_match_id)]]
#nrow(bg_hhOwnKids[is.na(match_TT)])#10670
#without sex
bg_hhTypeTenure[is.na(hh_type_3),("bg_TTbg_match_id"):=
...
bg_hhOwnKids[is.na(match_TT),("match_TT"):=
               bg_hhTypeTenure[.SD,list(rent_own),on=.(bg_TTbg_match_id)]]
#nrow(bg_hhOwnKids[is.na(match_TT)])#7158 
#at tract
bg_hhTypeTenure[is.na(hh_type_3),("tr_TTbg_match_id"):=
...
#nrow(bg_hhOwnKids[is.na(match_TT)])#6260
...
#nrow(bg_hhOwnKids[is.na(match_TT)])==0
...
bg_hhTypeTenure[,("sex"):=fcase(str_detect(family_type_7,"Female"),"Female",
                                ...
                                default = sex)]
#alone is broken in the several thousand range - also need it to account for rel_in_house!!
#bg_hhTypeTenure[,("alone1"):=fcase(str_detect(family_type_7,"solitary"),"Living alone",
#                                  str_detect(family_type_7,"not alone"),"Not living alone",
#                                  str_detect(family_type_7,"Married"),"Not living alone",
#                                  default = alone)]
#table(bg_hhTypeTenure[,alone],bg_hhTypeTenure[,alone1]) #about 8k off
...
#test <- table(bg_hhOwnKids[,tract],
#              bg_hhOwnKids[,own_kids],
#              bg_hhOwnKids[,hh_type_3])==
#  table(bg_hhTypeTenure[,tract],
#        bg_hhTypeTenure[,own_kids],
#        bg_hhTypeTenure[,hh_type_3])
#length(test[test==F])/
#length(test) #at tract, == 0; at block_group, == .43 
...
#do with copath and codom - need to draw - then should be better match, below.
...
if(names(tr_hhTypeOwnKidsRE_data_from_census)[11]=="label_1"){
  #labels determined by hand
  label_c1 <- c("family_2","family_type","own_kids","kid_age_range_3","re_code","race") 
...
#remember this is only families
#reshape a bit and make list of individuals
Geoids <- colnames(tr_hhTypeOwnKidsRE_data[,.SD,.SDcols = startsWith(names(tr_hhTypeOwnKidsRE_data),state)])
tr_hhTypeOwnKidsRE_melted <- melt(tr_hhTypeOwnKidsRE_data, id.vars = c("family_2","family_type","own_kids","kid_age_range_3","re_code","race"), measure.vars = Geoids,
...
#remember only families
#add I to the A in tr_hhTypeOwnKidsR, 
tr_hhTypeOwnKidsR[re_code=="A",("tr_ownkidsRI_match_id"):=
                    paste0(GEOID,family_2,family_type,own_kids,kid_age_range_3,as.character(100000+sample(1:.N))),
                  by=.(GEOID,family_2,family_type,own_kids,kid_age_range_3)]
...
#matching on family_type_7 ensures it's only the families
...
bg_hhTypeRE[re_code%in%c("Q","R","S","T","U","V"),("kid_age_range_3"):=
              tr_hhTypeOwnKidsHnotP[.SD,list(kid_age_range_3),on=.(tr_ownkidsHnotP_match_id)]]
...
#nrow(tr_hhTypeOwnKidsHnotP[is.na(match_bg)])/nrow(tr_hhTypeOwnKidsHnotP) #8%

#distribute rest among different family types for age of kids, but not overwriting own_kids or re_code
#distribute again, using the own_kids on bg_hhTypeRE
tr_hhTypeOwnKidsHnotP[is.na(match_bg),("tr_ownkidsHnotP_match2_id"):=
                        paste0(GEOID,own_kids,as.character(100000+sample(1:.N))),
                      by=.(GEOID,own_kids)]
bg_hhTypeRE[re_code%in%c("Q","R","S","T","U","V")&is.na(kid_age_range_3),("tr_ownkidsHnotP_match2_id"):=
              paste0(tract,own_kids,as.character(100000+sample(1:.N))),
            by=.(tract,own_kids)]
bg_hhTypeRE[re_code%in%c("Q","R","S","T","U","V")&is.na(kid_age_range_3),
            ("kid_age_range_3"):=
              tr_hhTypeOwnKidsHnotP[.SD,list(kid_age_range_3),on=.(tr_ownkidsHnotP_match2_id)]]
...
bg_hhTypeRE[re_code%in%c("I","P"),("tr_ownkidsIP_match_id"):=
              paste0(tract,family_type_7,own_kids,as.character(100000+sample(1:.N))),
            by=.(tract,family_type_7,own_kids)]
bg_hhTypeRE[re_code%in%c("I","P"),("kid_age_range_3"):=
              tr_hhTypeOwnKidsR[.SD,list(kid_age_range_3),on=.(tr_ownkidsIP_match_id)]]
...
#distribute again, saving the own_kids on bg_hhTypeRE
tr_hhTypeOwnKidsR[re_code=="A"&is.na(match_bgIP),("tr_ownkidsIP_match2_id"):=
                        paste0(GEOID,own_kids,as.character(100000+sample(1:.N))),
                      by=.(GEOID,own_kids)]
bg_hhTypeRE[re_code%in%c("I","P")&is.na(kid_age_range_3),("tr_ownkidsIP_match2_id"):=
              paste0(tract,own_kids,as.character(100000+sample(1:.N))),
            by=.(tract,own_kids)]
bg_hhTypeRE[re_code%in%c("I","P")&is.na(kid_age_range_3),("kid_age_range_3"):=
              tr_hhTypeOwnKidsR[.SD,list(kid_age_range_3),on=.(tr_ownkidsIP_match2_id)]]
tr_hhTypeOwnKidsR[re_code=="A"&is.na(match_bgIP),("match_bgIP"):=
                        bg_hhTypeRE[.SD,list(family_type_7),on=.(tr_ownkidsIP_match2_id)]]
#nrow(tr_hhTypeOwnKidsR[re_code=="A"])-nrow(tr_hhTypeOwnKidsR[!is.na(match_bgIP)]) #11303
table(bg_hhTypeRE[,kid_age_range_3])

#rest, including the non-matches on A and the H not P re_codes
tr_hhTypeOwnKidsR[is.na(match_bgIP),("tr_ownkidsrest_match_id"):=
                    paste0(GEOID,family_type_7,own_kids,as.character(100000+sample(1:.N))),
                  by=.(GEOID,family_type_7,own_kids)]
bg_hhTypeRE[is.na(kid_age_range_3),("tr_ownkidsrest_match_id"):=
              paste0(tract,family_type_7,own_kids,as.character(100000+sample(1:.N))),
            by=.(tract,family_type_7,own_kids)]
bg_hhTypeRE[is.na(kid_age_range_3),("kid_age_range_3"):=
              tr_hhTypeOwnKidsR[.SD,list(kid_age_range_3),on=.(tr_ownkidsrest_match_id)]]
tr_hhTypeOwnKidsR[is.na(match_bgIP),("match_bgIP"):=
                    bg_hhTypeRE[.SD,list(re_code),on=.(tr_ownkidsrest_match_id)]]

table(tr_hhTypeOwnKidsR[,kid_age_range_3])-table(bg_hhTypeRE[,kid_age_range_3])
table(bg_hhTypeRE[,own_kids],bg_hhTypeRE[,kid_age_range_3],useNA = "ifany")
...
#nrow(bg_hhTenureAR[is.na(tenure)]) #got ~90% #1123846/10491147
...
#nrow(bg_hhTenureAR)-nrow(bg_hhTypeRE[!is.na(age_range_9)]) #431113 / became 416166 with bg_hhTenureRE
...
#        bg_hhTypeRE[,rent_own],
#        bg_hhTypeRE[,re_code_14]))
#mean(test[1:14]) #.045 - all attributable to bad match on re_code with Type
...
#fix weirdness on kid_18; remember that it's a different underlying count!! kid_18 is all kids, not just own;kid_age_range_3 is own kids
bg_hh18Tenure[,("kid_18_match"):=str_replace(kid_18," children"," own children")]

#MISSING STUFF HERE - SEE NEWEST SCREEN GRAB IN DESKTOP
...
bg_hh18Tenure[,("tr_18_match_id"):=
              paste0(tract,tenure,kid_18_match,as.character(100000+sample(1:.N))),
            by=.(tract,tenure,kid_18_match)]
...
bg_hhTypeRE[,("bg_18_match_id"):=
                     paste0(GEOID,rent_own,own_kids,kid_age_range_3,as.character(100000+sample(1:.N))),
                   by=.(GEOID,rent_own,own_kids,kid_age_range_3)]
bg_hh18Tenure[,("bg_18_match_id"):=
                paste0(GEOID,tenure,kid_18_match,kid_age_range_3,as.character(100000+sample(1:.N))),
              by=.(GEOID,tenure,kid_18_match,kid_age_range_3)]
bg_hh18Tenure[,("match_18TRE"):=
                bg_hhTypeRE[.SD,list(rent_own),on=.(bg_18_match_id)]]
bg_hhTypeRE[,c("kid_age_range_3t","all_kid_18"):=
              bg_hh18Tenure[.SD,c(list(kid_age_range_3),list(kid_18)),
                                   on=.(bg_18_match_id)]]
#ugh - what am I needing to measure here??
table(bg_hhTypeRE[,own_kids],bg_hhTypeRE[,kid_age_range_3t])
#nrow(bg_hhTypeRE[own_kids=="With own children under 18 years"])-nrow(bg_hhTypeRE[is.na(all_kid_18)]) #434374 - have to pick up rest of kids, not own, but don't know if they're family or not
bg_hhTypeRE[is.na(all_kid_18),("bg_18a_match_id"):=
...
bg_hhTypeRE[is.na(all_kid_18),c("kid_age_range_3t","all_kid_18"):=
              bg_hh18Tenure[.SD,c(list(kid_age_range_3),list(kid_18)),
                            on=.(bg_18a_match_id)]]


bg_hhTypeRE[,("bg_18_match2_id"):=
              paste0(GEOID,own_kids,kid_age_range_3,as.character(100000+sample(1:.N))),
            by=.(GEOID,own_kids,kid_age_range_3)]
bg_hh18Tenure[,("bg_18_match2_id"):=
                paste0(GEOID,kid_18,kid_age_range_3,as.character(100000+sample(1:.N))),
              by=.(GEOID,kid_18,kid_age_range_3)]
bg_hh18Tenure[is.na(match_18TRE),("match_18TRE"):=
                bg_hhTypeRE[.SD,list(rent_own),on=.(bg_18_match2_id)]]
bg_hhTypeRE[,c("kid_age_range_3","all_kid_18","rent_own"):=
              bg_hh18Tenure[.SD,c(list(kid_age_range_3),list(kid_18),list(tenure)),
                            on=.(bg_18_match2_id)]]
...
bg_hhTypeRE[,("household_75"):=fcase(as.numeric(substr(age_range_9,13,14))>74,
                                     "Households with one or more people 75 years and over",
                                     default = "Households with no people 75 years and over")]
bg_hhTypeRE[,("household_65"):=fcase(as.numeric(substr(age_range_9,13,14))>64,
                                     "Households with one or more people 65 years and over",
                                     default = "Households with no people 65 years and over")]
bg_hhTypeRE[,("household_60"):=fcase(as.numeric(substr(age_range_9,13,14))>59,
                                     "Households with one or more people 60 years and over",
                                     default = "Households with no people 60 years and over")]
...
#nrow(bg_hh65SizeType[!is.na(household_75)])==nrow(tr_hh75SizeType[household_75=="Households with one or more people 75 years and over"]) 
...
#table(bg_hh65SizeType[,household_60],useNA = "ifany")
...
#                                               !is.na(household_60), #same as 65 here
...
#move over 60-65
table(bg_hh65SizeType[,household_60],useNA = "ifany")
table(tr_hh60SizeType[,household_60],useNA = "ifany")
...
bg_hh65SizeType[,("household_60"):=fcase(is.na(household_60),
                                         "Households with no people 60 years and over",
                                         default = household_60)]
#table(bg_hh65SizeType[,household_75])==table(tr_hh75SizeType[,household_75])
...
tr_hh60SizeType[is.na(match_bg_60_75),
                ("bg_15_match_id"):=
                  paste0(GEOID,hh_size_2,household,as.character(100000+sample(1:.N))),
                by=.(GEOID,hh_size_2,household)]
bg_hh65SizeType[household_60=="Households with no people 60 years and over",
                ("bg_15_match_id"):=
                  paste0(tract,hh_size_2,household,as.character(100000+sample(1:.N))),
                by=.(tract,hh_size_2,household)]
tr_hh60SizeType[is.na(match_bg_60_75),
...
#table(bg_hh65SizeType[,household_60])==table(tr_hh60SizeType[,household_60])
#table(bg_hh65SizeType[,household_60],bg_hh65SizeType[,household_65],bg_hh65SizeType[,household_75],useNA = "ifany")

#households can have more than one at 60, but if they have any they count as household and thus are more than just the ones with head at that age

#then back to bg_hhTypeRE
#look for age of hh and start with them, since they have to be at least one in that household.
#match for all the ones already designated
#afterwards, find matches for households with seniors also in them
#all householders over 64 and/o 74 have at least one person their own age
#testing from earlier assignments
table(bg_hhTypeRE[,household_75],bg_hhTypeRE[,age_range_9],useNA = "ifany")
table(bg_hhTypeRE[,household_60],bg_hhTypeRE[,age_range_9],useNA = "ifany")
bg_hhTypeRE[age_range_3=="Householder 65 years and over",("bg_65T75_match_id"):=
              paste0(tract,no_spouse_sex,family,household_75,household_65,household_60,
                     as.character(100000+sample(1:.N))),
            by=.(tract,no_spouse_sex,family,household_75,household_65,household_60)]
bg_hh65SizeType[household_65=="Households with one or more people 65 years and over",
                ("bg_65T75_match_id"):=
                  paste0(tract,no_spouse,household,household_75,household_65,household_60,
                         as.character(100000+sample(1:.N))),
                by=.(tract,no_spouse,household,household_75,household_65,household_60)]
bg_hhTypeRE[age_range_3=="Householder 65 years and over",
            c("household_75","household_65","household_60"):=
              bg_hh65SizeType[.SD,c(list(household_75),list(household_65),list(household_60)),
                              on=.(bg_65T75_match_id)]]
bg_hh65SizeType[household_65=="Households with one or more people 65 years and over",
                ("match_bg60_75T"):=
                  bg_hhTypeRE[.SD,list(re_code_14),on=.(bg_65T75_match_id)]]
nrow(bg_hh65SizeType[is.na(household_60)&
                       household_65=="Households with one or more people 65 years and over"])==0
table(bg_hhTypeRE[,household_75],bg_hhTypeRE[,age_range_9],useNA = "ifany")
table(bg_hhTypeRE[,household_60],bg_hhTypeRE[,age_range_9],useNA = "ifany")
table(bg_hhTypeRE[is.na(household_60),age_range_9]) #all 65+ 

#picking up the NAs on the original assignment for householder already over 64
bg_hhTypeRE[is.na(household_60),("bg_65T75a_match_id"):=
              paste0(tract,family,
                     as.character(100000+sample(1:.N))),
            by=.(tract,family)]
bg_hh65SizeType[household_65=="Households with one or more people 65 years and over"&
                ...
                ("bg_65T75a_match_id"):=
                  paste0(tract,household,
                         as.character(100000+sample(1:.N))),
                by=.(tract,household)]
bg_hhTypeRE[is.na(household_60),
            c("household_75","household_65","household_60"):=
              bg_hh65SizeType[.SD,c(list(household_75),list(household_65),list(household_60)),
                              on=.(bg_65T75a_match_id)]]
bg_hh65SizeType[household_65=="Households with one or more people 65 years and over"&
                  is.na(match_bg60_75T),
                ("match_bg60_75T"):=
                  bg_hhTypeRE[.SD,list(re_code_14),on=.(bg_65T75a_match_id)]]
table(bg_hhTypeRE[,household_75],bg_hhTypeRE[,age_range_9],useNA = "ifany") #6386 missed
table(bg_hhTypeRE[,household_65],bg_hhTypeRE[,age_range_9],useNA = "ifany") #all
table(bg_hhTypeRE[,household_60],bg_hhTypeRE[,age_range_9],useNA = "ifany") #all

bg_hhTypeRE[,("county"):=substr(GEOID,4,6)]
bg_hh65SizeType[,("county"):=substr(GEOID,4,6)]
bg_hhTypeRE[as.numeric(substr(age_range_9,13,14))>74 &
              household_75!="Households with one or more people 75 years and over",
            ("bg_65T75f_match_id"):=
              paste0(county,no_spouse_sex,family,as.character(100000+sample(1:.N))),
            by=.(county,no_spouse_sex,family)]
bg_hh65SizeType[household_75=="Households with one or more people 75 years and over" &
                  is.na(match_bg60_75T),
                ("bg_65T75f_match_id"):=
                  paste0(county,no_spouse,household,as.character(100000+sample(1:.N))),
                by=.(county,no_spouse,household)]
bg_hhTypeRE[as.numeric(substr(age_range_9,13,14))>74 &
              household_75!="Households with one or more people 75 years and over",
            ("household_75"):=
              bg_hh65SizeType[.SD,list(household_75),
                              on=.(bg_65T75f_match_id)]]
bg_hh65SizeType[household_75=="Households with one or more people 75 years and over" &
                  is.na(match_bg60_75T),
                ("match_bg60_75T"):=
                  bg_hhTypeRE[.SD,list(re_code_14),on=.(bg_65T75f_match_id)]]
#table(bg_hhTypeRE[,household_60])
table(bg_hhTypeRE[,household_75],useNA = "ifany")
table(bg_hh65SizeType[,household_75],useNA = "ifany") #1177159
table(bg_hhTypeRE[,household_75],bg_hhTypeRE[,age_range_9],useNA = "ifany")
#42 not matched - assign and don't worry about it
bg_hhTypeRE[,("household_75"):=fcase(is.na(household_75),
                                     "Households with one or more people 75 years and over",
                                     default = household_75)]

#some householders of other ages also have a member who is over 74; assigning only to slightly older hh
#family captures all variation in hh_size_2
bg_hhTypeRE[as.numeric(substr(age_range_9,13,14))>34 &
              household_75!="Households with one or more people 75 years and over",
            ("bg_65T75b_match_id"):=
                  paste0(tract,no_spouse_sex,family,as.character(100000+sample(1:.N))),
                by=.(tract,no_spouse_sex,family)]
bg_hh65SizeType[household_75=="Households with one or more people 75 years and over" &
                is.na(match_bg60_75T),
                ("bg_65T75b_match_id"):=
                  paste0(tract,no_spouse,household,as.character(100000+sample(1:.N))),
                by=.(tract,no_spouse,household)]
bg_hhTypeRE[as.numeric(substr(age_range_9,13,14))>34 &
              household_75!="Households with one or more people 75 years and over",
            ("household_75_notHead"):=
              bg_hh65SizeType[.SD,list(household_75),
...
#table(bg_hhTypeRE[,household_60])
table(bg_hhTypeRE[,household_75_notHead],useNA = "ifany") #42 more than target, b/c of extra above
table(bg_hh65SizeType[,household_75],useNA = "ifany") #1177159
table(bg_hhTypeRE[,household_75],bg_hhTypeRE[,age_range_9],useNA = "ifany")
table(bg_hhTypeRE[,household_75_notHead],bg_hhTypeRE[,age_range_9],useNA = "ifany")

#for 65 and up, household with younger head 
bg_hhTypeRE[household_65!="Households with one or more people 65 years and over" &
              age_range_3=="Householder 35 to 64 years", 
            ("bg_65T75c_match_id"):=
...
bg_hhTypeRE[household_65!="Households with one or more people 65 years and over" &
              age_range_3=="Householder 35 to 64 years",
            ("household_65_notHead"):=
              bg_hh65SizeType[.SD,list(household_65),
                              on=.(bg_65T75c_match_id)]]
bg_hh65SizeType[household_65=="Households with one or more people 65 years and over" &
                  is.na(match_bg60_75T),
                ("match_bg60_75T"):=
                  bg_hhTypeRE[.SD,list(re_code_14),on=.(bg_65T75c_match_id)]]
table(bg_hhTypeRE[,household_60],useNA = "ifany") #should be same as 65
table(bg_hhTypeRE[,household_65]) 
table(bg_hhTypeRE[,household_65_notHead])
table(bg_hhTypeRE[,household_65],bg_hhTypeRE[,age_range_9],useNA = "ifany")
table(bg_hhTypeRE[,household_65_notHead],bg_hhTypeRE[,age_range_9],useNA = "ifany")

#for 60-65, we already have everyone with over 64 listed
bg_hhTypeRE[household_60!="Households with one or more people 60 years and over" &
              as.numeric(substr(age_range_9,13,14))>34,
            ("bg_60T65a_match_id"):=
              paste0(tract,no_spouse_sex,family,as.character(100000+sample(1:.N))),
            by=.(tract,no_spouse_sex,family)]
...
bg_hhTypeRE[household_60!="Households with one or more people 60 years and over" &
              as.numeric(substr(age_range_9,13,14))>34,
            ("household_60_notHead"):=
              bg_hh65SizeType[.SD,list(household_60),
                              on=.(bg_60T65a_match_id)]]
bg_hh65SizeType[household_60=="Households with one or more people 60 years and over" &
                  is.na(match_bg60_75T),
                ("match_bg60_75T"):=
                  bg_hhTypeRE[.SD,list(re_code_14),on=.(bg_60T65a_match_id)]]
table(bg_hhTypeRE[,household_60_notHead],bg_hhTypeRE[,age_range_9],useNA = "ifany")
...
bg_hhTypeRE[as.numeric(substr(hh_size_7,1,1))>3 & 
              household_60=="Households with one or more people 60 years and over" &
              all_kid_18=="With children under 18",
...
#            c("household_60","household_65","household_75","no_spouse"):=
#              bg_hh65SizeType[.SD,c(list(household_60),list(household_65),list(household_75),
#                                    list(no_spouse)),on=.(tr_gen_match_id)]]
#bg_hh65SizeType[household_60=="Households with one or more people 60 years and over" &
#                  is.na(match_bg60_75T),
#                ("match_bg60_75T"):=
#                  bg_hhTypeRE[.SD,list(re_code),on=.(tr_gen_match_id)]]
#
#table(bg_hhTypeRE[,household_60])
#table(bg_hhTypeRE[,household_65]) 
#table(bg_hhTypeRE[,household_75])
...
#            c("household_60","household_65","household_75","no_spouse"):=
#              bg_hh65SizeType[.SD,c(list(household_60),list(household_65),list(household_75),
#                                    list(no_spouse)),on=.(bg_gen2_match_id)]]
#bg_hh65SizeType[household_60=="Households with one or more people 60 years and over" &
#                  is.na(match_bg60_75T),
#                ("match_bg60_75T"):=
...
#            c("household_60","household_65","household_75","no_spouse"):=
#              bg_hh65SizeType[.SD,c(list(household_60),list(household_65),list(household_75),
#                                    list(no_spouse)),on=.(bg_1person_match_id)]]
#bg_hh65SizeType[household_60=="Households with one or more people 60 years and over" &
#                  is.na(match_bg60_75T),
#                ("match_bg60_75T"):=
#                  bg_hhTypeRE[.SD,list(re_code),on=.(bg_1person_match_id)]]
##table(bg_hhTypeRE[,household_60]) #still 21 short?
##table(bg_hhTypeRE[,household_65]) #17 short
##table(bg_hhTypeRE[,household_75]) #2 short
#bg_hhTypeRE[,("household_60"):=fcase(is.na(household_60),
#                                     "Households with no people 60 years and over",
#                                     default = household_60)]
#bg_hhTypeRE[,("household_65"):=fcase(is.na(household_65),
#                                     "Households with no people 65 years and over",
#                                     default = household_65)]
#bg_hhTypeRE[,("household_75"):=fcase(is.na(household_75),
#                                     "Households with no people 75 years and over",
#                                     default = household_75)]
...
nrow(bg_hhTypeRE[is.na(hh_size_7)])
nrow(bg_hhSizeTenureR[is.na(match_R)])
#take out re_code_7, HvL and family, and move re_code_7 and re_code_HI over to bg_hhTypeRE? 
table(bg_hhTypeRE[,re_code_7])
table(bg_hhSizeTenureR[is.na(match_R),size])
bg_hhTypeRE[is.na(hh_size_7),("bgSizeTR_match_id"):=
              paste0(GEOID,rent_own,size_3,as.character(100000+sample(1:.N))),
            by=.(GEOID,rent_own,size_3)]
bg_hhSizeTenureR[is.na(match_R),("bgSizeTR_match_id"):=
                   paste0(GEOID,rent_own,size_3,as.character(100000+sample(1:.N))),
                 by=.(GEOID,rent_own,size_3)]
bg_hhTypeRE[is.na(hh_size_7),c("hh_size_7","re_code_7ST","re_code_HI"):=
              bg_hhSizeTenureR[.SD,c(list(size),list(re_code),
                                     list(re_code_HI)),on=.(bgSizeTR_match_id)]]
bg_hhSizeTenureR[is.na(match_R),("match_R"):=
                   bg_hhTypeRE[.SD,list(re_code),on=.(bgSizeTR_match_id)]]
nrow(bg_hhTypeRE[is.na(hh_size_7)])
nrow(bg_hhSizeTenureR[is.na(match_R)])
...
#find more other matches then finish off size
...
#need to fix: table(bg_hhTTSA[,alone],bg_hhTTSA[,family_type_7])
# table(bg_hhTTSA[,alone],bg_hhTTSA[,no_spouse])
