...
#Only build Fortbend Households
households=subset(households,households$county==157)
...
for (index in 1:length(tracts)){
  sample.set=rbind(sample.set,master(157,tracts[index],5,seed=1))
}


#Write households to csv

write.csv(sample.set,"sugarland.csv")
