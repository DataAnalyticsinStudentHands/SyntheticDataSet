...
vars <- c("B05001_002E","B05001_003E","B05001_004E","B05001_005E","B05001_006E")
#Names of these variables
#U.S. citizen, born in the United States#U.S. citizen, born in Puerto Rico or U.S. Island Areas#U.S. citizen, born abroad of American parent(s)#U.S. citizen by naturalization#Not a U.S. citizen
# Get data for all census tracts in TX
census_data <- getCensusApi(acs5_2014_api, key=key, vars, region="for=tract:*&in=state:48")

colnames(census_data)<-c("state","county","tract","U.S. citizen, born in the United States","U.S. citizen, born in Puerto Rico or U.S. Island Areas","U.S. citizen, born abroad of American parent(s)","U.S. citizen by naturalization","Not a U.S. citizen")
#write data frame to csv file without quotes
#write.csv(census_data, file = "citizenstuffforvotingstuff.csv",row.names=FALSE, na="", quote = FALSE)