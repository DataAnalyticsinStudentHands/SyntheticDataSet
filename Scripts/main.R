...
complete_sample_set=foreach (index = 1:786,.combine='rbind')%dopar%{
  ...
  potential_buildings$UNITS[is.na(potential_buildings$UNITS) | potential_buildings$UNITS %in% c("1/1", "1/2")] = 1
  potential_buildings$UNITS[potential_buildings$UNITS %in% c("", "A", "B", "C", "D")] = avg_units

  # From the potential buildings, subset for only buildings not used for group quarters
  houses = potential_buildings[potential_buildings$BUILDING_STYLE_CODE %in% c("101","102", "103", "104", "107","108","109","125","8177","8178","8179","8351","8354","8401","8548","8549","8550","8986","8988","105","8300","8352","8338","8459","8493","8546","8547","8596","8984","8987","8989" ),]
