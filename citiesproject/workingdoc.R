...
getadultasthma <- function(county, tract, syntheticdataset,seed){
  ...
  if (!is.na(adultasthma$Data_Value)){
    over18ages=c("18 to 19","20 to 24","25 to 29","30 to 34","35 to 44","45 to 54","55 to 64","65 to 74","75 to 84","Over 85")
...
get65menuptodate <- function(county, tract, syntheticdataset,seed){
  ...
  if (!is.na(oldmen$Data_Value)){
    ...
    older.men.up.to.date.on.preventitive.services=rep("Not Available for this Census Tract",nrow(syntheticdataset))
  }
...
get65womenuptodate <- function(county, tract, syntheticdataset,seed){
  ...
  oldwomen=oldmen[(oldwomen$tract==tract)&(oldwomen$county==county),]
  if (!is.na(oldwomen$Data_Value)){
    over65ages=c("65 to 74","75 to 84","Over 85")
    older.men.up.to.date.on.preventative.services=ifelse((syntheticdataset$age %in% over65ages)&syntheticdataset$sex=="Male",sample(c("yes","no"),1,prob=c(oldmen$Data_Value/100,(1-oldmen$Data_Value/100))),NA)
  }
  else{
  ...
  over65ages=c("65 to 74","75 to 84","Over 85")
  older.women.up.to.date.on.preventative.services=ifelse((syntheticdataset$age %in% over65ages)&syntheticdataset$sex=="Female",sample(c("yes","no"),1,prob=c(oldwomen$Data_Value/100,(1-oldwomen$Data_Value/100))),NA)
  syntheticdataset$older.women.up.to.date.on.preventative.services=older.women.up.to.date.on.preventative.services
  return(syntheticdataset)
}
