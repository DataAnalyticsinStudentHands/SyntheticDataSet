...
#delayed language
DLD <- read.csv2("~/Downloads/OP_PUDF_2022_TX_delayed_language.csv",header=TRUE, sep = ",")
...
under5girls <- sex_age_race_DT[age_range=="Under 5 years"&race=="_"&sex=="Female"]
tracts_demog <- tracts_demog[under5girls[,7:8],on="GEOID"]
setnames(tracts_demog,"number_sams","under_5_girls")
under5boys <- sex_age_race_DT[age_range=="Under 5 years"&race=="_"&sex=="Male"]
...
setnames(DLD,"PAT_ZIP","zip")
setnames(zctasDT,"NAME20","zip")
DLD_reduced <- DLD[,c("zip","PAT_COUNTY","SEX_CODE","RACE","ETHNICITY","LENGTH_OF_SERVICE","FIRST_PAYMENT_SRC")]
DLD_reduced[,("white"):=.SD[RACE=="4",.N],by=zip]
DLD_reduced[,("black"):=.SD[RACE=="3",.N],by=zip]
DLD_reduced[,("other_race"):=.SD[RACE=="1" | RACE=="2" | RACE=="5",.N],by=zip]
DLD_reduced[,("hispanic"):=.SD[ETHNICITY=="1",.N],by=zip]
DLD_reduced[,("male"):=.SD[SEX_CODE=="M",.N],by=zip]
DLD_reduced[,("female"):=.SD[SEX_CODE=="F",.N],by=zip]
DLD_reduced[,("unknown_gender"):=.SD[SEX_CODE=="U",.N],by=zip]
DLD_reduced[,("same_day"):=.SD[LENGTH_OF_SERVICE=="1",.N],by=zip]
DLD_reduced[,("more_than_one_day"):=.SD[LENGTH_OF_SERVICE!="1",.N],by=zip]
DLD_reduced[,("medicaid"):=.SD[FIRST_PAYMENT_SRC=="MC",.N],by=zip]
DLD_zip <- DLD_reduced[,.SD[1],by=zip]

zctasDT_DLD <- DLD_zip[zctasDT,on="zip"]
zctasDT_DLD <- zctasDT_DLD[!is.na(male)]
st_write(zctasDT_DLD,"~/Downloads/zctasDT_ZIP_DLD_2022.geojson",driver = "GeoJSON",factorsAsCharacter=FALSE)
zip_DLD <- zctasDT_DLD[tracts_demog[,c("zip","zip_under_5")],on="zip"]
zip_DLD <- zip_DLD[!is.na(medicaid)]
zip_DLD <- zip_DLD[,.SD[1],by=zip]
zip_DLD[,("total_DLD"):=same_day+more_than_one_day]
zip_DLD[,("DLD_pct"):=as.numeric(total_DLD)/as.numeric(zip_under_5)]
st_write(zip_DLD,"~/Downloads/DLD_zip_2022.geojson",driver = "GeoJSON",factorsAsCharacter=FALSE)
st_write(tracts_demog,"~/Downloads/DLD_tracts_2022.geojson",driver = "GeoJSON",factorsAsCharacter=FALSE)
