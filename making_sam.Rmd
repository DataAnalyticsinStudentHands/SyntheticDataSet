...
bgSAR_10[re_code=="A",("bg_saW_hh_match_id"):=
                    paste0(tract,race,ethnicity,sex,age_range,as.character(100000+sample(1:.N))),
                  by=.(tract,race,ethnicity,sex,age_range)]
trHHr_10[race=="A",("bg_saW_hh_match_id"):=
                    paste0(tract,race,ethnicity,sex,age_range,as.character(100000+sample(1:.N))),
                  by=.(tract,race,ethnicity,sex,age_range)]
bgSAR_10[re_code=="A",("household"):=
                    trHHr_10[.SD, list("In household"), on = .(bg_saW_hh_match_id)]]
bgSAR_10[re_code=="A"&household!="In household",("household"):="In group quarters"]
#if race=="A", and not "In household", then "In group quarters"
#tests
nrow(trHHr_10[race=="A"])==nrow(bgSAR_10[re_code=="A"&household=="In household"])
test <- table(
  ...
  bgSAR_10[re_code=="A"&household=="In household",age_range]
)==table(
...
START HERE - remember to put the counts for race and ethnicity combinations from bgSAR as part of the if_clause for matching, but on group quarters??? 
Construct it until about to lose commutativity on something, then go back to non-White bgSAR and get numbers per cell and implement an if clause... [may have to do ethnicity in a weird way]



What we have now is a table with the correct number of total rows, with all sex, race, and age_range characteristics properly distributed at the block group level. Ethnicity is correctly distributed for race=="A" and we know the number of Hispanics that need to be distributed at the tract level for the households. We'll add that explicitly from the household ethnicity tables, but before taking that step, we want to ensure that we aren't losing other information that we have. 
