...
The idea with tr_r_eh_spec_prob is that if a row by race is showing up in the gq_spec, but not in the same gq_spec by ethnicity, then we know something about how they are distributed. We expect a lot to be determined by the final cast by tract to bgSAR, with the blocks over-riding from here. Have to make sure that the calculation accounts for zeros, and should it be race over eth, or eth over race???

```{r join over17 gq but those without ethnicity and ordered}
#FROM MONDAY MORNING;NEED TO TEST!!
...
nrow(tr_r_gq_over17_10[!is.na(matched17)&is.na(ethnicity)]), " matched. However, ",
nrow(tr_r_gq_10[is.na(sex)]), " still need a final determination for sex in the overall group quarters race table, in order to match with the givens in bggq")
#SHOULD DO A TABLE TEST TO SEE THAT ALL OVER17 SEX NOW MATCH TO ONE OVER ON TR_R
#THEN GO TO BGSAR FOR THE ETHNICITY, ETC BY BLOCK_GROUP? 
```

What can we do with the counts?? maybe if pull over the tract level race/eth for the rest from bgSAR - then can match by the ones who would fit within the givens...?

Or just order by probability of ethnicity from each - put the count by race back in, and count_r/count_h_remain on one side and then just order(first_age,prob_eth) and joing with bggq at the tract level on 

MOVE TO BGGQ for these 24149
Then the rest, by race - with ethnicity not given? or do a count from bggq with 



We could have the last 271 match just on race...
...
BEFORE DOING THIS: WE HAVE THE COUNTS BY ETHNICITY, AND THE SEX BY GQ_TYPE BY RACE FOR OVER 18; remember we may want the blocks decided here for race/ethnicity to override bgSAR's ethnicity...
...
```
MAYBE SKIP THIS STEP??? OR ONLY USE PARTS OF REST OF FILE, IN ANY CASE!!!
