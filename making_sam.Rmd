...
```
We should check, just like we did on the block group for group quarters, why the totals don't sum, but it's probably similar and we have the right main numbers after expanding.
...
#match on ethnicity, but not on sex and ethnicity - although close! - should think about how to get these to match first...
#maybe add beg_age and end_age to problem cases?? or do them all by some sort of larger process of elimination, 
#and add quarters to households before merging with bgSAR? 
paste0("Here, ethnicity works by each tract for re_code=='A', but not sex, even by itself without tract")
...
length(test[test==FALSE])==0
table(bgSAR_10[re_code=="A"&household!="In household"&first_age>17,sex])
#which is the ground
table(tr_r_gq_10[re_code=="A",sex])
#NOW TEST ON BGGQ_10 - SEE IF THAT FITS, AND THEN THINK ABOUT GOING BACK AND FORTH WITH KIDS AND OTHER ETHNICITIES...
```
...
```{r looking at group quarters after households removed for age range}
table(bggq_10[geoid=="48_201_531700_1",tr_gq_specs])
table(bgSAR_10[geoid=="48_201_531700_1"&household!="In household",age_range])
#and by tract
table(bggq_10[tract=="48201531700",tr_gq_specs])
table(bgSAR_10[tract=="48201531700"&household!="In household",age_range])
#and by type - something is slightly off...
table(bggq_10[tract=="48201531700",gq_type])
table(bgSAR_10[tract=="48201531700"&household!="In household",age_range])
```
