...
#now we'll create matching ids with an if clause
...
START HERE SLOWLY - IS THE INITIAL NEEDED??? MAY GET RID OF ALL _COUNT
...
Joining PCT21 (tr_sa_gq_10) with P43 (bggq_10), since PCT21 just adds group quarter specifics. This follows the rules for triangle commutativity.
...
```{r add gq_s_a_type_count to tr_r_gq_10 from tr_r_gq_over17_10}
...
tr_r_gq_10[,("tr_r_t_gq_match_id"):=
                    paste0(tract,re_code,gq_type,as.character(100000+sample(1:.N))),
                  by=.(tract,re_code,gq_type)]
tr_r_gq_10[,c("sex","gq_s_a_type_count"):=
                    tr_r_gq_over17_10[.SD, c(list(sex),list(gq_s_a_type_count)), on = .(tr_r_t_gq_match_id)]]
tr_r_gq_10[is.na(gq_s_a_type_count),("age_range"):=.("Under 18 years")]
#tests
nrow(tr_r_gq_10[!is.na(gq_s_a_type_count)])==nrow(tr_r_gq_over17_10)
nrow(tr_r_gq_10[!is.na(sex)])==nrow(tr_r_gq_over17_10)
test <- summary(tr_r_gq_over17_10[,gq_s_a_type_count])==
  summary(tr_r_gq_10[!is.na(gq_s_a_type_count),gq_s_a_type_count])
length(test[test==FALSE])==0
nrow(tr_r_gq_10[is.na(gq_s_a_type_count)])==nrow(tr_r_gq_10[!is.na(age_range)])
```
