...
trSAR_10[ethnicity=="H"&re_code!="A",("ethSAR_match"):=
           bgSAR_10[.SD, list(H_re_code), on = .(ethSAR_match_id)]]
#test that they all moved over
nrow(trSAR_10[!is.na(ethSAR_match)])==nrow(!is.na(bgSAR_10[!is.na(H_re_code)]))
bgSAR_10[is.na(H_re_code),("H_re_code"):=0]
bgSAR_10[order(-H_re_code),("H_re_code"):=.SD[1,H_re_code],
         by=.(tract,sex,age_range,re_code)]
#rows that we know by elimination are not possible matches on non-White Hispanic or Latino:
nrow(bgSAR_10[re_code!="A"&H_re_code==0])
#rows that could be matches and are not eliminated on just sex, race, and age
nrow(bgSAR_10[re_code!="A"&H_re_code!=0])-nrow(trSAR_10[!is.na(ethSAR_match)])
#put in something to help order the potential matches
bgSAR_10[re_code!="A",("H_marker"):=H_block+H_re_code] 
#be sure all are marked
nrow(bgSAR_10[!is.na(H_marker)])==nrow(bgSAR_10[re_code!="A"])
bgSAR_10[order(-H_marker)&re_code!="A",("HvI"):=if_else(1:.N>bg_H_remain,"notHvI","HvI"),by=.(geoid,sex,age_range)]
trSAR_10[re_code!="A",("HvI"):=if_else(ethnicity=="H","HvI","notHvI")]

bgSAR_10[is.na(ethnicity),("ethSAR_match1_id"):=paste0(tract,sex,age_range,re_code,HvI,as.character(100000+seq.int(1:.N))),
