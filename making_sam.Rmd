...
#maindir = "~/Users/areb219/Library/CloudStorage/OneDrive-UniversityOfHouston/Social Network Hypergraphs/" #areb laptop
...
trSAR_data_from_census <- trSAR_data_from_census %>%
      select(name,label,concept,starts_with(st_county))
#needs to be numeric for the sum in the test
trSAR_data_from_census <- as.data.table(trSAR_data_from_census)
trSAR_data_from_census[,4:ncol(trSAR_data_from_census)] <- 
  trSAR_data_from_census[,lapply(.SD[,4:ncol(trSAR_data_from_census)], as.numeric)]
head(trSAR_data_from_census[,1:5])
```
...
```{r test for na}
    percent_na <- trSAR_data_from_census[,sum(is.na(.SD))] / 
      (trSAR_data_from_census[,sum(!is.na(.SD))]+trSAR_data_from_census[,sum(is.na(.SD))])
...
test <- colSums(trSAR_data_from_census[label=="Total",4:ncol(trSAR_data_from_census)])*2 ==
      colSums(trSAR_data_from_census[label!="Total",4:ncol(trSAR_data_from_census)])
...
pop_totals_tract <- trSAR_data_from_census[label=="Total"&concept=="SEX BY AGE",4:ncol(trSAR_data_from_census)]
...
paste0("Number of Harris County tracts: ", ncol(trSAR_data_from_census[,4:ncol(trSAR_data_from_census)]))
...
bgSAR_data_from_census <- 
  censusData_byGroupName(censusdir, vintage="2010", state, censuskey, 
                         groupname = "P12",county_num = county,
                         block="block_group",api_type="dec/sf1",path_suff="est.csv")
bgSAR_data <- as.data.table(bgSAR_data_from_census)
bgSAR_data[,4:ncol(bgSAR_data)] <- 
  bgSAR_data[,lapply(.SD[,4:ncol(bgSAR_data)], as.numeric)]
rm(bgSAR_data_from_census)
```
...
check_summary <- census_table_check(bgSAR_data, "SEX BY AGE","individuals") #should get a function that fails appropriately
cat(check_summary[1])
pop_totals_bg <- bgSAR_data[label=="Total"&concept=="SEX BY AGE",4:ncol(bgSAR_data)]
sum(pop_totals_tract[,])==sum(pop_totals_bg[,])
paste0("Number of Harris County block groups: ", ncol(bgSAR_data[,4:ncol(bgSAR_data)]))
...
trSARE <- trSAR_data_from_census %>%
  pivot_longer(4:ncol(trSAR_data_from_census),names_to = "tract", values_to = "number_sams",
...
paste0("There are ",length(unique(trSARE$age)), " unique categories for age (each year up to 100 and then two over 100 groups).")
paste0("There are ",length(unique(trSARE$re_code)), " unique categories for race and ethnicity.")
paste0("Along with two categories for sex, we have ",length(unique(trSARE$age))*length(unique(trSARE$re_code))*2," different categories people can fall into.")
rm(trSAR_data_from_census)
```
...
dt <- as.data.table(list(unique(trSAE$re_code),unique(trSAE$concept)))
...
bgSARE <- bgSAR_data %>%
  pivot_longer(4:ncol(bgSAR_data),names_to = "geoid", values_to = "number_sams",
...
paste0("Total population in this representation is: ",nrow(bgSAR))
...
bgSAR[order(tract,first_age),("age_match_id"):=
...
trSAR[order(tract,age),("age_match_id"):=
                  ...
                  by=.(tract,sex,re_code)]
trSAR[,("age_range"):=
                    bgSAR[.SD, list(age_range), on = .(age_match_id)]]
test <- table(bgSAR[,tract],
              bgSAR[,sex],
              bgSAR[,re_code],
              bgSAR[,age_range]
              )==
...
trSAR[ethnicity=="H"&re_code!="A",("H_re_code"):=.N,by=.(tract,sex,age_range,re_code)]
#then merge
trSAR[ethnicity=="H"&re_code!="A",("ethSAR_match_id"):=
           paste0(tract,sex,age_range,re_code,as.character(100000+sample(1:.N))),
           by=.(tract,sex,age_range,re_code)]
bgSAR[re_code!="A",("ethSAR_match_id"):=
           paste0(tract,sex,age_range,re_code,as.character(100000+sample(1:.N))),
           by=.(tract,sex,age_range,re_code)]
bgSAR[re_code!="A",("H_re_code"):=
                    trSAR[.SD, list(H_re_code), on = .(ethSAR_match_id)]]
trSAR[ethnicity=="H"&re_code!="A",("ethSAR_match_poss"):=
           bgSAR[.SD, list(H_re_code), on = .(ethSAR_match_id)]]
#test that they all moved over
nrow(trSAR[!is.na(ethSAR_match_poss)])==nrow(!is.na(bgSAR[!is.na(H_re_code)]))
bgSAR[re_code!="A"&is.na(H_re_code),("H_re_code"):=0]
bgSAR[order(-H_re_code),("H_re_code"):=.SD[1,H_re_code],
         by=.(tract,sex,age_range,re_code)]
paste0("We know by elimination that ",nrow(bgSAR[re_code!="A"&H_re_code==0]),
...
paste0("Taken together with the ones where multiple matches are possible (",
       nrow(bgSAR[re_code!="A"&H_re_code!=0]),") minus the ones that needed to match (",
       nrow(trSAR[!is.na(ethSAR_match_poss)]),"), we get: ",
       nrow(bgSAR[re_code!="A"&H_re_code==0])+
         nrow(bgSAR[re_code!="A"&H_re_code!=0])-
         nrow(trSAR[!is.na(ethSAR_match_poss)]),
       " which equals the number of rows that don't have ethnicity of H or I: ",
       nrow(trSAR[!ethnicity%in%c("H","I")]))
```

```{r assign HvI and order bgSAR by block group with number left and count misaligned re_code and ethnicity}
setorder(bgSAR,-bg_H_remain,-H_re_code)
bgSAR[re_code!="A",("HvI"):=if_else(1:.N>bg_H_remain,"notHvI","HvI"),by=.(geoid,sex,age_range)]
trSAR[re_code!="A",("HvI"):=if_else(ethnicity=="H","HvI","notHvI")]
#a few tests
table(bgSAR[,HvI])==table(trSAR[,HvI])
test <- table(bgSAR[,tract],
              bgSAR[,sex],
              bgSAR[,age_range],
              bgSAR[,HvI])==
...
sum_missed <- sum(abs(table(bgSAR[,tract],
              bgSAR[,re_code],
              bgSAR[,HvI])==
  table(trSAR[,tract],
        trSAR[,re_code],
...
trSAR[re_code!="A",
         ("ethSAR_id"):=
           paste0(tract,sex,age_range,re_code,HvI,as.character(100000+sample(1:.N))),
           by=.(tract,sex,age_range,re_code,HvI)]
bgSAR[is.na(ethnicity),("ethnicity"):=
                    trSAR[.SD, list(ethnicity), on = .(ethSAR_id)]]
trSAR[re_code!="A",
         ("ethSAR_match"):=
           bgSAR[.SD, list(H_re_code), on = .(ethSAR_id)]]
nrow(trSAR[!is.na(ethSAR_match) | re_code=="A"])==nrow(bgSAR[!is.na(ethnicity)])
paste0("There are a relatively few (",nrow(trSAR[is.na(ethSAR_match)&re_code!="A"])," or ",
       round(nrow(trSAR[is.na(ethSAR_match)&re_code!="A"])/nrow(trSAR),5)*100,
       ...
       nrow(trSAR[is.na(ethSAR_match)&re_code!="A"&ethnicity=="H"])==0, ") so we can just join without HvI")
...
table(trSAR$re_code,trSAR$ethnicity)
paste0("Then the block level up to this point - which looks visually to be in the right direction: ")
table(bgSAR$re_code,bgSAR$ethnicity)
```

```{r finish assigning ethnicity to bg and do tests}
bgSAR[is.na(ethnicity),("ethSAR2_id"):=
           paste0(tract,sex,age_range,re_code,as.character(100000+sample(1:.N))),
         by=.(tract,sex,age_range,re_code)]
trSAR[is.na(ethSAR_match)&re_code!="A",
         ("ethSAR2_id"):=
           paste0(tract,sex,age_range,re_code,as.character(100000+sample(1:.N))),
           by=.(tract,sex,age_range,re_code)]
bgSAR[is.na(ethnicity),("ethnicity"):=
                    trSAR[.SD, list(ethnicity), on = .(ethSAR2_id)]]
test <- table(bgSAR[,tract],
              bgSAR[,sex],
              bgSAR[,age_range],
              bgSAR[,re_code],
...
test <- table(bgSAR[ethnicity%in%c("H","I"),tract],
              bgSAR[ethnicity%in%c("H","I"),sex],
              bgSAR[ethnicity%in%c("H","I"),age_range],
              bgSAR[ethnicity%in%c("H","I"),ethnicity])==
  table(bgSAE[,tract],
        bgSAE[,sex],
        bgSAE[,age_range],
        bgSAE[,re_code])
...
sum(abs(table(bgSAR[ethnicity=="I",geoid],
              bgSAR[ethnicity=="I",sex],
              bgSAR[ethnicity=="I",age_range])-
  table(bgSAE[re_code=="I",geoid],
        bgSAE[re_code=="I",sex],
        bgSAE[re_code=="I",age_range])))==0
bg_sum_eth <- sum(abs(table(bgSAR[ethnicity=="H",geoid],
              bgSAR[ethnicity=="H",sex],
              bgSAR[ethnicity=="H",age_range])-
  table(bgSAE[re_code=="H",geoid],
        bgSAE[re_code=="H",sex],
        bgSAE[re_code=="H",age_range])))
#which is still less than 1% misaligned on Hispanic or Latino (or not) for non-White individuals
bg_sum_eth/nrow(bgSAE[re_code=="H"])
#and for "H" alone
bg_sum_eth_H <- sum(abs(table(bgSAR[ethnicity=="H",geoid])-
  table(bgSAE[re_code=="H",geoid])))
#which is still less than 1% misaligned on Hispanic or Latino (or not) for non-White individuals
bg_sum_eth_H/nrow(bgSAE[re_code=="H"])
```
...
rm(trSAE)
rm(trSAR)
bgSAR[,c("name","label","total","concept","sams_race_id","sar_match_id",
...
if (file.exists(paste0(censusdir,vintage,"/working/bgSAR.RDS"))){
  file.remove(paste0(censusdir,vintage,"/working/bgSAR.RDS"))
}
if (file.exists(paste0(censusdir,vintage,"/working"))){
    print(sprintf("found folder %s", paste0(censusdir,vintage,"/working")))
    saveRDS(bgSAR,paste0(censusdir,vintage,"/working/bgSAR.RDS"))
  }else{
