...
#move number of total Hispanic or Latino onto all rows in block by sex and age_range
bgSAE_10[re_code=="H"&is.na(r_code),("H_block"):=.N,by=.(geoid,sex,age_range)]
bgSAE_10[re_code=="H"&is.na(r_code),("H_tract"):=.N,by=.(tract,sex,age_range)]
...
bgSAE_10[re_code=="H"&is.na(r_code),("sar3_match_id"):=
                    paste0(geoid,sex,age_range,as.character(100000+sample(1:.N))),
                  by=.(geoid,sex,age_range)]
bgSAR_10[re_code!="A",c("H_block","H_tract"):=
...
#bgSAR_10[,("bg_H_remain"):=.SD[1,list(as.integer(H_block))],
#         by=.(geoid,sex,age_range)]
...
https://q.uiver.app/?q=
...
There is some possibility, in the age matching, that someone from one of the block groups has an ethnicity that will be misaligned with the ages by race, since we weren't matching on ethnicity (it isn't known for the non-White block groups, after all). That would be a narrow window for the difference, however, and we will be able to show that no known information is lost.

Now we join the tract table to the block groups, by age_range with count by race with Hispanic or Latino Remaining. 
...
#move H_re_code to bg - then order by re_code in each block and seq.int in that block up to H_block
trSAR_10[ethnicity=="H",("H_re_code"):=.N,by=.(tract,sex,age_range,re_code)]
#then merge
trSAR_10[ethnicity=="H",("ethSAR_match_id"):=
...
bgSAR_10[re_code!="A",("H_re_code_full"):=.SD[1,list(as.integer(H_re_code))],
         by=.(tract,sex,age_range,re_code)]
#H_re_code+H_block just to give a sense for how they contribute to order of whole
bgSAR_10[re_code!="A",("H_marker"):=H_block+H_re_code_full] 
bgSAR_10[order(-H_marker), #&substr(geoid,15,15)=="7" - and only by H_re_code_full????
         ("ethSAR_match1_id"):=
           if_else(.N<H_block,
              paste0(tract,sex,age_range,re_code,as.character(100000+seq.int(1:.N))),
              "no_match"),
         by=.(tract,sex,age_range,re_code)]
trSAR_10[ethnicity=="H",
         ("ethSAR_match1_id"):=
           paste0(tract,sex,age_range,re_code,as.character(100000+sample(1:.N))),
           by=.(tract,sex,age_range,re_code)]
bgSAR_10[,("ethnicity"):=
                    trSAR_10[.SD, list(ethnicity), on = .(ethSAR_match1_id)]]
#then do rest of ethnicity
bgSAR_10[is.na(ethnicity)]
trSAR_10[re_code!="A"&ethnicity!="H"] #check numbers
...
#that will move people too aggressively into the same blocks, but should be tempered by the available distribution
#starting from smallest on both should also move it up a bit


#want to sample up to H_block
#bgSAR_10[,("id"):=paste0(tract,sex,age_range,re_code)]


#trSAR sampled from ethnicity=="H" x age x sex
#if you have the ratio of H_re_code to H_tract on tr and use that as sample odds??
trSAR_10[ethnicity=="H",("H_tract"):=.N,by=.(tract,sex,age_range)]

trSAR_10[order(-H_re_code),seq.int(1:.N)]
bgSAR_10[]#less than number in block, and then subtract again max(as.integer(substr(bgSAR_10[,geoid]))) - do each block sampling
```

THEN FINISH THE TRIANGLE BACK TO bgSAR with ethnicity from trSAR with sex, age_range and HvI non-HvI at tract level.
...
```{r join by HvI and non HvI}
bgSAR_10[,("HvI"):=if_else(is.na(ethnicity),"notHvI","HvI")] #some notHvI will end up being H
trSAR_10[,("HvI"):=if_else(ethnicity%in%c("H","I"),"HvI","notHvI")]
```
...
Then we can empty the bgSAR_10 re_code and race, in order to allow the ethnicity to be attached, but only insofar as it varies between the block group and the tract, which only differs for the non-White population. 

START HERE -- GETTING FALSE!!!
WE KNOW HOW MANY HISPANICS IN EACH BLOCK - ORDER EACH BY GEOID 


```{r join rest of bgSAR by emptying some cells and structuring possible distributions}
#take bgSAR and create a way to triangulate on the ethnicity
...
#then from trSAR_10 matched on everything and ethnicity by sex and age
...
#then match on trSAR on all at the block level
#test that they are the same size
...
length(test[test==FALSE])==0
#see how many of the block group designations on trSAR no longer match with bgSAR
test <- table(bgSAR_10[,tract],
              bgSAR_10[,geoid],
              bgSAR_10[,sex],
              bgSAR_10[,re_code],
...
```{r add block group geoid to trSAR}
#then move over bg on race, sex, and age_range
bgSAR_10[,("geoid_match_id"):=
                    paste0(tract,sex,age_range,re_code,as.character(100000+seq.int(1:.N))),
                  by=.(tract,sex,age_range,re_code)]
trSAR_10[,("geoid_match_id"):=
                    paste0(tract,sex,age_range,re_code,as.character(100000+seq.int(1:.N))),
                  by=.(tract,sex,age_range,re_code)]
trSAR_10[,("geoid"):=
                    bgSAR_10[.SD, list(geoid), on = .(geoid_match_id)]]
test <- table(bgSAR_10[,tract],
              bgSAR_10[,geoid],
              bgSAR_10[,sex],
              bgSAR_10[,re_code],
