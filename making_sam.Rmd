...
To say that "race" is embedded in a biological determination and "ethnicity" in a cultural context, however, is to miss how the structuring questions are motivated and thus to obscure at least some potential pathways toward a more acceptable general approach to embedding relations between individuals and social structures than as sedimentation of layers of causes and effects. 
...
```
We should check, just like we did on the block group for group quarters, why the totals don't sum, but it's probably similar and we have the right main numbers after expanding. Here, we do a very ad hoc process on just the overall totals.
...
PCT21 is tract level group quarters, with sex by age.
...
Joining PCT21 (tr_sa_gq_10) with P43 (bggq_10), since PCT21 just adds group quarter specifics. This follows the rules for triangle commutativity, but has a potential of mismatching at the block group level so we move it over with a caveat by putting a tr_ prefix on the variable.
...
But now we need to do some better matching, and see if that could also help us determine which group quarter specifics belong in each block group. We know how many we're supposed to have by race and how many by ethnicity of "H" and "I" in households and group quarters, but not in the same place, and we know some age_range information on group quarters, but not at as granular of a level as for the households. What we want to do is a matching that reduces down through a matching algorithm, but on several overlapping categories. First, we'll add counts by several important subsets to the bgSAR_10 table. Then, as we add the others, we can be sure we haven't excluded any. 

Try to do it for adding to ethnicity (tr_e_gq_10) to race (tr_r_gq_10), then see if adding the counts helps us determine how to build the larger table.
...
tr_r_gq_10[re_code=="A"&is.na(tr_e_gq),("tr_r_gq_match1_id"):=
                    paste0(tract,gq_type,gq_specs,as.character(100000+sample(1:.N))),
                  by=.(tract,gq_type,gq_specs)]
tr_e_gq_10[re_code=="H",("tr_r_gq_match1_id"):=
                    paste0(tract,gq_type,gq_specs,as.character(100000+sample(1:.N))),
                  by=.(tract,gq_type,gq_specs)]
tr_r_gq_10[re_code=="A"&is.na(tr_e_gq),("tr_e_gq"):=
...
tr_r_gq_10[re_code=="A",("ethnicity"):=list(tr_e_gq)]
table(tr_r_gq_10$tr_e_gq,tr_r_gq_10$ethnicity)
#now do re_code!="A"; the remaining hispanic are tr_eh_remain_count as an integer for everyone in that subgroup
tr_r_gq_10[is.na(tr_e_gq),("tr_r_gq_match2_id"):=
                    paste0(tract,gq_type,gq_specs,as.character(100000+sample(1:.N))),
                  by=.(tract,gq_type,gq_specs)]
...
tr_e_gq_10[is.na(matched),("tr_eh_remain_count"):=.N,
           by=.(tract,gq_type,gq_specs)]
tr_r_gq_10[is.na(tr_e_gq),("tr_eh_remain_count"):=
                    tr_e_gq_10[.SD, list(as.integer(tr_eh_remain_count)), 
                               on = .(tr_r_gq_match2_id)]]
#and for matching with over 17
...
##tests - not sure...
#then add something to the blanks so we know what they might have been??? Is that a density?
...
tr_r_gq_10[is.na(ethnicity)&order(tr_eh_remain_type_count),
           ...
           by = .(tract,gq_type)]
paste0("There remain ",nrow(tr_e_gq_10[re_code=="H"])-(nrow(tr_r_gq_10[re_code=="A"])-nrow(tr_e_gq_10[re_code=="I"]))," non-White Hispanic or Latino individuals that have not been assigned")  
nrow(tr_e_gq_10[is.na(matched)])==nrow(tr_e_gq_10[re_code=="H"])-
  (nrow(tr_r_gq_10[re_code=="A"])-nrow(tr_e_gq_10[re_code=="I"]))
```

Once we have expanded each matching category to have the count for that category of possible Hispanic or Latino, which is less than double the ones who matched, we know by adjunction that the ones without any matching categories across guest quarter specifics for that tract must not be Hispanic or Latino. 
...
tr_e_gq_over17_10[re_code=="I",("tr_r_gq_match_id"):=
                    paste0(tract,sex,gq_type,as.character(100000+sample(1:.N))),
                  by=.(tract,sex,gq_type)]
tr_r_gq_over17_10[re_code=="A",("tr_e_gq"):=
...
#now do ones who are re_code=="A" for race and "H" for ethnicity.
tr_r_gq_over17_10[re_code=="A"&is.na(tr_e_gq),("tr_r_gq_match1_id"):=
                    paste0(tract,gq_type,as.character(100000+sample(1:.N))),
                  by=.(tract,gq_type)]
tr_e_gq_over17_10[re_code=="H",("tr_r_gq_match1_id"):=
                    paste0(tract,gq_type,as.character(100000+sample(1:.N))),
                  by=.(tract,gq_type)]
tr_r_gq_over17_10[re_code=="A"&is.na(tr_e_gq),("tr_e_gq"):=
...
#those count as ethnicity known
tr_r_gq_over17_10[re_code=="A",("ethnicity"):=list(tr_e_gq)]
...
tr_r_gq_over17_10[is.na(tr_e_gq),("tr_r_gq_match2_id"):=
                    paste0(tract,sex,gq_type,as.character(100000+sample(1:.N))),
                  by=.(tract,sex,gq_type)]
...
tr_e_gq_over17_10[is.na(matched),("tr_eh_remain_type_count"):=.N,
           by=.(tract,sex,gq_type)]
tr_r_gq_over17_10[is.na(tr_e_gq),("tr_eh_remain_type_count"):=
                    tr_e_gq_over17_10[.SD, 
...
nrow(tr_r_gq_10[!is.na(sex)])
#number that didn't have ethnicity
nrow(tr_r_gq_10[is.na(ethnicity)])
#number that do not have ethnicity matched and did not get sex (or over 18) assigned
...
nrow(tr_r_gq_10[is.na(sex)]), " still need a final determination in the overall group quarters table")
```

Each of that almost 20k has the counts attached to them; can we use the other tables to shrink the size of the unknowns?
Should try to draw it!!!
IS IT JUST THAT THE SMALLER THE DENOMINATOR (I.E., THE COUNT), THE FURTHER TO THE LEFT IT GOES AND ONE IS FILLING IN FROM THE LEFT FOR EACH CATEGORY??? WHAT DOES THAT LOOK LIKE FOR THE ACTUAL ASSIGNMENT? DOES IT LOOK SOMETHING LIKE, IF TMP AND NEW, THEN YES, ELSE TMP2?  
Not a description of the likelihood (or probability) that one individual has a particular characteristic, but the description of how the relations are distributed within the whole (also described as a fraction). 

Our next step is to see if using the data on sex, age range and group quarters types and specifics, by tract, we can further eliminate some of the categories we might assign, in preparation for then using it with the full population and household tables to finally assign all the possibilities. We take advantage of the fact that sex has only been assigned where we are sure.

```{r age_range used to narrow down group quarter tables}
...
nrow(tr_r_gq_10[!is.na(matched_sts)])==nrow(bggq_10[!is.na(sts_ethnicity)]) #FALSE b/c a few are NAs because of age<18
nrow(tr_r_gq_10[!is.na(sex)])
```

```{r preliminary sts_re_code and age range assignments}
tr_r_gq_10[is.na(matched_sts),("tr_sts_remain_count"):=.N,
           by=.(tract,sex,gq_type,gq_specs)]
#taking sex out of the match, although it's still in the counts.
tr_r_gq_10[is.na(matched_sts),("tr_sts_gq_match_id"):=
                    paste0(tract,gq_type,gq_specs,as.character(100000+sample(1:.N))),
                  by=.(tract,gq_type,gq_specs)]
bggq_10[is.na(sts_re_code),("tr_sts_gq_match_id"):=
                    paste0(tract,gq_type,tr_gq_specs,as.character(100000+sample(1:.N))),
                  by=.(tract,gq_type,tr_gq_specs)]
bggq_10[is.na(sts_re_code),c("sts_re_code","sts_ethnicity","tr_sts_remain_count"):=
                    tr_r_gq_10[.SD,c(list(re_code),list(ethnicity),list(tr_sts_remain_count)), 
                     on = .(tr_sts_gq_match_id)]]
...
nrow(tr_r_gq_10[!is.na(matched_sts)])
nrow(tr_r_gq_10[!is.na(matched_sts)])==nrow(bggq_10[!is.na(sts_re_code)])
nrow(bggq_10[is.na(sts_re_code)])==0
nrow(tr_r_gq_10[!is.na(matched_sts)])==nrow(bggq_10[!is.na(sts_ethnicity)]) #FALSE b/c a few are NAs because of age<18
```
...
We had hoped that some of the age range options wouldn't match, so that we could assign the "Under 18 years" to some of the rows by elimination, but that hadn't happened. What we could do is use the ones where we're sure that race and ethnicity match (i.e., bggq_10[is.na(tr_sts_remain_count)] and trHHr_10[re_code=="A"]) and get as many on that as possible out of the overall.
...
length(test[test==FALSE])==0
test <- table(bggq_10[sts_re_code=="A"&age_range!="Under 18 years",tract],
              bggq_10[sts_re_code=="A"&age_range!="Under 18 years",sex],
              bggq_10[sts_re_code=="A"&age_range!="Under 18 years",sts_re_code],
              bggq_10[sts_re_code=="A"&age_range!="Under 18 years",sts_ethnicity] 
              )==
  table(bgSAR_10[re_code=="A"&household!="In household"&first_age>17,tract],
        bgSAR_10[re_code=="A"&household!="In household"&first_age>17,sex],
        bgSAR_10[re_code=="A"&household!="In household"&first_age>17,re_code],
        bgSAR_10[re_code=="A"&household!="In household"&first_age>17,ethnicity])
length(test[test==FALSE])==0 #FALSE b/c ??????
```

Group quarters are not concentrated, with most block groups having only one kind of residence, even as specified at the detailed level, which means that further specification by age isn't necessary. As part of being as complete as possible, we add a bit of a step to give preference to some things known outside the data tables themselves, as a matter of interpretation. Although the actual distribution still decides the structure of the final distribution, it is possible for some push toward one or the other subtype to be given with this technique.
...
We know the area around campus well, and there are some group quarters for students mixed in with residential neighborhoods with a broad range of children. First, let's see if in that block group, the elimination through assignment of people listed as "In households" provides enough distinction. We find that everyone in that census block who is in group quarters is in student housing, and that no one under 15 is listed as such; however, we also find a number of older people. The totals for the populations match, and can be represented as commuting in terms of the relations between age, sex, race, and ethnicity. 

```{r looking at group quarters after households removed for age range}
nrow(bggq_10[geoid=="48_201_312000_2"&sa_re_code=="A"])
nrow(bggq_10[tract=="48201312000"&sa_re_code=="A"])
...
#and by type - something is slightly off...
...
nrow(trHHr_10_initial[is.na(name)])==0
test <- table(
...
test <- table(
  ...
  bggq_10[age_range!="Under 18 years",institutionalized],
  bggq_10[age_range!="Under 18 years",gq_type],
  bggq_10[age_range!="Under 18 years",gq_specs]
)
length(test[test==FALSE])==0
```
