...
#ugh - coming up about 125 off, so trying a redo...
Before we go too far, let's see if the _initial file has the matches to the group quarters in a way that makes the individual determinations more effective.
Looks like _initial didn't really work - need to just do an anti-join??
...
tr_sa_gq_10[,("bg_sa_gq_match_id"):=
                    paste0(tract,sex,age_range,gq_type,as.character(100000+sample(1:.N))),
                  by=.(tract,sex,age_range,gq_type)]
bggq_10[,("gq_spec_count_sa"):=
                    tr_sa_gq_10[.SD, list(gq_spec_count_sa), on = .(bg_sa_gq_match_id)]]
#tests
nrow(bggq_10[is.na(gq_spec_count_sa)])==0
test <- summary(bggq_10[,gq_spec_count_sa])==
  summary(tr_sa_gq_10[,gq_spec_count_sa])
length(test[test==FALSE])==0
```
