...
We're doing a join, and some times the join could be done with one of the built-in merge functions. Here, because we want to emphasize the questions about the ordering and relations in terms of those orders, we make explicit the process of assigning numbers to each individual within a subset and then matching on those numbers to add variables to the other tables.
...
trHHr_10_initial <- trHHr_10[bgSAR_initial_10, on=.(tract,race,ethnicity,sex,age_range,first_age,last_age)]
nrow(trHHr_10_initial)==nrow(bgSAR_10)
test <- table(trHHr_10_initial[,tract],
              trHHr_10_initial[,race],
              trHHr_10_initial[,sex],
              trHHr_10_initial[,age_range]
) == table(
  bgSAR_10[,tract],
  bgSAR_10[,race],
  bgSAR_10[,sex],
...
length(test[test==FALSE])==0
test <- table(trHHr_10_initial[race=="A",tract],
              trHHr_10_initial[race=="A",ethnicity],
              trHHr_10_initial[race=="A",sex],
              trHHr_10_initial[race=="A",age_range]
) == table(
...
length(test[test==FALSE])==0
```
What we have now is a table with the correct number of total rows, with all sex, race, and age_range characteristics properly distributed. Ethnicity is correctly distributed for race=="A" and we know the number of Hispanics that need to be distributed at the tract level. (Will we be able to change some of the block group distribution?).  
...
Joining PCT21 (tr_sa_gq_10) with P43 (bggq_10), since PCT21 just adds group quarter specifics, but do it as [to be gq_spec_remain calculated already on the tract level at PCT21]

```{r add gq_spec_remain to bggq from tr_sa_gq}
tr_sa_gq_10[,("gq_spec_remain"):=.N,by=c("tract","sex","age_range","institutionalized","gq_type","gq_specs")]
...
tr_sa_gq_10[,("bg_sa_gq_match_id"):=
                  ...
                  by=.(tract,sex,gq_type,age_range)]
bggq_10[,("gq_spec_remain"):=
                    tr_sa_gq_10[.SD, list(gq_spec_remain), on = .(bg_sa_gq_match_id)]]
#tests

```

Joining PCT20 and PCT22, since they have the same race, ethnicity, and group quarters type. PCT22 is for age over 17 only, but the under 18 group is relatively small (1,927, or less than 5%). Let's try a left-join.

```{r add gq_spec_remain_age to bggq from tr_sa_gq}
tr_sa_gq_10[,("gq_spec_remain"):=.N,by=c("tract","sex","age_range","institutionalized","gq_type","gq_specs")]
bggq_10[,("bg_sa_gq_match_id"):=
                    paste0(tract,sex,age_range,gq_type,as.character(100000+sample(1:.N))),
                  by=.(tract,sex,gq_type,age_range)]
tr_sa_gq_10[,("bg_sa_gq_match_id"):=
                    paste0(tract,sex,gq_type,age_range,as.character(100000+sample(1:.N))),
                  by=.(tract,sex,gq_type,age_range)]
bggq_10[,("gq_spec_remain_age"):=
                    tr_sa_gq_10[.SD, list(gq_spec_remain), on = .(bg_sa_gq_match_id)]]
#tests

```

```{r add sex to tr_r_gq from tr_r_gq_over17}
#first, add sex_remain and age_remain by gq_spec to tr_r_gq
#second, add over17 sex by race with remains by race all the way to gq_spec
```

```{r add gq_race_remain to bggq from tr_r_gq}
tr_r_gq_10[,("gq_race_remain_type"):=.N,by=c("tract","race","institutionalized","gq_type")]
tr_r_gq_10[,("gq_race_remain_spec"):=.N,by=c("tract","race","institutionalized","gq_type","gq_specs")]
...
tr_sa_gq_10[,("bg_r_gq_match_id"):=
                    paste0(tract,sex,gq_type,age_range,as.character(100000+sample(1:.N))),
                  by=.(tract,sex,gq_type,age_range)]
bggq_10[,c("gq_race_remain_type","gq_race_remain_spec"):=
                    tr_sa_gq_10[.SD, c(list(gq_race_remain_type),list(gq_race_remain_spec)), on = .(bg_r_gq_match_id)]]
#tests
...
THEN put the remains stuff on to the _initial table on tract and solve...
