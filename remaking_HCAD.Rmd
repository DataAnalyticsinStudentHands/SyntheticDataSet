...
#HCAD Site Codes are in: https://hcad.org/hcad-resources/hcad-appraisal-codes/hcad-site-codes/
...
HCAD_res_build <- HCAD_res_build %>% 
  rename(account=acct,state_code=property_use_cd,improv_typ=impr_tp,impr_mdl_cd=impr_mdl_cd,
         Depr_Val=dpr_val,qual_descript=dscr,living_sf=im_sq_ft) %>%
  select(account,state_code,improv_typ,impr_mdl_cd,structure_dscr,
          Depr_Val,qual_descript,date_erected,yr_remodel,
...
HCAD_real_build <- HCAD_real_build %>% 
  ...
  select(account,state_code,improv_typ,impr_mdl_cd,Depr_Val,qual_descript,
...
#         bldg_data_descr=type_descr,num_units=units) %>%
#  #distinct(account,.keep_all = TRUE) %>%
#  filter(!row_number()==1) %>%
