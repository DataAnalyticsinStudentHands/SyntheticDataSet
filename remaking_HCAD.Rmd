...
ifelse(file.exists(paste0(housingdir, vintage, "/Parcels/Parcels.shp")),
       ...
       HCAD_parcels <- st_read(paste0(housingdir, vintage, "/Parcels_", vintage, "_Oct/Parcels.shp"),stringsAsFactors = FALSE,quiet = TRUE))
...
#  mutate(
#      exempt_cat_vec = as.vector(str_split," "), #need 
...
#        str_split(exempt_cat," ") %in% c('RES','PAR') ~ TRUE
#      ),
#      vet = case_when(
...
HCAD_fixtures <- HCAD_fixtures %>%
  ...
  group_by(account) %>%
  summarise(total_num_units = sum(num_apts)) 
...
HCAD_geom_dt <- HCAD_res_fixt_dt[HCAD_geom_dt,on=.(account)]
#275k without matches!!!
nrow(HCAD_geom_dt_2[is.na(improv_typ)])
...
CensusSubBlockforHCADParcels <- st_within(HCAD_geom$centroid, census_sub_blocks)
#unlist into vector
...
CensusSubBlock19forHCADParcels <- st_within(HCAD_geom$centroid, census_sub_blocks19)
#unlist into vector
...
# add census geoid information to each parcel
HCAD_geom$subgeoid=census_sub_blocks$GEOID[CensusSubBlock19forHCADParcelsunlisted]
